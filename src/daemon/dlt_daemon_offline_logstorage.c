    ...
    int verbose)
{
    ...
    //TBD: REVIEW check if need to pass ecuidlen also
    ...
    //TBD: REVIEW check apid is null terminated, to use strlen(apid)
    req.apidlen = strlen(apid);
    dlt_set_id_v2(req.apid, apid, req.apidlen);
    //TBD: REVIEW check ctid is null terminated, to use strlen(apid)
                                                  ...
                                                  int verbose)
{
    ...
    //TBD: Check ecuid length is captured using strlen in runtime
    ...
    for (i = 0; i < user_list->num_contexts; i++) {
        if (cmp_flag == DLT_DAEMON_LOGSTORAGE_CMP_APID) {
            //TBD: Check tmp_id_size = apid2len + 1 is required for null termination
            tmp_id_size = user_list->contexts[i].apid2len + 1;
            dlt_set_id_v2(&tmp_id, user_list->contexts[i].apid2, user_list->contexts[i].apid2len);
        }
        else if (cmp_flag == DLT_DAEMON_LOGSTORAGE_CMP_CTID) {
            //TBD: Check tmp_id_size = ctid2len + 1 is required for null termination

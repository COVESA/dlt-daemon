    ...
    *num_keys = 0;
         ...
         (apids && strcmp(apids, ".*") == 0 && ctids && strcmp(ctids, ".*") == 0 && ecuid)))
    {
        ...
        *keys = calloc(*num_keys, DLT_OFFLINE_LOGSTORAGE_MAX_KEY_LEN);
        ...
        int str_ret = snprintf(*keys, DLT_OFFLINE_LOGSTORAGE_MAX_KEY_LEN, "%s", curr_key);
        if (str_ret < 0 || str_ret >= DLT_OFFLINE_LOGSTORAGE_MAX_KEY_LEN) {
                dlt_vlog(LOG_ERR, "Key truncated: %s\n", curr_key);
                return -1;
        }
        return 0;
    }
    ...
    *keys = calloc(*num_keys, DLT_OFFLINE_LOGSTORAGE_MAX_KEY_LEN);
    ...
    for (int i = 0; i < num_apids; i++) {
        ...
        for (int j = 0; j < num_ctids; j++) {
            ...
            int str_ret = snprintf(*keys + (key_index * DLT_OFFLINE_LOGSTORAGE_MAX_KEY_LEN),

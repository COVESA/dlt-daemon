                                      ...
                                      int size, int html)
{
    ...
    if (rest > 0) {
        ...
        if (dlt_print_hex_string(
                text, textlength,
                (uint8_t *)(ptr + ((size / DLT_COMMON_HEX_CHARS) *
                                   DLT_COMMON_HEX_CHARS)),
                rest) < DLT_RETURN_OK)
            return DLT_RETURN_ERROR;
        text += 2 * rest + (rest - 1);
                                      ...
                                      size_t textlength, int type, int verbose)
{
    ...
    if (DLT_MSG_IS_NONVERBOSE_V2(msg)) {
        // In version 2, message ID is not part of payload but stored in header
                     ...
                     int resync, int verbose)
{
    ...
    if (length < (size_t)((int32_t)msg->headersize -
                          (int32_t)sizeof(DltStorageHeader) + msg->datasize))
...
#pragma GCC diagnostic ignored "-Wdate-time"
    snprintf(buf, size,
             ...
             _DLT_PACKAGE_REVISION, __DATE__, __TIME__, _DLT_SYSTEMD_ENABLE,
             _DLT_SYSTEMD_WATCHDOG_ENABLE, _DLT_TEST_ENABLE, _DLT_SHM_ENABLE);

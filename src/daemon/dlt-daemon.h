...
typedef struct {
    ...
    char ivalue[NAME_MAX + 1]; /**< (String: Directory) Directory where to store the persistant configuration (Default:
                                  /tmp) */
    ...
    char avalue[NAME_MAX + 1]; /**< (String: Directory) Filename of the app id default level config (Default:
                                  /etc/dlt-log-levels.conf) */
#endif
#ifdef DLT_TRACE_LOAD_CTRL_ENABLE
    char lvalue[NAME_MAX + 1]; /**< (String: Directory) Filename of DLT trace limit file (Default:
                                  /etc/dlt-trace_load.conf) */
#endif
    ...
    char offlineTraceDirectory[DLT_DAEMON_FLAG_MAX]; /**< (String: Directory) Store DLT messages to local directory
    ...
    bool offlineTraceFilenameTimestampBased; /**< (Boolean) timestamp based or index based (Default: true=Timestamp
                                                based)                            */
    ...
    char loggingFilename[DLT_DAEMON_FLAG_MAX]; /**< (String: Filename) The logging filename if internal logging mode is
                                                  log to file (Default: /tmp/log) */
    ...
    char pathToECUSoftwareVersion[DLT_DAEMON_FLAG_MAX];    /**< (String: Filename) The file from which to read the ECU
                                                              version from.                                */
    char ecuSoftwareVersionFileField[DLT_DAEMON_FLAG_MAX]; /**< Reads a specific VALUE from a FIELD=VALUE ECU version
                                                              file.                                         */
    ...
    unsigned int
        offlineLogstorageMaxCounter; /**< (int) Maximum offline logstorage file counter index until wraparound */
    ...
    char
        userPipesDir[DLT_PATH_MAX]; /**< (String: Directory) directory where dltpipes reside (Default: /tmp/dltpipes) */
    ...
    int autoResponseGetLogInfoOption;            /**< (int) The Option of automatic get log info response during context
    ...
    int contextTraceStatus;    /**< (int) trace status sent to context if registered with default trace status  or if
                                  enforced    */
    int enforceContextLLAndTS; /**< (Boolean) Enforce log-level, trace-status not to exceed contextLogLevel,
                                  contextTraceStatus   */
    DltBindAddress_t*
        ipNodes; /**< (String: BindAddress) The daemon accepts connections only on this list of IP addresses        */

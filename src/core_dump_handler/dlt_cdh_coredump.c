...
cdh_status_t read_elf_headers(proc_info_t *p_proc)
{
    ...
    p_proc->m_pPhdr =
        (ELF_Phdr *)malloc(sizeof(ELF_Phdr) * p_proc->m_Ehdr.e_phnum);

    if (p_proc->m_pPhdr == NULL) {
        syslog(LOG_ERR, "Cannot allocate Phdr memory (%d headers)",
               p_proc->m_Ehdr.e_phnum);
        return CDH_NOK;
    }

    for (phnum = 0; phnum < p_proc->m_Ehdr.e_phnum; phnum++)
        /* Read Programm header */
        stream_read(&p_proc->streamer, &p_proc->m_pPhdr[phnum],
                    sizeof(ELF_Phdr));
...
int getNotePageIndex(proc_info_t *p_proc)
{
    ...
    for (i = 0; i < p_proc->m_Ehdr.e_phnum; i++) {
        syslog(LOG_INFO,
               "==Note section prog_note:%d type:0x%X offset:0x%X size:0x%X "
               "(%dbytes)",
               i, p_proc->m_pPhdr[i].p_type, p_proc->m_pPhdr[i].p_offset,
               p_proc->m_pPhdr[i].p_filesz, p_proc->m_pPhdr[i].p_filesz);
...
cdh_status_t read_notes(proc_info_t *p_proc)
{
    ...
    if (stream_move_to_offest(&p_proc->streamer,
                              p_proc->m_pPhdr[prog_note].p_offset) != CDH_OK) {
        syslog(LOG_ERR, "Cannot move to note header");
        return CDH_NOK;
    }

    if ((p_proc->m_Nhdr =
             (char *)malloc(p_proc->m_pPhdr[prog_note].p_filesz)) == NULL) {
        syslog(LOG_ERR, "Cannot allocate Nhdr memory (note size %d bytes)",
               p_proc->m_pPhdr[prog_note].p_filesz);
        return CDH_NOK;
    }

    if (stream_read(&p_proc->streamer, p_proc->m_Nhdr,
                    p_proc->m_pPhdr[prog_note].p_filesz) != CDH_OK) {
...
cdh_status_t treat_coredump(proc_info_t *p_proc)
{
    ...
    if (read_elf_headers(p_proc) == CDH_OK) {
        /* TODO: No NOTES here leads to crash elsewhere!!! dlt_cdh_crashid.c:
         * around line 76 */
...
finished:
    ...
    /* In all cases, we try to finish to read/compress the coredump until the
     * end */

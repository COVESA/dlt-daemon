...
char* origin_name(char* src)
{
    if (strlen((char*)basename(src)) > 10) {
        ...
        DLT_LOG(dltsystem, DLT_LOG_ERROR, DLT_STRING("dlt-system-filetransfer, error in recreating origin name!"));
        return NULL;
    }
}
...
{
    DLT_LOG(dltsystem, DLT_LOG_DEBUG, DLT_STRING("dlt-system-filetransfer, creating unique temporary file name."));
    time_t t = time(NULL);
    int ok;
...
void send_dumped_file(FiletransferOptions const* opts, char* dst_tosend)
{
    ...
    DLT_LOG(dltsystem, DLT_LOG_DEBUG, DLT_STRING("dlt-system-filetransfer, sending dumped file:"), DLT_STRING(fn));
    ...
    DLT_LOG(dltsystem, DLT_LOG_DEBUG, DLT_STRING("dlt-system-filetransfer, sent dumped file"));
}
...
{
    DLT_LOG(
        dltsystem, DLT_LOG_DEBUG, DLT_STRING("dlt-system-filetransfer, compressing file from:"), DLT_STRING(src),
        DLT_STRING("to:"), DLT_STRING(dst));
    char* buf;
...
{
    DLT_LOG(dltsystem, DLT_LOG_DEBUG, DLT_STRING("dlt-system-filetransfer, sending a file."));
    ...
    if (rn == NULL) {
        DLT_LOG(dltsystem, DLT_LOG_ERROR, DLT_STRING("file information not available, may be file got overwritten"));
        return -1;
    }
    ...
    if (opts->Compression[which] > 0) {
        ...
        if (rename(src, dst_tocompress) < 0) {
            DLT_LOG(
                dltsystem, DLT_LOG_ERROR, DLT_STRING("Could not move file"), DLT_STRING(src),
        ...
        len = strlen(fdir) + strlen(SUBDIR_TOSEND) + strlen(rn) + strlen(COMPRESS_EXTENSION)
        ...
        DLT_LOG(dltsystem, DLT_LOG_DEBUG, DLT_STRING("dlt-system-filetransfer, Moving file to tmp directory."));
        ...
        DLT_LOG(
            dltsystem, DLT_LOG_DEBUG, DLT_STRING("dlt-system-filetransfer, Rename:"), DLT_STRING(src),
            DLT_STRING("to: "), DLT_STRING(dst_tosend));

        /*moving in subdir, from where it can be compressed */
        if (rename(src, dst_tosend) < 0) {
            DLT_LOG(
                dltsystem, DLT_LOG_ERROR, DLT_STRING("Could not move file"), DLT_STRING(src), DLT_STRING(dst_tosend));
            free(rn);
            free(dst_tosend);
            free(src_copy);
            return -1;
    ...
    DLT_LOG(dltsystem, DLT_LOG_DEBUG, DLT_STRING("dlt-system-filetransfer, File ready to send"));
...
int flush_dir_send(FiletransferOptions const* opts, const char* compress_dir, const char* send_dir)
{
    ...
    if (dir != NULL) {
        while ((dp = readdir(dir)) != NULL) {
            ...
            /*if we have a file here and in the to_compress dir, we delete the to_send file: we can not be sure, that it
             * has been properly compressed! */
            if (!strncmp(
                    dp->d_name + strlen(dp->d_name) - strlen(COMPRESS_EXTENSION), COMPRESS_EXTENSION,
                ...
                if (stat(path_uncompressed, &sb) == -1) {
                    ...
                    DLT_LOG(dltsystem, DLT_LOG_DEBUG, DLT_STRING("dlt-system-filetransfer, sending file."));

                    send_dumped_file(opts, fn);
                } else {
                    /*There is an uncompressed file. Compression seems to have been interrupted -> delete the compressed
                     * file instead of sending it! */
                    DLT_LOG(
                        dltsystem, DLT_LOG_DEBUG,
                        DLT_STRING("dlt-system-filetransfer, uncompressed version exists. Deleting partially "
                                   "compressed version."));

                    if (sb.st_mode & S_IFREG) {
                        if (remove(fn) != 0)
                            /*"Error deleting file". Continue? If we would cancel, maybe the dump is never sent!
                             * Deletion would again be tried in next LC. */
                            DLT_LOG(dltsystem, DLT_LOG_ERROR, DLT_STRING("Error deleting file:"), DLT_STRING(fn));
                    } else {
                        /*"Oldfile is a not reg file. Is this possible? Can we compress a directory?:
                         * %s\n",path_uncompressed); */
                        DLT_LOG(
                            dltsystem, DLT_LOG_DEBUG,
                            DLT_STRING("dlt-system-filetransfer, Oldfile is a not regular file! Do we have a problem?"),
                            DLT_STRING(fn));
                    }
                }
        ...
        DLT_LOG(dltsystem, DLT_LOG_ERROR, DLT_STRING("Could not open directory"), DLT_STRING(send_dir));
        return -1;
    }
...
{
    /*check for files in compress_dir. Assumption: a file which lies here, should have been compressed, but that action
     * was interrupted. */
    ...
    if (dir != NULL) {
        while ((dp = readdir(dir)) != NULL) {
            ...
            DLT_LOG(
                dltsystem, DLT_LOG_DEBUG, DLT_STRING("dlt-system-filetransfer, old file found in compress-directory."));
        ...
        DLT_LOG(dltsystem, DLT_LOG_ERROR, DLT_STRING("Could not open directory"), DLT_STRING(compress_dir));
        return -1;
    }
...
int flush_dir_original(FiletransferOptions const* opts, int which)
{
    ...
    if (dir != NULL) {
        while ((dp = readdir(dir)) != NULL) {
            ...
            DLT_LOG(dltsystem, DLT_LOG_DEBUG, DLT_STRING("dlt-system-filetransfer, old file found in directory."));
        ...
        DLT_LOG(dltsystem, DLT_LOG_ERROR, DLT_STRING("Could not open directory"), DLT_STRING(sdir));
        return -1;
    }
...
{
    DLT_LOG(dltsystem, DLT_LOG_DEBUG, DLT_STRING("dlt-system-filetransfer, flush directory of old files."));
...
int init_filetransfer_dirs(DltSystemConfiguration* config)
{
    ...
    for (i = 0; i < opts->Count; i++) {
        ...
        if ((0 != ret) && (EEXIST != errno)) {
            DLT_LOG(
                dltsystem, DLT_LOG_ERROR, DLT_STRING("dlt-system-filetransfer, error creating subdirectory: "),
                DLT_STRING(subdirpath), DLT_STRING(" Errorcode: "), DLT_INT(errno));
            free(subdirpath);
            return -1;
        }
...
#ifdef linux
        ino.fd[i] = inotify_add_watch(ino.handle, opts->Directory[i], IN_CLOSE_WRITE | IN_MOVED_TO);

        if (ino.fd[i] < 0) {
            char buf[1024];
            snprintf(
                buf, 1024, "Failed to add inotify watch to directory %s in dlt-system file transfer.",
                opts->Directory[i]);
        ...
        if (ie->len > 0) {
            if ((ie->mask & IN_CLOSE_WRITE) || (ie->mask & IN_MOVED_TO)) {
                ...
                for (j = 0; j < opts->Count; j++)
                    if (ie->wd == ino.fd[j]) {
                        DLT_LOG(
                            dltsystem, DLT_LOG_DEBUG, DLT_STRING("dlt-system-filetransfer, found new file."),
                            DLT_STRING(ie->name));
                        ...
                        if (length > PATH_MAX) {
                            DLT_LOG(
                                filetransferContext, DLT_LOG_ERROR,
                                DLT_STRING("dlt-system-filetransfer: Very long path for file transfer. Cancelling "
                                           "transfer! Length is: "),
                                DLT_INT((int)length));
                            return -1;
                        }

    ...
    if (strncmp(answer, resp_ok, strlen(resp_ok)) == 0) {
        ...
        if (get_command() == DLT_SERVICE_ID_PASSIVE_NODE_CONNECTION_STATUS) {
            if ((int)sizeof(DltServicePassiveNodeConnectionInfo) > len) {
                ...
                pr_verbose("Expected: %zu,\nreceived: %d", sizeof(DltServicePassiveNodeConnectionInfo), len);
                ret = -1;
            } else {
                DltServicePassiveNodeConnectionInfo* info = (DltServicePassiveNodeConnectionInfo*)(payload);
...
DltControlMsgBody* dlt_passive_node_prepare_message_body()
{
    ...
    if (get_command() == DLT_SERVICE_ID_PASSIVE_NODE_CONNECT) {
        ...
        DltServicePassiveNodeConnect* serv = (DltServicePassiveNodeConnect*)mb->data;
        ...
        DltServicePassiveNodeConnectionInfo* serv = (DltServicePassiveNodeConnectionInfo*)mb->data;
...
static int dlt_passive_node_ctrl_single_request()
{
    ...
    if (dlt_control_init(dlt_passive_node_analyze_response, get_ecuid(), get_verbosity()) != 0) {
...
static void usage()
{
    ...
    printf(
        "Send a trigger to DLT daemon to (dis)connect a passive node "
...
int main(int argc, char* argv[])
{
    ...
    if ((get_command() == UNDEFINED)
        || ((get_command() == DLT_SERVICE_ID_PASSIVE_NODE_CONNECT) && (g_options.node_id[0] == '\0')

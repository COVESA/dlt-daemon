 ...
 * @param len            Key length
 * @param apid           Application ID from key is stored here
 * @param ctid           Context id from key is stored here
 * @return               0 on success -1 on error
 */
    ...
    char *ecuid, int loglevel, int verbose)
{
    ...
    if (strncmp(ecuid, daemon->ecuid, DLT_ID_SIZE) == 0) {
        old_log_level = (int)context->storage_log_level;
        ...
        old_log_level = (int)context->log_level;
    ...
    int max_device, int verbose)
{
    ...
    while (*(tmp) != NULL) {
        for (i = 0; i < (*tmp)->num_keys; i++) {
            ...
            strncpy(key,
                    ((*tmp)->key_list +
                     (long)(i * DLT_OFFLINE_LOGSTORAGE_MAX_KEY_LEN)),

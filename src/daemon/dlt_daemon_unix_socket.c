...
DltReturnValue dlt_daemon_unix_android_get_socket(int* sock, const char* sock_path)
{
    ...
    if ((sock == NULL) || (sock_path == NULL)) {
        ...
        if (sock_name == NULL) {
            dlt_log(LOG_WARNING, "dlt_daemon_unix_android_get_socket: can't get socket name from its path");
            ret = DLT_RETURN_ERROR;
        } else {
            *sock = android_get_control_socket(sock_name);
            if (*sock < 0) {
                dlt_log(LOG_WARNING, "dlt_daemon_unix_android_get_socket: can get socket from init");
                ret = DLT_RETURN_ERROR;
            } else {
                if (listen(*sock, 1) == -1) {

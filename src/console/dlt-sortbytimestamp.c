 ...
 */
void write_messages(int ohandle, DltFile* file, TimestampIndex* timestamps, uint32_t message_count)
{
    struct iovec iov[2];
...
int main(int argc, char* argv[])
{
    ...
    if ((begin < 0) || (end < 0) || (begin > end) || (begin >= file.counter) || (end >= file.counter)) {
    ...
    for (num = begin; num <= end; num++) {
        ...
        delta_systime =
            (uint32_t)llabs((int64_t)timestamp_index[curr_idx + 1].systmsp - timestamp_index[curr_idx].systmsp);
        ...
        if (delta_tmsp > FIFTY_SEC_IN_MSEC || delta_systime >= THREE_MIN_IN_SEC) {
            ...
            for (i = 0; i < count; i++) {
                memcpy((void*)&temp_timestamp_index[i], (void*)&timestamp_index[start + i], sizeof(TimestampIndex));
            }
            qsort((void*)temp_timestamp_index, count, sizeof(TimestampIndex), compare_index_timestamps);
    ...
    if (count == message_count) {
        qsort((void*)timestamp_index, message_count + 1, sizeof(TimestampIndex), compare_index_timestamps);

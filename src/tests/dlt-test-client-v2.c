 ...
 *
 * \file dlt-test-client-v2.c
 */

/*******************************************************************************
**                                                                            **
**  SRC-MODULE: dlt-test-client-v2.c                                          **
**                                                                            **
**  TARGET    : linux                                                         **
**                                                                            **
**  PROJECT   : DLT                                                           **
...
int main(int argc, char *argv[])
{
    ...
    dlt_client_register_fetch_next_message_callback(
    ...
    if (dltdata.fvalue) {
        if (dlt_filter_load_v2(&(dltdata.filter), dltdata.fvalue,
                               dltdata.vflag) < DLT_RETURN_OK) {
    ...
    if (dlt_client_connect(&g_dltclient, dltdata.vflag) != DLT_RETURN_ERROR) {
        ...
        dlt_client_main_loop_v2(&g_dltclient, &dltdata, dltdata.vflag);
...
int dlt_testclient_message_callback(DltMessageV2 *message, void *data)
{
    ...
    if (DLT_IS_HTYP_WEID(message->baseheaderv2->htyp2))
        dlt_set_storageheader_v2(&(message->storageheaderv2), len,
    ...
    if ((dltdata->fvalue == 0) ||
        (dltdata->fvalue &&
         (dlt_message_filter_check_v2(message, &(dltdata->filter),
                                      dltdata->vflag) == DLT_RETURN_TRUE))) {

        dlt_message_header_v2(message, text, sizeof(text), dltdata->vflag);

        if (dltdata->aflag)
            printf("%s ", text);

        dlt_message_payload_v2(message, text, sizeof(text), DLT_OUTPUT_ASCII,
                               dltdata->vflag);
        ...
        if (strcmp(text, "Test1: (Macro IF) Test all log levels") == 0) {
            ...
            if (DLT_IS_HTYP2_EH(message->baseheaderv2->htyp2)) {
                if ((DLT_GET_MSIN_MSTP(message->headerextrav2.msin)) ==
                    DLT_TYPE_LOG) {
        ...
        /* check test 2m */
        if (strcmp(text,
                   "Test2: (Macro IF) Test all variable types (verbose)") ==
            0) {
                ...
                if ((DLT_GET_MSIN_MSTP(message->headerextrav2.msin)) ==
                    ...
                    if (message->headerextrav2.noar >= 2) {
                        ...
                        DLT_MSG_READ_VALUE(type_info_tmp, ptr, datalength,
                                           uint32_t);
                        type_info =
                            (uint32_t)type_info_tmp; // TBD: Verify How to get
                                                     // type_info in v2

                        if (type_info & DLT_TYPE_INFO_STRG) {
                            /* skip string */
                            DLT_MSG_READ_VALUE(length_tmp, ptr, datalength,
                                               int16_t);
                            ...
                            if (length >= 0) {
                                ...
                                DLT_MSG_READ_VALUE(type_info_tmp, ptr,
                                                   datalength, uint32_t);
                                type_info = (uint32_t)
                                    type_info_tmp; // TBD: Verify How to get
                                                   // type_info in v2

                                if ((type_info & DLT_TYPE_INFO_STRG) &&
                                    ((type_info & DLT_TYPE_INFO_SCOD) ==
                                     DLT_SCOD_ASCII)) {
                                    if (datalength ==
                                        (sizeof(uint16_t) +
        ...
        /* check test 3m */
        /* Non verbose is not supported in v2 */
        // if (strcmp(text, "Test3: (Macro IF) Test all variable types
        // (non-verbose)") == 0) {
        //     printf("Test3m: (Macro IF) Test all variable types
        ...
        //             if (datalength == sizeof(uint16_t) + strlen("Hello
        //             world") + 1)
        ...
        //         if ((slen >= 0) && (tc_old ==
        //         dltdata->test_counter_macro[2]))
        //             printf("ID=%d, Datalength=%d => Failed!", id,
        //             datalength);
        //     }
        // }

        /* check test 4m */
        if (strcmp(text, "Test4: (Macro IF) Test different message sizes") ==
            0) {
                ...
                if ((DLT_GET_MSIN_MSTP(message->headerextrav2.msin)) ==
                    ...
                    if (DLT_IS_MSIN_VERB(message->headerextrav2.msin)) {
                        ...
                        if (message->headerextrav2.noar == 2) {
                            ...
                            /* first read the type info of the first argument:
                             * must be string */
                            DLT_MSG_READ_VALUE(type_info_tmp, ptr, datalength,
                                               uint32_t);
                            type_info = (uint32_t)type_info_tmp;
        ...
        if (strcmp(text, "Test5: (Macro IF) Test high-level API") == 0) {
            ...
            if (strcmp(text, "00\'01\'02\'03\'04\'05\'06\'07\'08\'09\'0a\'0b\'0"
                             "c\'0d\'0e\'0f") == 0)
                dltdata->test_counter_macro[4]++;
        ...
        //             /* Check payload, must be two arguments (2 raw data
        //             blocks) */
        //             /* If the payload is correct, the counter is increased by
        //             1 */ if (message->headerextrav2.noar == 2) {
        //                 /* verbose mode */
        //                 type_info = 0;
        //                 type_info_tmp = 0;
        //                 length = 0, length_tmp = 0; /* the macro can set this
        //                 variable to -1 */
        ...
        //                 /* first read the type info of the first argument:
        //                 must be string */ DLT_MSG_READ_VALUE(type_info_tmp,
        //                 ptr, datalength, uint32_t); type_info =
        //                 (uint32_t)type_info_tmp; //TBD: Verify How to get
        //                 type_info in v2
        ...
        //                     /* skip string */
        //                     DLT_MSG_READ_VALUE(length_tmp, ptr, datalength,
        //                     uint16_t); length = (int16_t)length_tmp;
        ...
        //                         /* read type of second argument: must be raw
        //                         */ DLT_MSG_READ_VALUE(type_info_tmp, ptr,
        //                         datalength, uint32_t); type_info =
        //                         (uint32_t)type_info_tmp; //TBD: Verify How to
        //                         get type_info in v2
        ...
        /* check test 8m */
        // if (strcmp(text, "Test 8: (Macro IF) Test truncated network trace")
        // == 0) {
        ...
        //             blocks) */
        //             /* If the payload is correct, the counter is increased by
        //             1 */ if (message->headerextrav2.noar == 4) {
        //                 type_info = 0;
        //                 type_info_tmp = 0;
        //                 length = 0, length_tmp = 0; /* the macro can set this
        //                 variable to -1 */
        ...
        //                 DLT_MSG_READ_VALUE(type_info_tmp, ptr, datalength,
        //                 uint32_t); type_info = (uint32_t)type_info_tmp;
        ...
        //                     DLT_MSG_READ_VALUE(length_tmp, ptr, datalength,
        //                     uint16_t); length = (int16_t)length_tmp;
        //                     DLT_MSG_READ_STRING(chdr, ptr, datalength,
        //                     (int)sizeof(chdr), length);

        //                     if (strcmp((char *)chdr, DLT_TRACE_NW_TRUNCATED)
        //                     == 0)
        //                         dltdata->test_counter_macro[7]++;
        ...
        //                         DLT_MSG_READ_VALUE(length_tmp, ptr,
        //                         datalength, uint16_t); length =
        //                         (int16_t)length_tmp; DLT_MSG_READ_STRING(hdr,
        //                         ptr, datalength, (int)sizeof(hdr), length);
        ...
        //                             uint32_t orig_size;
        //                             DLT_MSG_READ_VALUE(length_tmp32, ptr,
        //                             datalength, uint32_t); orig_size =
        //                             (uint32_t)length_tmp32;
        ...
        //                             DLT_MSG_READ_VALUE(type_info_tmp, ptr,
        //                             datalength, uint32_t); type_info =
        //                             (uint32_t)type_info_tmp; //TBD: Verify
        //                             How to get type_info in v2
        ...
        //                                 /* Size of the truncated message
        //                                 after headers */ if (length ==
        //                                 DLT_USER_BUF_MAX_SIZE - 41 -
        ...
        /* check test 9m */
        // if (strcmp(text, "Test 9: (Macro IF) Test segmented network trace")
        // == 0) {
        ...
        //                     if (strcmp((char *)chdr, DLT_TRACE_NW_START) ==
        //                     0)
        //                         dltdata->test_counter_macro[8]++;
        ...
        //                                 /* Segmentcount */
        //                                 DLT_MSG_READ_VALUE(type_info_tmp,
        //                                 ptr, datalength, uint32_t); type_info
        //                                 = (uint32_t)type_info_tmp; //TBD:
        //                                 Verify How to get type_info in v2

        //                                 if (type_info & DLT_TYPE_INFO_UINT) {
        //                                     uint16_t scount;
        //                                     DLT_MSG_READ_VALUE(length_tmp,
        //                                     ptr, datalength, uint16_t);
        //                                     scount = (uint16_t)length_tmp;
        ...
        //                     char chdr[10];
        //                     DLT_MSG_READ_VALUE(length_tmp, ptr, datalength,
        //                     uint16_t); length = (uint16_t)length_tmp;
        //                     DLT_MSG_READ_STRING(chdr, ptr, datalength,
        //                     (int)sizeof(chdr), length);

        //                     if (strcmp((char *)chdr, DLT_TRACE_NW_SEGMENT) ==
        //                     0)
        //                         dltdata->test_counter_macro[8]++;
        ...
        /* check test 3f */
        // if (strcmp(text, "Test3: (Function IF) Test all variable types
        // (non-verbose)") == 0) {
        //     printf("Test3f: (Function IF) Test all variable types
        ...
        /* check test 4f */
        if (strcmp(text, "Test4: (Function IF) Test different message sizes") ==
            0) {
                        ...
                        if (message->headerextrav2.noar == 2) {
                            ...
                            if (type_info & DLT_TYPE_INFO_STRG) {
                                ...
                                if (length >= 0) {
                                    ...
                                    /* read type of second argument: should be
                                     * raw */
                                    DLT_MSG_READ_VALUE(type_info_tmp, ptr,
                                                       datalength, uint32_t);
                                    type_info = (uint32_t)
        ...
        //                 type_info_tmp = 0;
        //                 length = 0;
        //                 length_tmp = 0; /* the macro can set this variable to
        //                 -1 */
        ...
        //                 /* first read the type info of the first argument:
        //                 should be string */ DLT_MSG_READ_VALUE(type_info_tmp,
        //                 ptr, datalength, uint32_t); type_info =
        //                 (uint32_t)type_info_tmp; //TBD: Verify How to get
        //                 type_info in v2
        ...
        //                         /* read type of second argument: should be
        //                         raw */ DLT_MSG_READ_VALUE(type_info_tmp, ptr,
        //                         datalength, uint32_t); type_info =
        //                         (uint32_t)type_info_tmp; //TBD: Verify How to
        //                         get type_info in v2
        ...
        /* check test 8f */
        // if (strcmp(text, "Test 8: (Function IF) Test truncated network
        // trace") == 0) {
        ...
        /* check test 9f */
        // if (strcmp(text, "Test 9: (Function IF) Test segmented network
        // trace") == 0) {
        ...
        if (dltdata->xflag)
            dlt_message_print_hex_v2(message, text, DLT_TESTCLIENT_TEXTBUFSIZE,
                                     dltdata->vflag);
        else if (dltdata->mflag)
            dlt_message_print_mixed_plain_v2(
                message, text, DLT_TESTCLIENT_TEXTBUFSIZE, dltdata->vflag);
        else if (dltdata->sflag)
            dlt_message_print_header_v2(message, text, sizeof(text),
                                        dltdata->vflag);
        ...
        if (dltdata->ovalue) {
            ...
            if (0 > bytes_written) {
                printf("dlt_testclient_message_callback, error in: "

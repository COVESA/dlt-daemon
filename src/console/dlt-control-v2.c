...
int main(int argc, char *argv[])
{
    ...
    if (dltdata.evalue) {
        ...
        dlt_set_id_v2(&(dltdata.ecuid), dltdata.evalue, (int8_t)dltdata.ecuidlen);
        g_dltclient.ecuid2len = dltdata.ecuidlen;
        dlt_set_id_v2(&(g_dltclient.ecuid2), dltdata.evalue, (int8_t)dltdata.ecuidlen);
    }
    else {
        dltdata.evalue = NULL;
    ...
    if (dlt_client_connect(&g_dltclient, dltdata.vflag) != DLT_RETURN_ERROR) {
        ...
        if (dltdata.mvalue && dltdata.avalue && dltdata.cvalue) {
            ...
            if (dlt_client_send_inject_msg_v2(&g_dltclient,
                                           dltdata.avalue,
                                           dltdata.cvalue,
        ...
        else if (dltdata.lvalue != DLT_INVALID_LOG_LEVEL)  /*&& dltdata.avalue && dltdata.cvalue)*/
        {
            if ((dltdata.avalue == 0) && (dltdata.cvalue == 0)) {
                ...
                if (0 != dlt_client_send_all_log_level_v2(&g_dltclient,
                ...
                /* send control message*/
                if (0 != dlt_client_send_log_level_v2(&g_dltclient,
                                                   dltdata.avalue,
                                                   dltdata.cvalue,
                                                   (uint8_t) dltdata.lvalue)) {
                    fprintf(stderr, "ERROR: Could not send log level\n");
        ...
        else if (dltdata.rvalue != DLT_INVALID_TRACE_STATUS)
        {
            if ((dltdata.avalue == 0) && (dltdata.cvalue == 0)) {
                ...
                if (0 != dlt_client_send_all_trace_status_v2(&g_dltclient,
                ...
                /* send control message*/
                if (0 != dlt_client_send_trace_status_v2(&g_dltclient,
                                                      dltdata.avalue,
                                                      dltdata.cvalue,
                                                      (uint8_t) dltdata.rvalue)) {
                    fprintf(stderr, "ERROR: Could not send trace status\n");
    ...
    if ((id > DLT_SERVICE_ID) && (id < DLT_SERVICE_ID_LAST_ENTRY) &&
        ...
        switch (id) {
            case DLT_SERVICE_ID_GET_LOG_INFO:
            {
                ...
                ret = dlt_message_payload_v2(message, resp_text,
                ...
                if ((ret == 0) &&
                    (resp->service_id == DLT_SERVICE_ID_GET_LOG_INFO)) {
                    ret = dlt_client_parse_get_log_info_resp_text_v2(resp,
                                                                  resp_text);

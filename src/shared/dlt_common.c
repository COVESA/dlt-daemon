...
void dlt_set_id_v2(char **id, const char *text, int8_t len)
{
    ...
    *id = (char *)malloc(((size_t)len * sizeof(char)) + 1);
    if (*id == NULL) {
        return;
    }
    memcpy(*id, text, (size_t)len);
    memset((*id + (size_t)len), '\0', 1);
}
...
DltReturnValue dlt_filter_load_v2(DltFilter *filter, const char *filename, int verbose)
{
    ...
    while (!feof(handle)) {
        ...
        if (strcmp(str1, "----") == 0) {
            ...
            dlt_set_id_v2(&apid, str1, (int8_t)apidlen);
        }
        ...
        if (strcmp(str1, "----") == 0) {
            ...
            dlt_set_id_v2(&ctid, str1, (int8_t)ctidlen);
        }
                              ...
                              const int32_t payload_min, const int32_t payload_max, int verbose)
{
    ...
    if (dlt_filter_find(filter, apid, ctid, log_level, payload_min, payload_max, verbose) < 0) {
        ...
        dlt_set_id_v2(&(filter->apid2[filter->counter]), apid, (int8_t)filter->apid2len[filter->counter]);
        filter->ctid2len[filter->counter] = (uint8_t)strlen(ctid);
        dlt_set_id_v2(&(filter->ctid2[filter->counter]), (ctid ? ctid : NULL), (int8_t)filter->ctid2len[filter->counter]);
        filter->log_level[filter->counter] = log_level;
        filter->payload_min[filter->counter] = payload_min;
        filter->payload_max[filter->counter] = payload_max;
    ...
    if (filter->counter > 0) {
        ...
        if (found) {
            ...
            for (k = j; k < (filter->counter - 1); k++) {
                ...
                dlt_set_id_v2(&(filter->apid2[k]), filter->apid2[k + 1], (int8_t)filter->apid2len[k + 1]);
                filter->ctid2len[k] = filter->ctid2len[k + 1];
                dlt_set_id_v2(&(filter->ctid2[k]), filter->ctid2[k + 1], (int8_t)filter->ctid2len[k + 1]);
                filter->log_level[k] = filter->log_level[k + 1];
                filter->payload_min[k] = filter->payload_min[k + 1];
                filter->payload_max[k] = filter->payload_max[k + 1];
            }
...
static DltReturnValue dlt_message_get_extendedparameters_from_recievedbuffer_v2(DltMessageV2 *msg, uint8_t* buffer, DltHtyp2ContentType msgcontent)
{
    ...
    if (DLT_IS_HTYP2_WEID(msg->baseheaderv2->htyp2)) {
        ...
        dlt_set_id_v2((char **)&ecid_buf, (const char*)(buffer + pntroffset + 1), (int8_t)msg->extendedheaderv2.ecidlen);
    ...
    if (DLT_IS_HTYP2_WACID(msg->baseheaderv2->htyp2)) {
        ...
        dlt_set_id_v2((char **)&apid_buf, (const char*)(buffer + pntroffset + 1), (int8_t)msg->extendedheaderv2.apidlen);
        ...
        dlt_set_id_v2((char **)&ctid_buf, (const char*)(buffer + pntroffset + 1), (int8_t)msg->extendedheaderv2.ctidlen);
    ...
    if (DLT_IS_HTYP2_WSFLN(msg->baseheaderv2->htyp2)) {
        memcpy(&(msg->extendedheaderv2.finalen),
               buffer + pntroffset,
               1);
        char fina_buf[DLT_ID_SIZE];
        dlt_set_id_v2((char **)&fina_buf, (const char*)(buffer + pntroffset + 1), (int8_t)msg->extendedheaderv2.finalen);

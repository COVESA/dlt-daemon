...
int main(int argc, char *argv[])
{
    ...
    for (num = 0; num < maxnum; num++) {
        ...
        if (gflag) {
            ...
            if (rvalue == -1) {
                /* Verbose mode */
                DLT_LOG_V2(mycontext1, lvalue,
                           DLT_WITH_FILENAME_LINENUMBER(__FILENAME__, __LINE__),
                           DLT_WITH_TAGS("TAG1", "TAG2", "TAG3"),
                           DLT_WITH_PRIVACYLEVEL(32), DLT_INT(num),
                           DLT_STRING(text));
            }
            else {
                DLT_LOG_V2(mycontext1, lvalue,
                           DLT_WITH_FILENAME_LINENUMBER(__FILENAME__, __LINE__),
                           DLT_WITH_TAGS("TAG1", "TAG2", "TAG3"),
                           DLT_WITH_PRIVACYLEVEL(32),
                           DLT_RAW(text, (uint16_t)rvalue));
            }
        }

...
int dlt_receiver_receive_socket_udp(struct clientinfostruct *clientinfo,
                                    DltReceiver *receiver)
{
    if ((receiver == NULL) || (clientinfo == NULL)) {
        printf(
            "NULL receiver or clientinfo in dlt_receiver_receive_socket_udp\n");
        return -1;
    }
    ...
    if ((receiver->bytesRcvd = (int32_t)recvfrom(
             clientinfo->fd, receiver->buf + receiver->lastBytesRcvd,
             (size_t)(receiver->buffersize - receiver->lastBytesRcvd), 0,
             (struct sockaddr *)&(clientinfo->addr), &addrlen)) <= 0) {
        printf("Error\n");
        perror("recvfrom");
...
int dlt_receive_message_callback_udp(DltMessage *message)
{
    ...
    dlt_message_payload(message, text, DLT_RECEIVE_TEXTBUFSIZE,
                        DLT_OUTPUT_ASCII, 0);
...
int main()
{
    ...
    if (setsockopt(clientinfo.fd, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(yes)) <
        0) {
    ...
    clientinfo.addr.sin_addr.s_addr =
        htonl(INADDR_ANY); /* N.B.: differs from sender */
    clientinfo.addr.sin_port = htons(HELLO_PORT);

    /* bind to receive address */
    if (bind(clientinfo.fd, (struct sockaddr *)&clientinfo.addr,
             sizeof(clientinfo.addr)) < 0) {
        perror("bind");
        exit(1);
    }
    ...
    if (setsockopt(clientinfo.fd, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreq,
                   sizeof(mreq)) < 0) {
        perror("setsockopt");
        exit(1);
    }
    ...
    while (1) {
        ...
        while (dlt_message_read(&msg,
                                (unsigned char *)(clientinfo.receiver.buf),
                                (unsigned int)clientinfo.receiver.bytesRcvd, 0,
                                0) == DLT_MESSAGE_ERROR_OK) {
        ...
        if (dlt_receiver_move_to_begin(&(clientinfo.receiver)) ==
            DLT_RETURN_ERROR) {
            /* Return value ignored */
            dlt_message_free(&msg, 0);
            return DLT_RETURN_ERROR;

...
int dlt_testclient_message_callback(DltMessageV2 *message, void *data)
{
    ...
    if ((dltdata->fvalue == 0) ||
        ...
        dltdata->bytes_received +=
            ((unsigned long)message->datasize +
             (unsigned long)message->headersizev2 -
             (unsigned long)message->storageheadersizev2);
        ...
        if (DLT_IS_HTYP2_EH(message->baseheaderv2->htyp2)) {
            ...
            if ((DLT_GET_MSIN_MSTP(message->headerextrav2.msin)) ==
                ...
                if (DLT_IS_MSIN_VERB(message->headerextrav2.msin)) {
                    ...
                    /* first read the type info of the first argument: must be
                     * string */
                    ...
                    if (type_info & DLT_TYPE_INFO_SINT) {
                        ...
                        if (value < dltdata->last_value) {
                            if (dltdata->nvalue ==
                                ...
                                printf("PASSED: %d Msg received, %d not "
                                       "received\n",
                                       dltdata->count_received_messages,
                                       dltdata->count_not_received_messages);
                            else
                                printf("FAILED: %d Msg received, %d not "
                                       "received\n",
                                       dltdata->count_received_messages,
                                       dltdata->count_not_received_messages);
                        ...
                        if (length >= 0) {
                            ...
                            if (type_info & DLT_TYPE_INFO_RAWD) {
                                /* get length of raw data block */
                                {
                                    ...
                                    DLT_MSG_READ_VALUE(len_tmp_u, ptr,
                                                       datalength, uint16_t);

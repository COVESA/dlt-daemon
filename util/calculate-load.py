...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Process DLT messages to calculate the highest average payload size."
    )
    ...
    offline_group.add_argument(
        "-f", "--file", type=str, help="Path to the DLT file when in offline mode"
    )
    ...
    if args.mode == "offline":
        if not args.file:
            parser.error(
                "Offline mode requires a file path using the -f or --file option."
            )
...
def main():
    ...
    if not args.app_id:
        line_regex = re.compile(
            rf"{args.ecu}-{{0,3}}\s+(\w{{1,4}}-{{0,3}})\s+(\w{{1,4}}-{{0,3}})\s+log"
        )
    else:
        if args.context_id:
            line_regex = re.compile(
                rf"{args.ecu}-{{0,3}}\s+({args.app_id}-{{0,3}})\s+({args.context_id}-{{0,3}})\s+log"
            )
        else:
            line_regex = re.compile(
                rf"{args.ecu}-{{0,3}}\s+({args.app_id}-{{0,3}})\s+(\w{{1,4}}-{{0,3}})\s+log"
            )
    ...
    try:
        ...
        while keep_running(end_time):
            ...
            if line_context_id not in load[line_app_id]:
                ...
                load[line_app_id][line_context_id]["window_start_time"] = datetime.now(
                    timezone.utc
                )
            ...
            if (
                current_time - load[line_app_id][line_context_id]["window_start_time"]
            ).total_seconds() >= 60 or not keep_running(end_time):
                average_payload_size = calculate_payload_size_in_window(
                    load[line_app_id][line_context_id]["payload_sizes"]
                )
                load[line_app_id][line_context_id]["highest_average"] = max(
                    load[line_app_id][line_context_id]["highest_average"],
                ...
                load[line_app_id][line_context_id][
                    "window_start_time"
                ] = current_time  # Reset window start time
        ...
        for app_id, context_ids in load.items():
            ...
            for context_id, data in context_ids.items():
                ...
                highest_average = (
                    max(data["highest_average"], average_payload_size) / 60
                )
                ...
                if not args.sum_contexts:
                    ...
                    results[app_id]["contexts"][context_id]["soft"] = round_up_to(
                        highest_average * args.soft_limit_factor, args.factor_multiplier
                    )
                    results[app_id]["contexts"][context_id]["hard"] = round_up_to(
                        highest_average * args.hard_limit_factor, args.factor_multiplier
                    )
                    results[app_id]["contexts"] = dict(
                        sorted(results[app_id]["contexts"].items())
                    )

            results[app_id]
            if not args.context_id:
                results[app_id]["measured"] = app_id_total
                results[app_id]["soft"] = round_up_to(
                    app_id_total * args.soft_limit_factor, args.factor_multiplier
                )
                results[app_id]["hard"] = round_up_to(
                    app_id_total * args.hard_limit_factor, args.factor_multiplier
                )

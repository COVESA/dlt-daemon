...
TEST(t_dlt_logstorage_list_add_config, normal)
{
    ...
    data = (DltLogStorageFilterConfig *)calloc(
        1, sizeof(DltLogStorageFilterConfig));
    listdata = (DltLogStorageFilterConfig *)calloc(
        1, sizeof(DltLogStorageFilterConfig));
...
TEST(t_dlt_logstorage_create_keys, normal)
{
    ...
    EXPECT_EQ(DLT_RETURN_OK,
              dlt_logstorage_create_keys(data.apids, data.ctids, ecuid, &keys,
                                         &num_keys));
}
...
TEST(t_dlt_logstorage_prepare_table, normal)
{
    ...
    EXPECT_EQ(DLT_RETURN_OK, dlt_logstorage_list_destroy(
                                 &handle.config_list, &file_config, path, 0));
}
...
TEST(t_dlt_logstorage_read_list_of_names, normal)
{
    ...
    if (namesPtr != NULL) {
        EXPECT_EQ(DLT_RETURN_OK,
                  dlt_logstorage_read_list_of_names(&namesPtr, value));
...
TEST(t_dlt_logstorage_store_config_excluded_apids, normal)
{
    ...
    if (config.excluded_apids != NULL) {
        EXPECT_EQ(DLT_RETURN_OK,
                  dlt_logstorage_store_config_excluded_apids(&config, value));
...
TEST(t_dlt_logstorage_store_config_excluded_apids, null)
{
    EXPECT_EQ(DLT_RETURN_ERROR,
              dlt_logstorage_store_config_excluded_apids(NULL, NULL));
}
...
TEST(t_dlt_logstorage_store_config_excluded_ctids, normal)
{
    ...
    if (config.excluded_ctids != NULL) {
        EXPECT_EQ(DLT_RETURN_OK,
                  dlt_logstorage_store_config_excluded_ctids(&config, value));
...
{
    EXPECT_EQ(DLT_RETURN_ERROR,
              dlt_logstorage_store_config_excluded_ctids(NULL, NULL));
}
...
TEST(t_dlt_logstorage_check_filename, abnormal)
{
    ...
    if (config.file_name != NULL) {
        EXPECT_EQ(DLT_RETURN_ERROR,
                  dlt_logstorage_check_filename(&config, value));
...
TEST(t_dlt_logstorage_check_sync_strategy, normal)
{
    ...
    EXPECT_EQ(DLT_RETURN_OK,
              dlt_logstorage_check_sync_strategy(&config, value));
...
TEST(t_dlt_logstorage_check_param, normal)
{
    ...
    EXPECT_EQ(DLT_RETURN_OK,
              dlt_logstorage_check_param(
                  &config, DLT_LOGSTORAGE_FILTER_CONF_FILESIZE, value));
    EXPECT_EQ(100, config.file_size);
}
...
TEST(t_dlt_logstorage_check_param, null)
{
    EXPECT_EQ(DLT_RETURN_ERROR,
              dlt_logstorage_check_param(
                  NULL, DLT_LOGSTORAGE_FILTER_CONF_FILESIZE, NULL));
}
...
TEST(t_dlt_logstorage_store_filters, normal)
{
    ...
    EXPECT_EQ(DLT_RETURN_OK,
              dlt_logstorage_store_filters(&handle, config_file_name));
    EXPECT_EQ(DLT_RETURN_OK, dlt_logstorage_list_destroy(
                                 &handle.config_list, &file_config, path, 0));
}
...
TEST(t_dlt_logstorage_device_disconnected, normal)
{
    ...
    EXPECT_EQ(DLT_RETURN_OK,
              dlt_logstorage_device_disconnected(&handle, reason));
}
...
TEST(t_dlt_logstorage_get_loglevel_by_key, normal)
{
    ...
    config = (DltLogStorageFilterConfig *)calloc(
        1, sizeof(DltLogStorageFilterConfig));
    ...
    if (config != NULL) {
        ...
        EXPECT_EQ(DLT_RETURN_OK,
                  dlt_logstorage_list_add(key, num_keys, config,
                                          &(handle.config_list)));
        EXPECT_GE(DLT_LOG_ERROR,
                  dlt_logstorage_get_loglevel_by_key(&handle, key));
...
TEST(t_dlt_logstorage_get_config, normal)
{
    ...
    EXPECT_EQ(DLT_RETURN_OK, dlt_logstorage_list_add(key0, num_keys, &value,
                                                     &(handle.config_list)));
    EXPECT_EQ(DLT_RETURN_OK, dlt_logstorage_list_add(key1, num_keys, &value,
                                                     &(handle.config_list)));
    EXPECT_EQ(DLT_RETURN_OK, dlt_logstorage_list_add(key2, num_keys, &value,
                                                     &(handle.config_list)));
...
TEST(t_dlt_logstorage_filter, normal)
{
    ...
    DltLogStorageFilterConfig *neg_filter_config[DLT_CONFIG_FILE_SECTIONS] = {
        0};
    handle.config_list = NULL;
    handle.newest_file_list = NULL;
    ...
    num =
        dlt_logstorage_filter(&handle, neg_filter_config, apid, ctid, ecuid, 0);
    ...
    DltLogStorageFilterConfig *t_neg_filter_config[DLT_CONFIG_FILE_SECTIONS] = {
        0};
    handle.config_list = NULL;
    handle.newest_file_list = NULL;
    ...
    num = dlt_logstorage_filter(&handle, t_neg_filter_config, apid, ctid, ecuid,
                                0);
    ...
    DltLogStorageFilterConfig
        *neg_filter_ctid_only_config[DLT_CONFIG_FILE_SECTIONS] = {0};
    handle.config_list = NULL;
    handle.newest_file_list = NULL;
    ...
    num = dlt_logstorage_filter(&handle, neg_filter_ctid_only_config, apid,
                                ctid, ecuid, 0);
    ...
    value.excluded_ctids = t_ctid;
    DltLogStorageFilterConfig
        *t_neg_filter_ctid_only_config[DLT_CONFIG_FILE_SECTIONS] = {0};
    handle.config_list = NULL;
    handle.newest_file_list = NULL;
    ...
    num = dlt_logstorage_filter(&handle, t_neg_filter_ctid_only_config, apid,
                                ctid, ecuid, 0);
    ...
    DltLogStorageFilterConfig
        *neg_filter_apid_only_config[DLT_CONFIG_FILE_SECTIONS] = {0};
    handle.config_list = NULL;
    handle.newest_file_list = NULL;
    ...
    num = dlt_logstorage_filter(&handle, neg_filter_apid_only_config, apid,
                                ctid, ecuid, 0);
    ...
    value.excluded_ctids = NULL;
    DltLogStorageFilterConfig
        *t_neg_filter_apid_only_config[DLT_CONFIG_FILE_SECTIONS] = {0};
    handle.config_list = NULL;
    handle.newest_file_list = NULL;
    ...
    num = dlt_logstorage_filter(&handle, t_neg_filter_apid_only_config, apid,
                                ctid, ecuid, 0);
...
TEST(t_dlt_logstorage_write, normal)
{
    ...
    msg.extendedheader->msin =
        (DLT_TYPE_LOG << DLT_MSIN_MSTP_SHIFT) |
        (uint8_t)(((log_level << DLT_MSIN_MTIN_SHIFT) & DLT_MSIN_MTIN) |
                  DLT_MSIN_VERB);
    ...
    EXPECT_EQ(
        DLT_RETURN_OK,
        dlt_logstorage_write(&handle, &uconfig, (unsigned char *)&(userheader),
...
TEST(t_dlt_logstorage_write_v2, normal)
{
    ...
    msg.baseheaderv2 =
        (DltBaseHeaderV2 *)(msg.headerbufferv2 + sizeof(DltStorageHeaderV2));
    ...
    EXPECT_EQ(DLT_RETURN_OK,
              dlt_logstorage_write(
                  &handle, &uconfig, (unsigned char *)&(userheader),
                  sizeof(DltUserHeader),
...
{
    EXPECT_EQ(0,
              dlt_logstorage_write(NULL, NULL, NULL, 1, NULL, 1, NULL, 1, 0));
}
...
TEST(t_dlt_logstorage_sync_caches, normal)
{
    ...
    EXPECT_EQ(DLT_RETURN_OK, dlt_logstorage_list_add(key, num_keys, &configs,
                                                     &(handle.config_list)));
...
TEST(t_dlt_logstorage_log_file_name, normal)
{
    ...
    dlt_logstorage_log_file_name(log_file_name, &file_config, &filter_config,
                                 "log", 10, 0);
...
TEST(t_dlt_logstorage_get_idx_of_log_file, normal)
{
    ...
    EXPECT_EQ(2, dlt_logstorage_get_idx_of_log_file(&file_config,
                                                    &filter_config, file));

    char *gz_file = const_cast<char *>("Test_142_20160509_191132.dlt.gz");
    filter_config.gzip_compression = 1;

    EXPECT_EQ(142, dlt_logstorage_get_idx_of_log_file(&file_config,
                                                      &filter_config, gz_file));
}
TEST(t_dlt_logstorage_get_idx_of_log_file, null)
{
    EXPECT_EQ(DLT_RETURN_ERROR,
              dlt_logstorage_get_idx_of_log_file(NULL, NULL, NULL));
}
...
TEST(t_dlt_logstorage_storage_dir_info, normal)
{
    ...
    EXPECT_EQ(DLT_RETURN_OK,
              dlt_logstorage_storage_dir_info(&file_config, path, &config));
}
TEST(t_dlt_logstorage_storage_dir_info, null)
{
    EXPECT_EQ(DLT_RETURN_ERROR,
              dlt_logstorage_storage_dir_info(NULL, NULL, NULL));
}
...
TEST(t_dlt_logstorage_open_log_file, normal)
{
    ...
    EXPECT_EQ(DLT_RETURN_OK, dlt_logstorage_open_log_file(
                                 &config, &file_config, path, 1, true, false));
    EXPECT_STREQ("Test_01.dlt", config.working_file_name);
    EXPECT_EQ(DLT_RETURN_OK, dlt_logstorage_open_log_file(&config, &file_config,
                                                          path, 1, true, true));
    EXPECT_STREQ("Test_01.dlt", config.working_file_name);
...
{
    EXPECT_EQ(DLT_RETURN_ERROR,
              dlt_logstorage_open_log_file(NULL, NULL, NULL, 0, true, false));
}
...
TEST(t_dlt_logstorage_prepare_on_msg, normal1)
{
    ...
    newest_file_name.newest_file =
        const_cast<char *>("Test_003_20200728_191132.dlt");
    newest_file_name.wrap_id = 0;
    newest_file_name.next = NULL;

    EXPECT_EQ(DLT_RETURN_OK,
              dlt_logstorage_prepare_on_msg(&config, &file_config, path, 1,
                                            &newest_file_name));
}
...
TEST(t_dlt_logstorage_prepare_on_msg, normal3)
{
    ...
    char *working_file_name =
        const_cast<char *>("Test_002_20160509_191132.dlt");
    config.apids = &apids;
    config.ctids = &ctids;
    config.file_name = const_cast<char *>("Test");
    config.records = NULL;
...
{
    EXPECT_EQ(DLT_RETURN_ERROR,
              dlt_logstorage_prepare_on_msg(NULL, NULL, NULL, 0, NULL));
}
...
TEST(t_dlt_logstorage_write_on_msg, normal)
{
    ...
    EXPECT_EQ(DLT_RETURN_OK,
              dlt_logstorage_prepare_on_msg(&config, &file_config, path, 1,
                                            &newest_file_name));
    EXPECT_EQ(DLT_RETURN_OK,
              dlt_logstorage_write_on_msg(&config, &file_config, path, data1,
                                          size, data2, size, data3, size));
    sprintf(tmp_file, "%s/%s", path, config.working_file_name);
    remove(tmp_file);
}
...
TEST(t_dlt_logstorage_write_on_msg, gzip)
{
    ...
    newest_file_name.file_name = const_cast<char *>("Test");
    newest_file_name.newest_file =
        const_cast<char *>("Test_003_20200728_191132.dlt.gz");
    newest_file_name.wrap_id = 0;
    newest_file_name.next = NULL;
...
{
    EXPECT_EQ(DLT_RETURN_ERROR,
              dlt_logstorage_write_on_msg(NULL, NULL, NULL, NULL, 0, NULL, 0,
                                          NULL, 0));
}
...
TEST(t_dlt_logstorage_sync_on_msg, normal)
{
    ...
    EXPECT_EQ(DLT_RETURN_OK,
              dlt_logstorage_sync_on_msg(&config, &file_config, path,
...
{
    EXPECT_EQ(DLT_RETURN_ERROR,
              dlt_logstorage_sync_on_msg(NULL, NULL, NULL, 0));
}
...
TEST(t_dlt_logstorage_prepare_msg_cache, normal)
{
    ...
    newest_info.newest_file =
        const_cast<char *>("Test_003_20200728_191132.dlt");
    newest_info.wrap_id = 0;
    newest_info.next = NULL;

    EXPECT_EQ(DLT_RETURN_OK, dlt_logstorage_prepare_msg_cache(
                                 &config, &file_config, path, 1, &newest_info));
...
{
    EXPECT_EQ(DLT_RETURN_ERROR,
              dlt_logstorage_prepare_msg_cache(NULL, NULL, NULL, 0, NULL));
}
...
TEST(t_dlt_logstorage_write_msg_cache, normal)
{
    ...
    if (config.cache != NULL) {
        config.file_size = 50;
        EXPECT_EQ(DLT_RETURN_OK, dlt_logstorage_write_msg_cache(
...
{
    EXPECT_EQ(DLT_RETURN_ERROR,
              dlt_logstorage_write_msg_cache(NULL, NULL, NULL, NULL, 0, NULL, 0,
                                             NULL, 0));
}
...
TEST(t_dlt_logstorage_split_key, null)
{
    ...
    EXPECT_EQ(DLT_RETURN_WRONG_PARAMETER,
              dlt_logstorage_split_key(NULL, NULL, NULL, NULL));
    EXPECT_EQ(DLT_RETURN_WRONG_PARAMETER,
              dlt_logstorage_split_key(NULL, apid, ctid, ecuid));
    EXPECT_EQ(DLT_RETURN_WRONG_PARAMETER,
              dlt_logstorage_split_key(key, NULL, ctid, ecuid));
    EXPECT_EQ(DLT_RETURN_WRONG_PARAMETER,
              dlt_logstorage_split_key(key, apid, NULL, ecuid));
    EXPECT_EQ(DLT_RETURN_WRONG_PARAMETER,
              dlt_logstorage_split_key(key, apid, ctid, NULL));
}
...
TEST(t_dlt_logstorage_update_all_contexts, normal)
{
    ...
    EXPECT_EQ(0, dlt_daemon_init(&daemon, daemon_local.RingbufferMinSize,
    ...
    EXPECT_EQ(DLT_RETURN_OK, dlt_logstorage_update_all_contexts(
                                 &daemon, &daemon_local, apid, 1, 1, ecu, 0));
    EXPECT_EQ(DLT_RETURN_OK, dlt_logstorage_update_all_contexts(
                                 &daemon, &daemon_local, apid, 0, 1, ecu, 0));
}

TEST(t_dlt_logstorage_update_all_contexts, null)
{
    EXPECT_EQ(DLT_RETURN_WRONG_PARAMETER, dlt_logstorage_update_all_contexts(
...
TEST(t_dlt_logstorage_update_context, normal)
{
    ...
    daecontext = dlt_daemon_context_add(
        &daemon, apid, ctid, DLT_LOG_DEFAULT, DLT_TRACE_STATUS_DEFAULT, 0,
        app->user_handle, desc, daemon.ecuid, 0);
    EXPECT_NE((DltDaemonContext *)(NULL), daecontext);
    EXPECT_EQ(DLT_RETURN_OK,
              dlt_logstorage_update_context(&daemon, &daemon_local, apid, ctid,
                                            ecu, 1, 0));
    EXPECT_EQ(DLT_RETURN_OK,
              dlt_logstorage_update_context(&daemon, &daemon_local, apid, ctid,
                                            ecu, 0, 0));
...
TEST(t_dlt_logstorage_update_context, null)
{
    EXPECT_EQ(
        ...
        dlt_logstorage_update_context(NULL, NULL, NULL, NULL, NULL, 0, 0));
}
...
{
    EXPECT_EQ(DLT_RETURN_WRONG_PARAMETER,
              dlt_logstorage_update_context_loglevel(NULL, NULL, NULL, 0, 0));
}

/* Begin Method:
 * dlt_logstorage::t_dlt_daemon_logstorage_reset_application_loglevel*/
TEST(t_dlt_daemon_logstorage_reset_application_loglevel, normal)
{
    ...
    EXPECT_EQ(0, dlt_daemon_init_user_information(
                     &daemon, &daemon_local.pGateway, 0, 0));
    EXPECT_NO_THROW(dlt_daemon_logstorage_reset_application_loglevel(
        &daemon, &daemon_local, device_index, 1, 0));
}

TEST(t_dlt_daemon_logstorage_reset_application_loglevel, null)
{
    EXPECT_NO_THROW(
        dlt_daemon_logstorage_reset_application_loglevel(NULL, NULL, 0, 0, 0));
}
...
TEST(t_dlt_daemon_logstorage_get_loglevel, normal)
{
    ...
    EXPECT_EQ(DLT_RETURN_OK,
              dlt_logstorage_list_add(key, num_keys, &value,
                                      &(daemon.storage_handle->config_list)));
    EXPECT_NO_THROW(dlt_daemon_logstorage_update_application_loglevel(
        &daemon, &daemon_local, device_index, 0));
...
{
    EXPECT_EQ(DLT_RETURN_WRONG_PARAMETER,
              dlt_daemon_logstorage_get_loglevel(NULL, 0, NULL, NULL));
}

/* Begin Method:
 * dlt_logstorage::t_dlt_daemon_logstorage_update_application_loglevel*/
...
{
    EXPECT_NO_THROW(
        dlt_daemon_logstorage_update_application_loglevel(NULL, NULL, 0, 0));
}
...
TEST(t_dlt_daemon_logstorage_write, normal)
{
    ...
    msg.extendedheader->msin =
        (uint8_t)((DLT_TYPE_LOG << DLT_MSIN_MSTP_SHIFT) |
                  ((log_level << DLT_MSIN_MTIN_SHIFT) & DLT_MSIN_MTIN) |
                  DLT_MSIN_VERB);
    msg.extendedheader->noar = 1;
    dlt_set_id(msg.extendedheader->apid, apid);
    dlt_set_id(msg.extendedheader->ctid, ctid);
    ...
    EXPECT_EQ(DLT_RETURN_OK,
              dlt_logstorage_list_add(key0, num_keys, &value,
                                      &(daemon.storage_handle->config_list)));
    EXPECT_EQ(DLT_RETURN_OK,
              dlt_logstorage_list_add(key1, num_keys, &value,
                                      &(daemon.storage_handle->config_list)));
    EXPECT_EQ(DLT_RETURN_OK,
              dlt_logstorage_list_add(key2, num_keys, &value,
                                      &(daemon.storage_handle->config_list)));
    EXPECT_EQ(
        DLT_RETURN_OK,
        dlt_daemon_logstorage_write(
            &daemon, &uconfig, (unsigned char *)&(userheader),
            sizeof(DltUserHeader), msg.headerbuffer + sizeof(DltStorageHeader),
            (int)(msg.headersize - sizeof(DltStorageHeader)), data, size));
    dlt_message_free(&msg, 0);
}
...
{
    EXPECT_EQ(
        -1, dlt_daemon_logstorage_write(NULL, NULL, NULL, 0, NULL, 0, NULL, 0));
}

/* Begin Method:
 * dlt_logstorage::t_dlt_daemon_logstorage_setup_internal_storage*/
...
TEST(t_dlt_daemon_logstorage_setup_internal_storage, null)
{
    EXPECT_EQ(
        DLT_RETURN_WRONG_PARAMETER,
        dlt_daemon_logstorage_setup_internal_storage(NULL, NULL, NULL, 0));
}

/* Begin Method:
 * dlt_logstorage::dlt_daemon_logstorage_set_logstorage_cache_size*/
...
{
    EXPECT_EQ(DLT_RETURN_WRONG_PARAMETER,
              dlt_daemon_logstorage_cleanup(NULL, NULL, 0));
}
...
TEST(t_dlt_daemon_logstorage_sync_cache, normal)
{
    ...
    EXPECT_EQ(DLT_RETURN_OK,
              dlt_logstorage_list_add(key, num_keys, &configs,
                                      &(daemon.storage_handle->config_list)));
    EXPECT_EQ(DLT_RETURN_OK, dlt_daemon_logstorage_sync_cache(
                                 &daemon, &daemon_local, path, 0));
}

TEST(t_dlt_daemon_logstorage_sync_cache, null)
{
    EXPECT_EQ(DLT_RETURN_WRONG_PARAMETER,
              dlt_daemon_logstorage_sync_cache(NULL, NULL, NULL, 0));
}
...
TEST(t_dlt_daemon_logstorage_get_device, normal)
{
    ...
    EXPECT_NE((DltLogStorage *)NULL, dlt_daemon_logstorage_get_device(
                                         &daemon, &daemon_local, path, 0));
}

TEST(t_dlt_daemon_logstorage_get_device, null)
{
    EXPECT_EQ((DltLogStorage *)NULL,
              dlt_daemon_logstorage_get_device(NULL, NULL, NULL, 0));
}
...
TEST(t_dlt_logstorage_find_dlt_header, normal)
{
    ...
    if (config.cache != NULL) {
        ...
        EXPECT_EQ(
            2, dlt_logstorage_find_dlt_header(config.cache, 0, sizeof(data)));
        free(config.cache);
    }
}
...
TEST(t_dlt_logstorage_find_dlt_header, null)
{
    ...
    if (config.cache != NULL) {
        ...
        memcpy(config.cache, data, sizeof(data));
        EXPECT_EQ(DLT_RETURN_ERROR, dlt_logstorage_find_dlt_header(
                                        config.cache, 0, sizeof(data)));
        free(config.cache);
    }
}
...
TEST(t_dlt_logstorage_find_last_dlt_header, normal)
{
    ...
    if (config.cache != NULL) {
        ...
        /* DLT header starts from index 2 */
        EXPECT_EQ(2, dlt_logstorage_find_last_dlt_header(config.cache, 0,
                                                         sizeof(data)));
        free(config.cache);
    }
}
...
TEST(t_dlt_logstorage_find_last_dlt_header, null)
{
    ...
    if (config.cache != NULL) {
        /* config.cache =(void *) "a,b,D,L,T,0x01"; */
        memcpy(config.cache, data, sizeof(data));
        EXPECT_EQ(-1, dlt_logstorage_find_last_dlt_header(config.cache, 0,
                                                          sizeof(data)));
        free(config.cache);
    }
}
...
TEST(t_dlt_logstorage_sync_to_file, normal)
{
    ...
    g_logstorage_cache_max = 16;
    unsigned int size = 10;
    unsigned char data1[10] = {'a',  'b', 'D', 'L', 'T',
                               0x01, 'c', 'd', 'e', 'f'};
    ...
    config.cache =
        calloc(1, config.file_size + sizeof(DltLogStorageCacheFooter));

    if (config.cache != NULL) {
        EXPECT_EQ(DLT_RETURN_OK,
                  dlt_logstorage_prepare_msg_cache(&config, &file_config, path,
                                                   1, &newest_info));
        EXPECT_EQ(DLT_RETURN_OK, dlt_logstorage_write_msg_cache(
                                     &config, &file_config, path, data1, size,
                                     data2, size, data3, size));

        footer = (DltLogStorageCacheFooter *)((uint8_t *)config.cache +
                                              config.file_size);

        EXPECT_EQ(DLT_RETURN_OK, dlt_logstorage_sync_to_file(
                                     &config, &file_config, path, footer,
...
TEST(t_dlt_logstorage_sync_msg_cache, normal)
{
    ...
    if (config.cache != NULL) {
        ...
        EXPECT_EQ(DLT_RETURN_OK, dlt_logstorage_write_msg_cache(
                                     ...
                                     data2, size, data3, size));
        EXPECT_EQ(DLT_RETURN_OK,
                  dlt_logstorage_sync_msg_cache(&config, &file_config, path,
                                                DLT_LOGSTORAGE_SYNC_ON_DEMAND));
        std::stringstream file_path;
        file_path << path << "/" << config.working_file_name;
        std::ifstream f(file_path.str());
        if (f.is_open()) {
...
{
    EXPECT_EQ(DLT_RETURN_ERROR,
              dlt_logstorage_sync_msg_cache(NULL, NULL, NULL, 0));
}
...
int main(int argc, char **argv)
{
    ...
    if (!cpid) {
        ...
        while (i) {
            ...
            memset(buffer, 0, 256);
            (void)(read(newsockfd[i - 1], buffer, 255) +
                   1); /* just ignore result */
            i--;
        }
         ...
         * "t_dlt_event_handler_register_connection*"; */

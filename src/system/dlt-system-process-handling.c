...
int daemonize()
{
    ...
    /*no close(fd); - we just intentionally pointed stdx to null! tbd: set
     * ignore for coverity */
    return 0;
}

/* Unregisters all DLT Contexts and closes all file descriptors */
void cleanup_processes(struct pollfd *pollfd, struct pollfd *journalPollFd,
                       sd_journal *j, DltSystemConfiguration *config)
{
    // Syslog cleanup
...
/* Creates timer for LogFile and LogProcess, that need to be called every
 * second. */
int register_timer_fd(struct pollfd *pollfd, int fdcnt)
{
    ...
    if (timerfd_settime(timerfd, 0, &timerValue, NULL) <
        0) { // init timer with 1 second
...
void start_dlt_system_processes(DltSystemConfiguration *config)
{
    ...
    uint8_t fdType[MAX_FD_NUMBER];       // Holds corresponding enum for process
                                         // identification
    ...
    if (config->Journal.Enable) {
        ...
        if (pthread_create(&journalThreadHandle, NULL, &journal_thread,
                           (void *)&params) != 0) {
            DLT_LOG(dltsystem, DLT_LOG_ERROR,
                    DLT_STRING("Failed to create journal_thread thread "),
    ...
    defined(DLT_SYSTEMD_JOURNAL_ENABLE)
                    watchdog_fd_handler(pollfd[i].fd,
                                        &config->Journal.MessageReceived);
#else
                    watchdog_fd_handler(pollfd[i].fd);
#endif
                }

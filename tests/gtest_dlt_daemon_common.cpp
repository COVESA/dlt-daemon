...
TEST(t_dlt_daemon_init_user_information, normal_multiple_lists)
{
    ...
    EXPECT_EQ(DLT_RETURN_OK,
              strncmp(daemon.ecuid, daemon.user_list[0].ecu, DLT_ID_SIZE));
    EXPECT_EQ(DLT_RETURN_OK, strncmp(gateway.connections[0].ecuid,
                                     daemon.user_list[1].ecu, DLT_ID_SIZE));
    EXPECT_EQ(DLT_RETURN_OK, strncmp(gateway.connections[1].ecuid,
                                     daemon.user_list[2].ecu, DLT_ID_SIZE));
...
TEST(t_dlt_daemon_find_users_list, normal_one_list)
{
    ...
    EXPECT_EQ(DLT_RETURN_OK,
              strncmp(user_list->ecu, daemon.ecuid, DLT_ID_SIZE));
...
TEST(t_dlt_daemon_find_users_list, normal_multiple_lists)
{
    ...
    EXPECT_EQ(
        DLT_RETURN_OK,
        strncmp(user_list->ecu, gateway.connections[0].ecuid, DLT_ID_SIZE));

    user_list = dlt_daemon_find_users_list(&daemon, &ecu3[0], 0);
    EXPECT_NE(user_list, nullptr);
    EXPECT_EQ(
        DLT_RETURN_OK,
        strncmp(user_list->ecu, gateway.connections[1].ecuid, DLT_ID_SIZE));
...
TEST(t_dlt_daemon_application_add, normal)
{
    ...
    app = dlt_daemon_application_add(&daemon, apid_buf, pid, desc_buf, fd, ecu,
                                     0);
    /*printf("### APP: APID=%s  DESCR=%s NUMCONTEXT=%i PID=%i USERHANDLE=%i\n",
     * app->apid,app->application_description, app->num_contexts, app->pid,
     * app->user_handle); */
...
TEST(t_dlt_daemon_application_add, abnormal)
{
     ...
     */
    /*    app = dlt_daemon_application_add(&daemon,(char *) apid, pid, (char *)
     * desc, 0); */
    ...
    /*    app = dlt_daemon_application_add(&daemon,(char *) apid, 0, (char *)
     * desc, 0); */
    /*    EXPECT_LE((DltDaemonApplication *) 0, app); */
    /*    app = dlt_daemon_application_add(&daemon,(char *) apid, 123, (char *)
     * desc, 0); */
    /*    EXPECT_EQ((DltDaemonApplication *) 0, app); */
    /*    dlt_daemon_application_del(&daemon,app, 0); */
    /*    dlt_daemon_applications_clear(&daemon, 0); */
    ...
    /*    app = dlt_daemon_application_add(&daemon,(char *) apid, pid, (char *)
     * desc, 0); */
    /*    EXPECT_EQ((DltDaemonApplication *)0,
     * dlt_daemon_application_add(&daemon,(char *) apid, pid, (char *) desc,
     * 12345678)); */
    /*    dlt_daemon_application_del(&daemon, app, 0); */
    /*    dlt_daemon_applications_clear(&daemon, 0); */
...
TEST(t_dlt_daemon_application_add, nullpointer)
{
    ...
    /* NULL-Pointer test */
    EXPECT_EQ((DltDaemonApplication *)0,
              dlt_daemon_application_add(NULL, NULL, 0, NULL, 0, NULL, 0));
    strncpy(desc_buf, desc, sizeof(desc_buf) - 1);
    desc_buf[sizeof(desc_buf) - 1] = '\0';
    EXPECT_EQ((DltDaemonApplication *)0,
              dlt_daemon_application_add(NULL, NULL, 0, desc_buf, 0, NULL, 0));
    memcpy(apid_buf, apid, DLT_ID_SIZE);
    apid_buf[DLT_ID_SIZE] = '\0';
    EXPECT_EQ((DltDaemonApplication *)0,
              dlt_daemon_application_add(NULL, apid_buf, 0, NULL, 0, NULL, 0));
    EXPECT_EQ(
        (DltDaemonApplication *)0,
        dlt_daemon_application_add(NULL, apid_buf, 0, desc_buf, 0, NULL, 0));
    EXPECT_EQ((DltDaemonApplication *)0,
              dlt_daemon_application_add(&daemon, NULL, 0, NULL, 0, NULL, 0));
    EXPECT_EQ(
        (DltDaemonApplication *)0,
        dlt_daemon_application_add(&daemon, NULL, 0, desc_buf, 0, NULL, 0));
    EXPECT_EQ((DltDaemonApplication *)0,
              dlt_daemon_application_add(NULL, NULL, 0, NULL, fd, NULL, 0));
    EXPECT_EQ((DltDaemonApplication *)0,
              dlt_daemon_application_add(NULL, NULL, 0, NULL, 0, ecu, 0));
}
...
TEST(t_dlt_daemon_application_find, normal)
{
    ...
    EXPECT_EQ((DltDaemonApplication *)0,
              dlt_daemon_application_find(&daemon, ecu, apid_buf, 0));
    ...
    nexi[DLT_ID_SIZE] = '\0';
    EXPECT_EQ((DltDaemonApplication *)0,
              dlt_daemon_application_find(&daemon, ecu, nexi, 0));
    EXPECT_LE(0, dlt_daemon_application_del(&daemon, app, ecu, 0));
    EXPECT_LE(0, dlt_daemon_applications_clear(&daemon, ecu, 0));
    EXPECT_EQ(0, dlt_daemon_free(&daemon, 0));
}
TEST(t_dlt_daemon_application_find, abnormal)
{
    ...
    /*    EXPECT_EQ((DltDaemonApplication *) 0,
     * dlt_daemon_application_find(&daemon, (char *) apid, 123456789)); */
    /*    dlt_daemon_application_del(&daemon, app, 0); */
    /*    dlt_daemon_applications_clear(&daemon, 0); */
    /*    EXPECT_EQ(0, dlt_daemon_free(&daemon, 0)); */
}
TEST(t_dlt_daemon_application_find, nullpointer)
{
    ...
    EXPECT_EQ((DltDaemonApplication *)0,
              dlt_daemon_application_find(NULL, NULL, NULL, 0));
    char apid_buf[DLT_ID_SIZE + 1] = {0};
    memcpy(apid_buf, apid, DLT_ID_SIZE);
    apid_buf[DLT_ID_SIZE] = '\0';
    EXPECT_EQ((DltDaemonApplication *)0,
              dlt_daemon_application_find(NULL, apid_buf, NULL, 0));
    EXPECT_EQ((DltDaemonApplication *)0,
              dlt_daemon_application_find(&daemon, NULL, NULL, 0));
}
...
TEST(t_dlt_daemon_applications_clear, normal)
{
    ...
    EXPECT_LE((DltDaemonApplication *)0,
              dlt_daemon_application_add(&daemon, apid1_buf, pid, desc1_buf, fd,
                                         ecu, 0));
    memcpy(apid1_buf, "TES1", DLT_ID_SIZE);
    apid1_buf[DLT_ID_SIZE] = '\0';
    strncpy(desc1_buf, "Test clear 1", sizeof(desc1_buf) - 1);
...
TEST(t_dlt_daemon_applications_clear, abnormal)
{
    ...
    /* Verbose != 0 or 1, expect error */
    /*    dlt_daemon_application_add(&daemon, (char *) "TEST", pid, (char *)
     * "Test clear", 0); */
...
TEST(t_dlt_daemon_applications_invalidate_fd, normal)
{
    ...
    app = dlt_daemon_application_add(&daemon, apid_buf, pid, desc_buf, fd, ecu,
                                     0);
    EXPECT_LE(0, dlt_daemon_applications_invalidate_fd(&daemon, ecu,
                                                       app->user_handle, 0));
    EXPECT_LE(0, dlt_daemon_application_del(&daemon, app, ecu, 0));
    EXPECT_LE(0, dlt_daemon_applications_clear(&daemon, ecu, 0));
    EXPECT_EQ(0, dlt_daemon_free(&daemon, 0));
...
TEST(t_dlt_daemon_applications_invalidate_fd, abnormal)
{
    ...
    /*    app = dlt_daemon_application_add(&daemon,(char *) apid, pid, (char *)
     * desc, 0); */
    /*    EXPECT_GE(-1, dlt_daemon_applications_invalidate_fd(&daemon,
     * app->user_handle, 123456789)); */
    /*    dlt_daemon_application_del(&daemon, app, 0); */
    /*    dlt_daemon_applications_clear(&daemon, 0); */
    /*    EXPECT_EQ(0, dlt_daemon_free(&daemon, 0)); */
}
...
TEST(t_dlt_daemon_applications_save, abnormal)
{
     ...
     * DLT_RUNTIME_DEFAULT_DIRECTORY,DLT_LOG_INFO, DLT_TRACE_STATUS_OFF,0,0));
     */
    /*    EXPECT_GE(-1, dlt_daemon_applications_save(&daemon, (char *) filename,
     * 0)); */
    ...
    /*    app = dlt_daemon_application_add(&daemon,(char *) apid, pid, (char *)
     * desc, 0); */
    /*    EXPECT_GE(-1, dlt_daemon_applications_save(&daemon, (char *)
     * "PATH_DONT_EXIST", 0)); */
    /*    dlt_daemon_application_del(&daemon, app, 0); */
    /*    dlt_daemon_applications_clear(&daemon, 0); */
    /*    EXPECT_EQ(0, dlt_daemon_free(&daemon, 0)); */
...
TEST(t_dlt_daemon_applications_load, abnormal)
{
     ...
     * DLT_RUNTIME_DEFAULT_DIRECTORY,DLT_LOG_INFO, DLT_TRACE_STATUS_OFF,0,0));
     */
    /*    EXPECT_GE(-1, dlt_daemon_applications_load(&daemon, (char *) filename,
     * 0)); */
    ...
    /*    app = dlt_daemon_application_add(&daemon,(char *) apid, pid, (char *)
     * desc, 0); */
    /*    EXPECT_GE(-1, dlt_daemon_applications_load(&daemon, (char *)
     * "PATH_DONT_EXIST", 0)); */
    /*    dlt_daemon_application_del(&daemon, app, 0); */
    /*    dlt_daemon_applications_clear(&daemon, 0); */
    /*    EXPECT_EQ(0, dlt_daemon_free(&daemon, 0)); */
...
TEST(t_dlt_daemon_context_add, normal)
{
    ...
    /*  DLT_LOG_ERROR   =           0x02,   / **< error with impact to correct
     * functionality * / */
    /*  DLT_LOG_WARN    =           0x03,   / **< warning, correct behaviour
...
TEST(t_dlt_daemon_context_add, abnormal)
{
    ...
    app = dlt_daemon_application_add(&daemon, apid, 0, desc, fd, ecu, 0);
    daecontext =
        dlt_daemon_context_add(&daemon, apid, ctid, invalid_log_level,
                               DLT_TRACE_STATUS_DEFAULT, 0, 0, desc, ecu, 0);
    /*printf("### CONTEXT: APID=%s\tCTID=%s\n",
     * daecontext->apid,daecontext->ctid); */
    ...
    daecontext =
        dlt_daemon_context_add(&daemon, apid, ctid, DLT_LOG_DEFAULT,
                               invalid_trace_status, 0, 0, desc, ecu, 0);
    /*printf("### CONTEXT: APID=%s\tCTID=%s\n",
     * daecontext->apid,daecontext->ctid); */
    EXPECT_EQ((DltDaemonContext *)0, daecontext);
    EXPECT_GE(-1, dlt_daemon_context_del(&daemon, daecontext, ecu, 0));
    ...
    /*    daecontext =
     * dlt_daemon_context_add(&daemon,apid_tl,ctid,DLT_LOG_DEFAULT,DLT_TRACE_STATUS_DEFAULT,0,0,desc,0);
     */
    /*    printf("### CONTEXT: APID=%s\tCTID=%s\n",
     * daecontext->apid,daecontext->ctid); */
    ...
    /*    daecontext =
     * dlt_daemon_context_add(&daemon,apid_ts,ctid,DLT_LOG_DEFAULT,DLT_TRACE_STATUS_DEFAULT,0,0,desc,0);
     */
    /*    //printf("### CONTEXT: APID=%s\tCTID=%s\n",
     * daecontext->apid,daecontext->ctid); */
    ...
    /*    daecontext =
     * dlt_daemon_context_add(&daemon,apid,ctid_tl,DLT_LOG_DEFAULT,DLT_TRACE_STATUS_DEFAULT,0,0,desc,0);
     */
    /*    //printf("### CONTEXT: APID=%s\tCTID=%s\n",
     * daecontext->apid,daecontext->ctid); */
    /*    EXPECT_STREQ(apid, daecontext->apid); */
    ...
    /*    app = dlt_daemon_application_add(&daemon, apid, 0, desc, 0); */
    /*    daecontext =
     * dlt_daemon_context_add(&daemon,apid,ctid_ts,DLT_LOG_DEFAULT,DLT_TRACE_STATUS_DEFAULT,0,0,desc,0);
     */
    /*    //printf("### CONTEXT: APID=%s\tCTID=%s\n",
     * daecontext->apid,daecontext->ctid); */
    /*    EXPECT_STREQ(apid, daecontext->apid); */
    ...
    /*    app = dlt_daemon_application_add(&daemon, apid, 0, desc, 0); */
    /*    daecontext =
     * dlt_daemon_context_add(&daemon,apid,ctid,DLT_LOG_DEFAULT,DLT_TRACE_STATUS_DEFAULT,0,0,desc,123456789);
     */
    /*    //printf("### CONTEXT: APID=%s\tCTID=%s\n",
     * daecontext->apid,daecontext->ctid); */
    /*    EXPECT_EQ((DltDaemonContext *) 0, daecontext); */
...
TEST(t_dlt_daemon_context_add, nullpointer)
{
    ...
    EXPECT_EQ(
        (DltDaemonContext *)0,
        dlt_daemon_context_add(NULL, NULL, NULL, 0, 0, 0, 0, NULL, NULL, 0));
    EXPECT_EQ(
        (DltDaemonContext *)0,
        dlt_daemon_context_add(NULL, NULL, NULL, 0, 0, 0, 0, desc, NULL, 0));
    EXPECT_EQ(
        (DltDaemonContext *)0,
        dlt_daemon_context_add(NULL, NULL, ctid, 0, 0, 0, 0, NULL, NULL, 0));
    EXPECT_EQ(
        (DltDaemonContext *)0,
        dlt_daemon_context_add(NULL, NULL, ctid, 0, 0, 0, 0, desc, NULL, 0));
    EXPECT_EQ(
        (DltDaemonContext *)0,
        dlt_daemon_context_add(NULL, apid, NULL, 0, 0, 0, 0, NULL, NULL, 0));
    EXPECT_EQ(
        (DltDaemonContext *)0,
        dlt_daemon_context_add(NULL, apid, NULL, 0, 0, 0, 0, desc, NULL, 0));
    EXPECT_EQ(
        (DltDaemonContext *)0,
        dlt_daemon_context_add(NULL, apid, ctid, 0, 0, 0, 0, NULL, NULL, 0));
    EXPECT_EQ(
        (DltDaemonContext *)0,
        dlt_daemon_context_add(NULL, apid, ctid, 0, 0, 0, 0, desc, NULL, 0));
    EXPECT_EQ(
        (DltDaemonContext *)0,
        dlt_daemon_context_add(&daemon, NULL, NULL, 0, 0, 0, 0, NULL, NULL, 0));
    EXPECT_EQ(
        (DltDaemonContext *)0,
        dlt_daemon_context_add(&daemon, NULL, NULL, 0, 0, 0, 0, desc, NULL, 0));
    EXPECT_EQ(
        (DltDaemonContext *)0,
        dlt_daemon_context_add(&daemon, NULL, ctid, 0, 0, 0, 0, NULL, NULL, 0));
    EXPECT_EQ(
        (DltDaemonContext *)0,
        dlt_daemon_context_add(&daemon, NULL, ctid, 0, 0, 0, 0, desc, NULL, 0));
    EXPECT_EQ(
        (DltDaemonContext *)0,
        dlt_daemon_context_add(&daemon, apid, NULL, 0, 0, 0, 0, NULL, NULL, 0));
    EXPECT_EQ(
        (DltDaemonContext *)0,
        dlt_daemon_context_add(&daemon, apid, NULL, 0, 0, 0, 0, desc, NULL, 0));
    EXPECT_EQ(
        (DltDaemonContext *)0,
        dlt_daemon_context_add(&daemon, apid, ctid, 0, 0, 0, 0, NULL, NULL, 0));
    EXPECT_EQ(0, dlt_daemon_free(&daemon, 0));
}
...
TEST(t_dlt_daemon_context_del, abnormal)
{
    ...
    /* No application used */
    /*    daecontext =
     * dlt_daemon_context_add(&daemon,apid,ctid,DLT_LOG_DEFAULT,DLT_TRACE_STATUS_DEFAULT,0,0,desc,0);
     */
    /*    EXPECT_GE(-1, dlt_daemon_context_del(&daemon, daecontext, 0)); */
    /*    EXPECT_GE(-1, dlt_daemon_application_del(&daemon, app, 0)); */
...
TEST(t_dlt_daemon_context_find, abnormal)
{
    ...
    EXPECT_EQ(DLT_RETURN_OK,
              strncmp(daemon.ecuid, daemon.user_list[0].ecu, DLT_ID_SIZE));
    EXPECT_EQ((DltDaemonContext *)0,
              dlt_daemon_context_find(&daemon, apid, ctid, ecu, 0));

    /* No apid */
    char no_apid[1] = "";
    app = dlt_daemon_application_add(&daemon, no_apid, 0, desc, fd, ecu, 0);
    daecontext =
        dlt_daemon_context_add(&daemon, no_apid, ctid, DLT_LOG_DEFAULT,
                               DLT_TRACE_STATUS_DEFAULT, 0, 0, desc, ecu, 0);
    EXPECT_EQ((DltDaemonContext *)0,
              dlt_daemon_context_find(&daemon, no_apid, ctid, ecu, 0));
    EXPECT_GE(-1, dlt_daemon_context_del(&daemon, daecontext, ecu, 0));
    ...
    app = dlt_daemon_application_add(&daemon, apid, 0, desc, fd, ecu, 0);
    daecontext =
        dlt_daemon_context_add(&daemon, apid, no_ctid, DLT_LOG_DEFAULT,
                               DLT_TRACE_STATUS_DEFAULT, 0, 0, desc, ecu, 0);
    EXPECT_EQ((DltDaemonContext *)0,
              dlt_daemon_context_find(&daemon, apid, no_ctid, ecu, 0));
    EXPECT_GE(-1, dlt_daemon_context_del(&daemon, daecontext, ecu, 0));
    EXPECT_LE(0, dlt_daemon_application_del(&daemon, app, ecu, 0));
    EXPECT_LE(0, dlt_daemon_contexts_clear(&daemon, ecu, 0));
    EXPECT_LE(0, dlt_daemon_applications_clear(&daemon, ecu, 0));
     ...
     * dlt_daemon_context_add(&daemon,apid,ctid,DLT_LOG_DEFAULT,DLT_TRACE_STATUS_DEFAULT,0,0,desc,0);
     */
    /*    EXPECT_EQ((DltDaemonContext *) 0 ,dlt_daemon_context_find(&daemon,
     * apid, ctid, 123456789)); */
    /*    dlt_daemon_context_del(&daemon, daecontext, 0); */
    /*    dlt_daemon_application_del(&daemon, app, 0); */
    /*    dlt_daemon_contexts_clear(&daemon, 0); */
...
TEST(t_dlt_daemon_context_find, nullpointer)
{
    ...
    EXPECT_EQ((DltDaemonContext *)0,
              dlt_daemon_context_find(NULL, NULL, NULL, NULL, 0));
    EXPECT_EQ((DltDaemonContext *)0,
              dlt_daemon_context_find(NULL, NULL, ctid, NULL, 0));
    EXPECT_EQ((DltDaemonContext *)0,
              dlt_daemon_context_find(NULL, apid, NULL, NULL, 0));
    EXPECT_EQ((DltDaemonContext *)0,
              dlt_daemon_context_find(NULL, apid, ctid, NULL, 0));
    EXPECT_EQ((DltDaemonContext *)0,
              dlt_daemon_context_find(&daemon, NULL, NULL, NULL, 0));
    EXPECT_EQ((DltDaemonContext *)0,
              dlt_daemon_context_find(&daemon, NULL, ctid, NULL, 0));
    EXPECT_EQ((DltDaemonContext *)0,
              dlt_daemon_context_find(&daemon, apid, NULL, NULL, 0));
    EXPECT_EQ((DltDaemonContext *)0,
              dlt_daemon_context_find(&daemon, NULL, NULL, ecu, 0));
}
...
TEST(t_dlt_daemon_contexts_invalidate_fd, normal)
{
    ...
    daecontext =
        dlt_daemon_context_add(&daemon, apid, ctid, DLT_LOG_DEFAULT,
                               DLT_TRACE_STATUS_DEFAULT, 0, 0, desc, ecu, 0);
    EXPECT_LE(0, dlt_daemon_contexts_invalidate_fd(&daemon, ecu,
                                                   app->user_handle, 0));
    EXPECT_LE(0, dlt_daemon_context_del(&daemon, daecontext, ecu, 0));
    EXPECT_LE(0, dlt_daemon_application_del(&daemon, app, ecu, 0));
    EXPECT_LE(0, dlt_daemon_contexts_clear(&daemon, ecu, 0));
...
TEST(t_dlt_daemon_contexts_invalidate_fd, abnormal)
{
     ...
     * DLT_RUNTIME_DEFAULT_DIRECTORY,DLT_LOG_INFO, DLT_TRACE_STATUS_OFF,0,0));
     */
    /*    EXPECT_GE(-1, dlt_daemon_contexts_invalidate_fd(&daemon,
     * app->user_handle, 0)); */
...
TEST(t_dlt_daemon_contexts_save, abnormal)
{
     ...
     * dlt_daemon_context_add(&daemon,apid,ctid,DLT_LOG_DEFAULT,DLT_TRACE_STATUS_DEFAULT,0,0,desc,0);
     */
    /*    EXPECT_GE(-1, dlt_daemon_contexts_save(&daemon, (char *)
     * "PATCH_NOT_EXISTS", 0)); */
    /*    dlt_daemon_context_del(&daemon, daecontext, 0); */
    /*    dlt_daemon_application_del(&daemon, app, 0); */
    /*    dlt_daemon_contexts_clear(&daemon, 0); */
...
TEST(t_dlt_daemon_contexts_load, abnormal)
{
     ...
     * dlt_daemon_context_add(&daemon,apid,ctid,DLT_LOG_DEFAULT,DLT_TRACE_STATUS_DEFAULT,0,0,desc,0);
     */
    /*    EXPECT_GE(-1, dlt_daemon_contexts_load(&daemon, (char *)
     * "PATCH_NOT_EXISTS", 0)); */
    /*    dlt_daemon_context_del(&daemon, daecontext, 0); */
    /*    dlt_daemon_application_del(&daemon, app, 0); */
    /*    dlt_daemon_contexts_clear(&daemon, 0); */
...
TEST(t_dlt_daemon_user_send_log_level, normal)
{
    ...
    daecontext =
        dlt_daemon_context_add(&daemon, apid, ctid, DLT_LOG_DEFAULT,
                               DLT_TRACE_STATUS_DEFAULT, 0, 1, desc, ecu, 0);
...
TEST(t_dlt_daemon_user_send_log_level, abnormal)
{
    ...
    /*    app = dlt_daemon_application_add(&daemon, apid, 0, desc, 0); */
    /*    daecontext =
     * dlt_daemon_context_add(&daemon,apid,ctid,DLT_LOG_DEFAULT,DLT_TRACE_STATUS_DEFAULT,0,-1,desc,0);
     */
    /*    EXPECT_GE(-1, dlt_daemon_user_send_log_level(&daemon, daecontext, 0));
     */
     ...
     * dlt_daemon_context_add(&daemon,apid,ctid,DLT_LOG_DEFAULT,DLT_TRACE_STATUS_DEFAULT,0,1,desc,0);
     */
    /*    EXPECT_GE(-1, dlt_daemon_user_send_log_level(&daemon, daecontext,
     * 123456789)); */
    /*    dlt_daemon_context_del(&daemon, daecontext, 0); */
    /*    dlt_daemon_application_del(&daemon, app, 0); */
    /*    dlt_daemon_contexts_clear(&daemon, 0); */
...
TEST(t_dlt_daemon_user_send_log_state, normal)
{
    ...
    snprintf(filename, DLT_DAEMON_COMMON_TEXTBUFSIZE, "%s/dlt%d", DLT_USER_DIR,
             pid);
    ...
    /*    dlt_daemon_init(&daemon, DLT_DAEMON_RINGBUFFER_MIN_SIZE,
     * DLT_DAEMON_RINGBUFFER_MAX_SIZE, DLT_DAEMON_RINGBUFFER_STEP_SIZE, "",0);
     */
...
TEST(t_dlt_daemon_user_send_log_state, abnormal)
{
     ...
     * dlt_daemon_context_add(&daemon,apid,ctid,DLT_LOG_DEFAULT,DLT_TRACE_STATUS_DEFAULT,0,0,desc,0);
     */
    /*    EXPECT_GE(-1, dlt_daemon_user_send_log_state(&daemon, app,
     * 123456789)); */
    /*    dlt_daemon_context_del(&daemon, daecontext, 0); */
    /*    dlt_daemon_application_del(&daemon, app, 0); */
    /*    dlt_daemon_contexts_clear(&daemon, 0); */
...
TEST(t_dlt_daemon_find_preconfigured_trace_load_settings, empty_trace_settings)
{
    ...
    daemon.preconfigured_trace_load_settings =
        (DltTraceLoadSettings *)malloc(sizeof(DltTraceLoadSettings));
...
TEST(t_dlt_daemon_find_preconfigured_trace_load_settings, app_id_not_found)
{
    ...
    strncpy(daemon.preconfigured_trace_load_settings[0].apid, "APP2",
            DLT_ID_SIZE);
...
static void setup_trace_load_settings(DltDaemon &daemon)
{
    ...
    strncpy(daemon.preconfigured_trace_load_settings[0].apid, "APP2",
            DLT_ID_SIZE);
    strncpy(daemon.preconfigured_trace_load_settings[0].ctid, "CTID",
            DLT_ID_SIZE);

    strncpy(daemon.preconfigured_trace_load_settings[1].apid, "APP1",
            DLT_ID_SIZE);
    strncpy(daemon.preconfigured_trace_load_settings[1].ctid, "CTID",
            DLT_ID_SIZE);
    ...
    strncpy(daemon.preconfigured_trace_load_settings[4].apid, "APP3",
            DLT_ID_SIZE);
    strncpy(daemon.preconfigured_trace_load_settings[4].ctid, "CT03",
            DLT_ID_SIZE);
    daemon.preconfigured_trace_load_settings[4].soft_limit = 111;
    daemon.preconfigured_trace_load_settings[4].hard_limit = 222;

    strncpy(daemon.preconfigured_trace_load_settings[5].apid, "APP3",
            DLT_ID_SIZE);
    strncpy(daemon.preconfigured_trace_load_settings[5].ctid, "CT01",
            DLT_ID_SIZE);
    daemon.preconfigured_trace_load_settings[5].soft_limit = 333;
    daemon.preconfigured_trace_load_settings[5].hard_limit = 444;

    strncpy(daemon.preconfigured_trace_load_settings[6].apid, "APP1",
            DLT_ID_SIZE);
    strncpy(daemon.preconfigured_trace_load_settings[6].ctid, "CT03",
            DLT_ID_SIZE);
...
TEST(t_dlt_daemon_application_add, trace_load_settings_not_found)
{
    ...
    app = dlt_daemon_application_add(&daemon, apid, pid, desc, fd, ecu, 0);

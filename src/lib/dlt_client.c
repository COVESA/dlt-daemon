...
DltReturnValue dlt_client_main_loop(DltClient *client, void *data, int verbose)
{
    ...
    while (fetch_next_message) {
                                ...
                                verbose) == DLT_MESSAGE_ERROR_OK)
        {
            ...
            int total_size = (int)((size_t)msg.headersize
                            + (size_t)msg.datasize
...
DltReturnValue dlt_client_send_ctrl_msg_v2(DltClient *client, char *apid, char *ctid, uint8_t *payload, uint32_t size)
{
    ...
    if (DLT_IS_HTYP2_WEID(msg.baseheaderv2->htyp2)) {
        ...
        dlt_set_id_v2((char **)&ecid_buf, client->ecuid2, (int8_t)msg.extendedheaderv2.ecidlen);
    ...
    if (DLT_IS_HTYP2_WACID(msg.baseheaderv2->htyp2)) {
        ...
        if (strcmp(apid, "") == 0){
            dlt_set_id_v2((char **)&apid_buf, DLT_CLIENT_DUMMY_APP_ID, (int8_t)msg.extendedheaderv2.apidlen);
        } else {
        ...
        if (strcmp(ctid, "") == 0){
            dlt_set_id_v2((char **)&ctid_buf, DLT_CLIENT_DUMMY_CON_ID, (int8_t)msg.extendedheaderv2.ctidlen);
        } else {
                                             ...
                                             uint32_t size)
{
    ...
    /* free message */
    if (dlt_client_send_ctrl_msg_v2(client, apid, ctid, payload,
                                 (uint32_t) (sizeof(uint32_t) + sizeof(uint32_t) + size)) == DLT_RETURN_ERROR) {
        free(payload);
        return DLT_RETURN_ERROR;
    }

...
#define DLT_SERVICE_GET_LOG_INFO_OPT7                                        \
    7 /* get Apid, ApDescription, Ctid, CtDescription, loglevel, tracestatus \
       */
#define LOG_ERR 3
/**
...
void usage()
{
    ...
    printf("  -x message    Control message injection in Hex e.g. 'ad 01 24 "
           "ef'\n");
    printf(
        "  -t milliseconds Timeout to terminate application (Default:1000)'\n");
    printf("  -l loglevel      Set the log level (0=off - 6=verbose default= "
           "-1)\n");
    printf("      supported options:\n");
    printf("       -l level -a apid -c ctid\n");
    printf("       -l level -a abc* (set level for all ctxts of apps name "
           "starts with abc)\n");
    printf("       -l level -a apid (set level for all ctxts of this app)\n");
    printf("       -l level -c xyz* (set level for all ctxts whose name starts "
           "with xyz)\n");
    ...
    printf("       -r tracestatus -a abc* (set status for all ctxts of apps "
           "name starts with abc)\n");
    printf("       -r tracestatus -a apid (set status for all ctxts of this "
           "app)\n");
    printf("       -r tracestatus -c xyz* (set status for all ctxts whose name "
           "starts with xyz)\n");
    printf(
 ...
 */
void dlt_process_get_log_info_v2(void)
{
    char *apid = NULL;
    char *ctid = NULL;
    AppIDsType app;
    ...
    DltServiceGetLogInfoResponse *resp = (DltServiceGetLogInfoResponse *)calloc(
    ...
    if (0 != dlt_client_get_log_info_v2(&g_dltclient)) {
    ...
    if (dlt_client_main_loop_v2(&g_dltclient, (void *)resp, 0) ==
        DLT_RETURN_TRUE)
    ...
    if (resp->service_id == DLT_SERVICE_ID_GET_LOG_INFO &&
        ...
        for (i = 0; i < resp->log_info_type.count_app_ids; i++) {
            ...
            apid = (char *)malloc((size_t)app.app_id2len + 1u);
            if (apid == NULL) {
            ...
            memcpy(apid, app.app_id2, app.app_id2len);
            apid[app.app_id2len] = '\0';
            ...
            for (j = 0; j < app.count_context_ids; j++) {
                ...
                ctid = (char *)malloc((size_t)con.context_id2len + 1u);
                if (ctid == NULL) {
                ...
                ctid[con.context_id2len] = '\0';

                if (con.context_description != 0) {
                    printf("CTID:%s %2d %2d %s\n", ctid, con.log_level,
                           con.trace_status, con.context_description);
                }
                else {
                    printf("CTID:%s %2d %2d\n", ctid, con.log_level,
                           con.trace_status);
 ...
 */
void dlt_process_get_software_version_v2(void)
{
    DltServiceGetSoftwareVersionResponse *resp =
        (DltServiceGetSoftwareVersionResponse *)calloc(
            1, sizeof(DltServiceGetSoftwareVersionResponse));
    ...
    /* send control message*/
    if (0 != dlt_client_get_software_version_v2(&g_dltclient)) {
    ...
    if (resp->service_id == DLT_SERVICE_ID_GET_SOFTWARE_VERSION &&
        resp->status == DLT_SERVICE_RESPONSE_OK && resp->payload != NULL) {
        printf("%s\n", resp->payload);
        free(resp->payload);
        resp->payload = NULL;
    }
...
int main(int argc, char *argv[])
{
    ...
    /* Initialize dltdata */
    dltdata = (DltReceiveData){.tvalue = 1000,
    ...
    while ((c = getopt(argc, argv, "vhSRye:b:a:c:s:m:x:t:l:r:d:f:i:ogjkup:")) !=
           -1)
        switch (c) {
        case 'v': {
            dltdata.vflag = 1;
        ...
        case 'l': {
            ...
            if ((dltdata.lvalue < DLT_LOG_DEFAULT) ||
                (dltdata.lvalue > DLT_LOG_VERBOSE)) {
        ...
        case 'r': {
            ...
            if ((dltdata.rvalue < DLT_TRACE_STATUS_DEFAULT) ||
                (dltdata.rvalue > DLT_TRACE_STATUS_ON)) {
                fprintf(
                    stderr,
                    "invalid trace status, supported trace status -1, 0, 1\n");
                return -1;
            }
    ...
    if (dltdata.yflag == DLT_CLIENT_MODE_SERIAL) {
        ...
        if (dlt_parse_config_param("ControlSocketPath",
                                   &g_dltclient.socketPath) ==
            DLT_RETURN_ERROR) {
            /* Failed to read from conf, copy default */
            if (dlt_client_set_socket_path(
                    &g_dltclient, DLT_DAEMON_DEFAULT_CTRL_SOCK_PATH) == -1) {
    ...
    if (dltdata.evalue) {
        ...
        dlt_set_id_v2(g_dltclient.ecuid2, dltdata.evalue,
                      g_dltclient.ecuid2len);
    }
    else {
        dltdata.evalue = NULL;
    ...
    if (dlt_client_connect(&g_dltclient, dltdata.vflag) != DLT_RETURN_ERROR) {
        ...
        if (dltdata.mvalue && dltdata.avalue && dltdata.cvalue) {
            ...
            if (dlt_client_send_inject_msg_v2(
                    &g_dltclient, dltdata.avalue, dltdata.cvalue,
                    (uint32_t)dltdata.svalue, (uint8_t *)dltdata.mvalue,
            ...
            if (dlt_client_send_inject_msg_v2(
                    &g_dltclient, dltdata.avalue, dltdata.cvalue,
                    (uint32_t)dltdata.svalue, buffer,
                    (uint32_t)size) != DLT_RETURN_OK) {
                fprintf(stderr, "ERROR: Could not send inject message\n");
                ret = -1;
            }
        }
        else if (dltdata.lvalue !=
                 DLT_INVALID_LOG_LEVEL) /*&& dltdata.avalue && dltdata.cvalue)*/
        {
            if ((dltdata.avalue == 0) && (dltdata.cvalue == 0)) {
                ...
                if (0 != dlt_client_send_all_log_level_v2(
                             &g_dltclient, (uint8_t)dltdata.lvalue)) {
                ...
                /* send control message*/
                if (0 != dlt_client_send_log_level_v2(
                             &g_dltclient, dltdata.avalue, dltdata.cvalue,
            ...
            if ((dltdata.avalue == 0) && (dltdata.cvalue == 0)) {
                ...
                if (0 != dlt_client_send_all_trace_status_v2(
                             &g_dltclient, (uint8_t)dltdata.rvalue)) {
                ...
                /* send control message*/
                if (0 != dlt_client_send_trace_status_v2(
                             &g_dltclient, dltdata.avalue, dltdata.cvalue,
            ...
            if (dlt_client_send_default_log_level_v2(
                    &g_dltclient, (uint8_t)dltdata.dvalue) != DLT_RETURN_OK) {
            ...
            /* send control message in*/
            if (dlt_client_send_default_trace_status_v2(
                    &g_dltclient, (uint8_t)dltdata.fvalue) != DLT_RETURN_OK) {
            ...
            /* send control message in*/
            if (dlt_client_send_timing_pakets_v2(
                    &g_dltclient, (uint8_t)dltdata.ivalue) != DLT_RETURN_OK) {
            ...
            /* send control message in*/
            if (dlt_client_send_store_config_v2(&g_dltclient) !=
                DLT_RETURN_OK) {
            ...
            /* send control message in*/
            if (dlt_client_send_reset_to_factory_default_v2(&g_dltclient) !=
                DLT_RETURN_OK) {
                fprintf(stderr,
                        "ERROR: Could not send reset to factory default\n");
                ret = -1;
            }
        }
            ...
            /* get log info */
            printf("Get log info:\n");
            dlt_process_get_log_info_v2();
        }
        else if (dltdata.kvalue == 1) {
            /* Get software version */
            printf("Get software version:\n");
            dlt_process_get_software_version_v2();
        }
        ...
        /* Wait timeout */
        ts.tv_sec =
            (long int)(dltdata.tvalue * NANOSEC_PER_MILLISEC) / NANOSEC_PER_SEC;
        ts.tv_nsec =
            (long int)(dltdata.tvalue * NANOSEC_PER_MILLISEC) % NANOSEC_PER_SEC;
        nanosleep(&ts, NULL);
    }
...
int dlt_receive_message_callback_v2(DltMessageV2 *message, void *data)
{
    ...
    if ((message == NULL) || (data == NULL) || !DLT_MSG_IS_CONTROL_V2(message))
        return -1;
    ...
    if ((id > DLT_SERVICE_ID) && (id < DLT_SERVICE_ID_LAST_ENTRY) &&
        ...
        case DLT_SERVICE_ID_GET_LOG_INFO: {
            ...
            if (DLT_IS_HTYP_WEID(message->baseheaderv2->htyp2))
                dlt_set_storageheader_v2(&(message->storageheaderv2),
                                         message->extendedheaderv2.ecidlen,
                                         message->extendedheaderv2.ecid);
            else
                dlt_set_storageheader_v2(&(message->storageheaderv2), 4,
                                         "LCTL");
            message->storageheadersizev2 =
                (uint32_t)(STORAGE_HEADER_V2_FIXED_SIZE +
                           message->storageheaderv2.ecidlen);

            /* Add Storage Header to Header Buffer and update header size */
            uint8_t *old_headerbuffer = message->headerbufferv2;
            uint8_t *new_headerbuffer = NULL;
            uint8_t *temp_buffer = NULL;
            int32_t old_headersize = message->headersizev2;

            if ((old_headerbuffer == NULL) || (old_headersize <= 0)) {
                dlt_client_cleanup(&g_dltclient, 0);
                return -1;
            }

            temp_buffer = (uint8_t *)malloc((size_t)old_headersize);
            if (temp_buffer == NULL) {
                dlt_client_cleanup(&g_dltclient, 0);
                return -1;
            }

            memcpy(temp_buffer, old_headerbuffer, (size_t)old_headersize);

            message->headersizev2 =
                old_headersize + (int32_t)message->storageheadersizev2;

            new_headerbuffer = (uint8_t *)malloc((size_t)message->headersizev2);
            if (new_headerbuffer == NULL) {
                free(temp_buffer);
                dlt_client_cleanup(&g_dltclient, 0);
                return -1;
            }

            message->headerbufferv2 = new_headerbuffer;
            free(old_headerbuffer);

            if (dlt_message_set_storageparameters_v2(message, 0) !=
                DLT_RETURN_OK) {
                free(temp_buffer);
                dlt_client_cleanup(&g_dltclient, 0);
                return -1;
            ...
            memcpy(message->headerbufferv2 + message->storageheadersizev2,
                   temp_buffer, (size_t)old_headersize);

            free(temp_buffer);
            temp_buffer = NULL;

            /* get response data */
            ret = dlt_message_header_v2(message, resp_text, DLT_RECEIVE_BUFSIZE,
                                        0);
            ...
            ret = dlt_message_payload_v2(
                message, resp_text, DLT_RECEIVE_BUFSIZE, DLT_OUTPUT_ASCII, 0);
            ...
            ret = dlt_set_loginfo_parse_service_id(resp_text, &resp->service_id,
                                                   &resp->status);

            if ((ret == 0) &&
                (resp->service_id == DLT_SERVICE_ID_GET_LOG_INFO)) {
                ret =
                    dlt_client_parse_get_log_info_resp_text_v2(resp, resp_text);
        ...
        case DLT_SERVICE_ID_GET_SOFTWARE_VERSION: {
            ...
            if (resp->length > (uint32_t)datalength) {
                fprintf(stderr,
                        "GET_SOFTWARE_VERSION payload length is invalid "
            ...
            resp->payload =
                (char *)calloc((size_t)resp->length + 1u, sizeof(char));

            if (resp->payload == NULL) {
                fprintf(stderr,
                        "GET_SOFTWARE_VERSION payload allocation failed.\n");
                dlt_client_cleanup(&g_dltclient, 0);
                return -1;
            }

            memcpy(resp->payload, ptr, (size_t)resp->length);

            resp->payload[resp->length] = '\0';

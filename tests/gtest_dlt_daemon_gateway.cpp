...
TEST(t_dlt_gateway_send_control_message, Normal)
{
    ...
    EXPECT_EQ(
        ...
        dlt_gateway_send_control_message(
            daemon_local.pGateway.connections, daemon_local.pGateway.connections->p_control_msgs, NULL, 0));
...
TEST(t_dlt_gateway_parse_get_log_info, normal)
{
    ...
    msg.datasize = static_cast<int32_t>(
        sizeof(DltServiceGetLogInfoResponse) + sizeof(AppIDsType) + sizeof(ContextIDsInfoType) + strlen(app_description)
    ...
    msg.headersize = static_cast<int32_t>(
        sizeof(DltStorageHeader) + sizeof(DltStandardHeader) + sizeof(DltExtendedHeader)
        + DLT_STANDARD_HEADER_EXTRA_SIZE(msg.standardheader->htyp));
...
TEST(t_dlt_gateway_process_on_demand_request, normal)
{
    ...
    EXPECT_EQ(
        DLT_RETURN_OK,
        dlt_gateway_process_on_demand_request(&daemon_local.pGateway, &daemon_local, node_id, conn_status, 1));
}
...
TEST(t_dlt_gateway_process_on_demand_request, abnormal)
{
    ...
    EXPECT_EQ(
        DLT_RETURN_ERROR,
        dlt_gateway_process_on_demand_request(&daemon_local.pGateway, &daemon_local, node_id, conn_status, 0));
}
...
TEST(t_dlt_gateway_check_param, abnormal)
{
    ...
    EXPECT_EQ(DLT_RETURN_ERROR, dlt_gateway_check_param(&gateway, &tmp, GW_CONF_PORT, value_1));
}

...
int main(int argc, char *argv[])
{
    ...
    DLT_REGISTER_CONTEXT_LLCCB(mycontext2, "TS1", "Test Context1 for injection",
                               dlt_user_log_level_changed_callback);
    DLT_REGISTER_CONTEXT_LLCCB(mycontext3, "TS2", "Test Context2 for injection",
                               dlt_user_log_level_changed_callback);
    ...
    DLT_REGISTER_LOG_LEVEL_CHANGED_CALLBACK(
        mycontext1, dlt_user_log_level_changed_callback);
    ...
    for (num = 0; num < maxnum; num++) {
        ...
        if (gflag) {
            ...
            if (rvalue == -1)
                ...
                DLT_LOG(mycontext1, lvalue, DLT_RAW(text, (uint16_t)rvalue));
        }
                                ...
                                uint32_t length)
{
    ...
    DLT_LOG(mycontext1, DLT_LOG_INFO, DLT_STRING("Injection: "),
            DLT_UINT32(service_id));
    printf("Injection %d, Length=%d \n", service_id, length);

    if (length > 0) {
        dlt_print_mixed_string(text, 1024, data, (int)length, 0);
        DLT_LOG(mycontext1, DLT_LOG_INFO, DLT_STRING("Data: "),
                DLT_STRING(text));
        printf("%s \n", text);
    }

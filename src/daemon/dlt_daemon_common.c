                                     ...
                                     int verbose)
{
    ...
    if (gateway_mode == 0) {
        ...
        dlt_set_id_v2(&(daemon->user_list[0].ecuid2), daemon->ecuid2, (int8_t)daemon->ecuid2len);
        ...
        for (i = 1; i < nodes; i++) {
            ...
            dlt_set_id_v2(&(daemon->user_list[0].ecuid2), gateway->connections[i - 1].ecuid2, (int8_t)gateway->connections[i - 1].ecuid2len);
        }
    }
    ...
    if (application == NULL) {
        ...
        if (user_list->num_applications != 0) {
            if ((user_list->num_applications % DLT_DAEMON_APPL_ALLOC_SIZE) == 0) {
                ...
                memcpy(user_list->applications,
                       old,
                       sizeof(DltDaemonApplication) * (size_t)user_list->num_applications);
                free(old);
            }
        }
        ...
        dlt_set_id_v2(&(application->apid2), apid, (int8_t)apidlen);
        application->pid = 0;
        application->application_description = NULL;
        application->num_contexts = 0;
        application->user_handle = DLT_FD_INIT;
                                    ...
                                    DltDaemonApplication **application)
{
    ...
    dlt_set_id_v2(&(search_app.apid2), apid, (int8_t)apidlen);
...
int dlt_daemon_applications_save_v2(DltDaemon *daemon, const char *filename, int verbose)
{
    ...
    if ((user_list->applications != NULL) && (user_list->num_applications > 0)) {
        ...
        if (fd != NULL) {
            for (i = 0; i < user_list->num_applications; i++) {
                dlt_set_id_v2(&apid, user_list->applications[i].apid2, (int8_t)user_list->applications[i].apid2len);
                                         ...
                                         int verbose)
{
    ...
    if (context == NULL) {
        ...
        context->apid2 = NULL;
        dlt_set_id_v2(&(context->apid2), apid, (int8_t)apidlen);
        context->apid2len = apidlen;
        context->ctid2 = NULL;
        dlt_set_id_v2(&(context->ctid2), ctid, (int8_t)ctidlen);
...
int dlt_daemon_contexts_save_v2(DltDaemon *daemon, const char *filename, int verbose)
{
    ...
    if ((user_list->contexts) && (user_list->num_contexts > 0)) {
        ...
        if (fd != NULL) {
            for (i = 0; i < user_list->num_contexts; i++) {
                dlt_set_id_v2(&apid, user_list->contexts[i].apid2, (int8_t)user_list->contexts[i].apid2len);
                dlt_set_id_v2(&ctid, user_list->contexts[i].ctid2, (int8_t)user_list->contexts[i].ctid2len);

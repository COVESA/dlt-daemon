...
struct logger *t_android_logger_open(struct logger_list *logger_list,
                                     log_id_t log_id)
{
                               ...
                               struct log_msg *_t_log_msg)
{
    ...
    if (!_t_log_msg->entry.lid) {
        _t_log_msg->entry.lid = LOG_ID_MAIN;
    }
    if (!_t_log_msg->buf[_t_log_msg->entry.hdr_size]) {
        _t_log_msg->buf[_t_log_msg->entry.hdr_size] =
            (unsigned char)ANDROID_LOG_INFO;
    }
...
TEST(t_usage, normal)
{
    ...
    EXPECT_NE(buffer.str().find(
                  "Format and forward Android messages from ANDROID to DLT."),
              string::npos);
...
TEST(t_init_configuration, normal)
{
    ...
    EXPECT_STREQ("/vendor/etc/dlt-logd-converter.conf",
                 logd_conf->conf_file_dir);
}
...
TEST(t_clean_mem, normal)
{
    ...
    EXPECT_TRUE(map_ctx_json.find("NetworkSecurityConfig") ==
                map_ctx_json.end());
...
TEST(t_init_logger, normal)
{
    ...
    t_logger_list->pid = 0;
    struct logger *logger_ptr =
        reinterpret_cast<struct logger *>(LOG_ID_MAIN | LOGGER_LOGD);
    ...
    logger_ptr =
        reinterpret_cast<struct logger *>(LOG_ID_SECURITY | LOGGER_LOGD);
...
TEST(t_get_log_context_from_log_msg, normal)
{
    ...
    _t_log_msg->entry.lid = LOG_ID_RADIO;
    EXPECT_EQ(&dlt_ctx_rdio, get_log_context_from_log_msg(_t_log_msg));
    _t_log_msg->entry.lid = LOG_ID_EVENTS;
    EXPECT_EQ(&dlt_ctx_evnt, get_log_context_from_log_msg(_t_log_msg));
    _t_log_msg->entry.lid = LOG_ID_SYSTEM;
    EXPECT_EQ(&dlt_ctx_syst, get_log_context_from_log_msg(_t_log_msg));
    _t_log_msg->entry.lid = LOG_ID_CRASH;
    EXPECT_EQ(&dlt_ctx_crsh, get_log_context_from_log_msg(_t_log_msg));
    _t_log_msg->entry.lid = LOG_ID_STATS;
    EXPECT_EQ(&dlt_ctx_stat, get_log_context_from_log_msg(_t_log_msg));
    _t_log_msg->entry.lid = LOG_ID_SECURITY;
    EXPECT_EQ(&dlt_ctx_secu, get_log_context_from_log_msg(_t_log_msg));
    _t_log_msg->entry.lid = LOG_ID_KERNEL;
    EXPECT_EQ(&dlt_ctx_krnl, get_log_context_from_log_msg(_t_log_msg));
    _t_log_msg->entry.lid = 1024;
...
TEST(t_get_timestamp_from_log_msg, normal)
{
    ...
    _t_log_msg->entry.nsec = 1000000;
    EXPECT_EQ(1000010, get_timestamp_from_log_msg(_t_log_msg));
}
...
TEST(t_get_log_level_from_log_msg, normal)
{
    ...
    EXPECT_EQ(DLT_LOG_VERBOSE, get_log_level_from_log_msg(_t_log_msg));
    _t_log_msg->buf[0] = (unsigned char)ANDROID_LOG_DEBUG;
    EXPECT_EQ(DLT_LOG_DEBUG, get_log_level_from_log_msg(_t_log_msg));
    _t_log_msg->buf[0] = (unsigned char)ANDROID_LOG_INFO;
    EXPECT_EQ(DLT_LOG_INFO, get_log_level_from_log_msg(_t_log_msg));
    _t_log_msg->buf[0] = (unsigned char)ANDROID_LOG_WARN;
    EXPECT_EQ(DLT_LOG_WARN, get_log_level_from_log_msg(_t_log_msg));
    _t_log_msg->buf[0] = (unsigned char)ANDROID_LOG_ERROR;
    EXPECT_EQ(DLT_LOG_ERROR, get_log_level_from_log_msg(_t_log_msg));
    _t_log_msg->buf[0] = (unsigned char)ANDROID_LOG_FATAL;
    EXPECT_EQ(DLT_LOG_FATAL, get_log_level_from_log_msg(_t_log_msg));
    _t_log_msg->buf[0] = (unsigned char)ANDROID_LOG_SILENT;
    EXPECT_EQ(DLT_LOG_OFF, get_log_level_from_log_msg(_t_log_msg));
    _t_log_msg->buf[0] = (unsigned char)ANDROID_LOG_UNKNOWN;
    EXPECT_EQ(DLT_LOG_DEFAULT, get_log_level_from_log_msg(_t_log_msg));
    _t_log_msg->buf[0] = (unsigned char)ANDROID_LOG_DEFAULT;
    EXPECT_EQ(DLT_LOG_DEFAULT, get_log_level_from_log_msg(_t_log_msg));
    delete _t_log_msg;
    _t_log_msg = nullptr;
}
...
TEST(t_logd_parser_loop, normal)
{
    ...
    for (uint idx = 0; idx < sizeof(message); idx++) {
        t_log_msg.buf[idx + t_log_msg.entry.hdr_size + sizeof(tag) + 1] =
            message[idx];
    }
    ...
    for (uint idx = 0; idx < sizeof(othe_message); idx++) {
        t_log_msg.buf[idx + t_log_msg.entry.hdr_size + sizeof(othe_tag) + 1] =
            othe_message[idx];
    }
    ...
    unsigned char radio_message[sizeof("It is from radio buffer")] =
        "It is from radio buffer";
    ...
    for (uint idx = 0; idx < sizeof(radio_message); idx++) {
        t_log_msg.buf[idx + t_log_msg.entry.hdr_size + sizeof(radio_tag) + 1] =
            radio_message[idx];
    }
    ret = logd_parser_loop(t_logger_list);
    EXPECT_EQ(DLT_RETURN_OK, ret);
    ...
    unsigned char no_json_message[sizeof("It is from kernel")] =
        "It is from kernel";
    ...
    for (uint idx = 0; idx < sizeof(no_json_message); idx++) {
        t_log_msg
            .buf[idx + t_log_msg.entry.hdr_size + sizeof(no_json_tag) + 1] =
            no_json_message[idx];
    }
    ret = logd_parser_loop(t_logger_list);
    EXPECT_EQ(DLT_RETURN_OK, ret);

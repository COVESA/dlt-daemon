...
DltReturnValue dlt_buffer_push(DltBuffer *, const unsigned char *,
                               unsigned int);
DltReturnValue dlt_buffer_push3(DltBuffer *, const unsigned char *,
...
void dlt_buffer_write_block(DltBuffer *, int *, const unsigned char *,
                            unsigned int);
...
TEST(t_dlt_buffer_init_dynamic, normal)
{
    ...
    EXPECT_LE(DLT_RETURN_OK,
              dlt_buffer_init_dynamic(
                  &init_dynamic, DLT_USER_RINGBUFFER_MIN_SIZE,
                  DLT_USER_RINGBUFFER_MAX_SIZE, DLT_USER_RINGBUFFER_STEP_SIZE));
    EXPECT_LE(DLT_RETURN_OK, dlt_buffer_free_dynamic(&init_dynamic));

    /* Min Values for a success init */
    EXPECT_LE(DLT_RETURN_OK,
              dlt_buffer_init_dynamic(&init_dynamic, 12, 12, 12));
    EXPECT_LE(DLT_RETURN_OK, dlt_buffer_free_dynamic(&init_dynamic));
}
TEST(t_dlt_buffer_init_dynamic, abnormal)
{
    ...
    /*    EXPECT_LE(DLT_RETURN_OK, dlt_buffer_init_dynamic(&buf,
     * DLT_USER_RINGBUFFER_MIN_SIZE, DLT_USER_RINGBUFFER_MAX_SIZE,
     * DLT_USER_RINGBUFFER_STEP_SIZE)); */
    /*  EXPECT_GE(-1, dlt_buffer_init_dynamic(&buf,
     * DLT_USER_RINGBUFFER_MIN_SIZE, DLT_USER_RINGBUFFER_MAX_SIZE,
     * DLT_USER_RINGBUFFER_STEP_SIZE)); */
    /*    EXPECT_LE(DLT_RETURN_OK,dlt_buffer_free_dynamic(&buf)); */

    /* Initialize buffer with max-value of uint32, expected 0 */
    /* TODO: what should the maximum parameter values be? UINT_MAX is too large
    ...
    /*    EXPECT_GE(-1, dlt_buffer_init_dynamic(&buf,
     * DLT_USER_RINGBUFFER_MAX_SIZE, DLT_USER_RINGBUFFER_MIN_SIZE,
     * DLT_USER_RINGBUFFER_STEP_SIZE)); */
    /*    EXPECT_LE(DLT_RETURN_OK,dlt_buffer_free_dynamic(&buf)); */
    ...
    /*    EXPECT_GE(-1,dlt_buffer_init_dynamic(&buf,
     * DLT_USER_RINGBUFFER_MIN_SIZE, DLT_USER_RINGBUFFER_MAX_SIZE,
     * DLT_USER_RINGBUFFER_MAX_SIZE * 2)); */
    /*    EXPECT_LE(DLT_RETURN_OK,dlt_buffer_free_dynamic(&buf)); */
}
TEST(t_dlt_buffer_init_dynamic, nullpointer)
{
    ...
    EXPECT_GE(DLT_RETURN_ERROR, dlt_buffer_init_dynamic(
                                    NULL, 0, 0, DLT_USER_RINGBUFFER_STEP_SIZE));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_buffer_init_dynamic(
                                    NULL, 0, DLT_USER_RINGBUFFER_MAX_SIZE, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_init_dynamic(NULL, 0, DLT_USER_RINGBUFFER_MAX_SIZE,
                                      DLT_USER_RINGBUFFER_STEP_SIZE));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_buffer_init_dynamic(
                                    NULL, DLT_USER_RINGBUFFER_MIN_SIZE, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_init_dynamic(NULL, DLT_USER_RINGBUFFER_MIN_SIZE, 0,
                                      DLT_USER_RINGBUFFER_STEP_SIZE));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_init_dynamic(NULL, DLT_USER_RINGBUFFER_MIN_SIZE,
                                      DLT_USER_RINGBUFFER_MAX_SIZE, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_init_dynamic(NULL, DLT_USER_RINGBUFFER_MIN_SIZE,
    ...
    EXPECT_GE(DLT_RETURN_ERROR, dlt_buffer_init_dynamic(
                                    &buf, 0, 0, DLT_USER_RINGBUFFER_STEP_SIZE));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_buffer_init_dynamic(
                                    &buf, 0, DLT_USER_RINGBUFFER_MAX_SIZE, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_init_dynamic(&buf, 0, DLT_USER_RINGBUFFER_MAX_SIZE,
                                      DLT_USER_RINGBUFFER_STEP_SIZE));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_buffer_init_dynamic(
                                    &buf, DLT_USER_RINGBUFFER_MIN_SIZE, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_init_dynamic(&buf, DLT_USER_RINGBUFFER_MIN_SIZE, 0,
                                      DLT_USER_RINGBUFFER_STEP_SIZE));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_init_dynamic(&buf, DLT_USER_RINGBUFFER_MIN_SIZE,
                                      DLT_USER_RINGBUFFER_MAX_SIZE, 0));
}
...
TEST(t_dlt_buffer_increase_size, abnormal)
{
    ...
    EXPECT_LE(DLT_RETURN_OK,
              dlt_buffer_init_dynamic(&buf, DLT_USER_RINGBUFFER_MAX_SIZE,
                                      DLT_USER_RINGBUFFER_MAX_SIZE,
                                      DLT_USER_RINGBUFFER_STEP_SIZE));
    EXPECT_GE(-1, dlt_buffer_increase_size(&buf));
    EXPECT_LE(DLT_RETURN_OK, dlt_buffer_free_dynamic(&buf));
    ...
    EXPECT_GE(-1, dlt_buffer_init_dynamic(&buf, DLT_USER_RINGBUFFER_MAX_SIZE,
                                          DLT_USER_RINGBUFFER_MIN_SIZE,
                                          DLT_USER_RINGBUFFER_STEP_SIZE));

    /* init with 0 step size should fail, expected -1 */
    EXPECT_GE(-1, dlt_buffer_init_dynamic(&buf, DLT_USER_RINGBUFFER_MIN_SIZE,
                                          DLT_USER_RINGBUFFER_MAX_SIZE, 0));
}
...
TEST(t_dlt_buffer_minimize_size, normal)
{
    ...
    for (int i = (DLT_USER_RINGBUFFER_MAX_SIZE / DLT_USER_RINGBUFFER_MIN_SIZE);
         i >= 0; i -= DLT_USER_RINGBUFFER_STEP_SIZE)
        EXPECT_LE(0, dlt_buffer_minimize_size(&buf));
...
TEST(t_dlt_buffer_push, normal)
{
    ...
    EXPECT_LE(DLT_RETURN_OK,
              dlt_buffer_init_dynamic(&buf, DLT_USER_RINGBUFFER_MIN_SIZE,
                                      ...
                                      DLT_USER_RINGBUFFER_STEP_SIZE));
    EXPECT_LE(DLT_RETURN_OK,
              dlt_buffer_push(&buf, (unsigned char *)&test, size));
    EXPECT_LE(DLT_RETURN_OK, dlt_buffer_free_dynamic(&buf));
    ...
    for (unsigned int i = 0; i <= (DLT_USER_RINGBUFFER_MIN_SIZE /
                                   (size + sizeof(DltBufferBlockHead)));
         i++) {
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_buffer_push(&buf, (unsigned char *)&test, size));
    }
...
TEST(t_dlt_buffer_push, nullpointer)
{
    ...
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push(NULL, (unsigned char *)&test, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push(NULL, (unsigned char *)&test, size));
}
...
TEST(t_dlt_buffer_push3, normal)
{
    ...
    EXPECT_LE(DLT_RETURN_OK,
              dlt_buffer_init_dynamic(&buf, DLT_USER_RINGBUFFER_MIN_SIZE,
                                      ...
                                      DLT_USER_RINGBUFFER_STEP_SIZE));
    EXPECT_LE(DLT_RETURN_OK,
              dlt_buffer_push3(&buf, (unsigned char *)&test, size, 0, 0, 0, 0));
    EXPECT_LE(DLT_RETURN_OK, dlt_buffer_free_dynamic(&buf));
    ...
    EXPECT_LE(DLT_RETURN_OK,
              dlt_buffer_init_dynamic(&buf, DLT_USER_RINGBUFFER_MIN_SIZE,
                                      ...
                                      DLT_USER_RINGBUFFER_STEP_SIZE));
    EXPECT_LE(DLT_RETURN_OK,
              dlt_buffer_push3(&buf, (unsigned char *)&test, size,
    ...
    for (unsigned int i = 0; i <= (DLT_USER_RINGBUFFER_MIN_SIZE /
                                   (size * 3 + sizeof(DltBufferBlockHead)));
         i++)
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_buffer_push3(&buf, (unsigned char *)&test, size,
                                   (unsigned char *)&test, size,
...
TEST(t_dlt_buffer_push3, nullpointer)
{
    ...
    /*Null Pointer, expected -1 */
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, NULL, 0, NULL, 0, NULL, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, NULL, 0, NULL, 0, NULL, size));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_buffer_push3(NULL, NULL, 0, NULL, 0,
                                                 (unsigned char *)&test, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_buffer_push3(NULL, NULL, 0, NULL, 0,
                                                 (unsigned char *)&test, size));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, NULL, 0, NULL, size, NULL, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, NULL, 0, NULL, size, NULL, size));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_buffer_push3(NULL, NULL, 0, NULL, size,
                                                 (unsigned char *)&test, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_buffer_push3(NULL, NULL, 0, NULL, size,
                                                 (unsigned char *)&test, size));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_buffer_push3(NULL, NULL, 0, (unsigned char *)&test, 0, NULL, 0));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_buffer_push3(NULL, NULL, 0, (unsigned char *)&test, 0, NULL, size));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, NULL, 0, (unsigned char *)&test, 0,
                               (unsigned char *)&test, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, NULL, 0, (unsigned char *)&test, 0,
                               (unsigned char *)&test, size));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_buffer_push3(NULL, NULL, 0, (unsigned char *)&test, size, NULL, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, NULL, 0, (unsigned char *)&test, size,
                               NULL, size));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, NULL, 0, (unsigned char *)&test, size,
                               (unsigned char *)&test, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, NULL, 0, (unsigned char *)&test, size,
                               (unsigned char *)&test, size));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, NULL, size, NULL, 0, NULL, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, NULL, size, NULL, 0, NULL, size));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_buffer_push3(NULL, NULL, size, NULL, 0,
                                                 (unsigned char *)&test, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_buffer_push3(NULL, NULL, size, NULL, 0,
                                                 (unsigned char *)&test, size));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, NULL, size, NULL, size, NULL, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, NULL, size, NULL, size, NULL, size));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_buffer_push3(NULL, NULL, size, NULL, size,
                                                 (unsigned char *)&test, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_buffer_push3(NULL, NULL, size, NULL, size,
                                                 (unsigned char *)&test, size));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_buffer_push3(NULL, NULL, size, (unsigned char *)&test, 0, NULL, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, NULL, size, (unsigned char *)&test, 0,
                               NULL, size));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, NULL, size, (unsigned char *)&test, 0,
                               (unsigned char *)&test, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, NULL, size, (unsigned char *)&test, 0,
                               (unsigned char *)&test, size));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, NULL, size, (unsigned char *)&test, size,
                               NULL, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, NULL, size, (unsigned char *)&test, size,
                               NULL, size));
    ...
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, NULL, size, (unsigned char *)&test, size,
                               (unsigned char *)&test, size));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_buffer_push3(NULL, (unsigned char *)&test,
                                                 0, NULL, 0, NULL, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_buffer_push3(NULL, (unsigned char *)&test,
                                                 0, NULL, 0, NULL, size));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, (unsigned char *)&test, 0, NULL, 0,
                               (unsigned char *)&test, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, (unsigned char *)&test, 0, NULL, 0,
                               (unsigned char *)&test, size));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_buffer_push3(NULL, (unsigned char *)&test,
                                                 0, NULL, size, NULL, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_buffer_push3(NULL, (unsigned char *)&test,
                                                 0, NULL, size, NULL, size));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, (unsigned char *)&test, 0, NULL, size,
                               (unsigned char *)&test, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, (unsigned char *)&test, 0, NULL, size,
                               (unsigned char *)&test, size));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, (unsigned char *)&test, 0,
                               (unsigned char *)&test, 0, NULL, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, (unsigned char *)&test, 0,
                               (unsigned char *)&test, 0, NULL, size));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_buffer_push3(NULL, (unsigned char *)&test,
                                                 0, (unsigned char *)&test, 0,
                                                 (unsigned char *)&test, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_buffer_push3(NULL, (unsigned char *)&test,
                                                 0, (unsigned char *)&test, 0,
                                                 (unsigned char *)&test, size));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, (unsigned char *)&test, 0,
                               (unsigned char *)&test, size, NULL, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, (unsigned char *)&test, 0,
                               (unsigned char *)&test, size, NULL, size));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, (unsigned char *)&test, 0,
                               (unsigned char *)&test, size,
                               (unsigned char *)&test, 0));
    ...
    EXPECT_GE(DLT_RETURN_ERROR, dlt_buffer_push3(NULL, (unsigned char *)&test,
                                                 size, NULL, 0, NULL, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_buffer_push3(NULL, (unsigned char *)&test,
                                                 size, NULL, 0, NULL, size));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, (unsigned char *)&test, size, NULL, 0,
                               (unsigned char *)&test, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, (unsigned char *)&test, size, NULL, 0,
                               (unsigned char *)&test, size));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_buffer_push3(NULL, (unsigned char *)&test,
                                                 size, NULL, size, NULL, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_buffer_push3(NULL, (unsigned char *)&test,
                                                 size, NULL, size, NULL, size));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, (unsigned char *)&test, size, NULL, size,
                               (unsigned char *)&test, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, (unsigned char *)&test, size, NULL, size,
                               (unsigned char *)&test, size));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, (unsigned char *)&test, size,
                               (unsigned char *)&test, 0, NULL, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_buffer_push3(NULL, (unsigned char *)&test, size,
                               (unsigned char *)&test, 0, NULL, size));
...
TEST(t_dlt_buffer_pull, normal)
{
    ...
    EXPECT_LE(DLT_RETURN_OK,
              dlt_buffer_init_dynamic(&buf, DLT_USER_RINGBUFFER_MIN_SIZE,
                                      DLT_USER_RINGBUFFER_MAX_SIZE,
                                      DLT_USER_RINGBUFFER_STEP_SIZE));
    EXPECT_LE(DLT_RETURN_OK, dlt_buffer_push(&buf, (unsigned char *)&header,
...
TEST(t_dlt_buffer_pull, abnormal)
{
    ...
    /*    EXPECT_GE(-1, dlt_buffer_pull(&buf, (unsigned char*)&header,
     * sizeof(DltUserHeader))); */
     ...
     * DLT_USER_RINGBUFFER_MIN_SIZE, DLT_USER_RINGBUFFER_MAX_SIZE,
     * DLT_USER_RINGBUFFER_STEP_SIZE)); */
    /*    EXPECT_LE(DLT_RETURN_OK, dlt_buffer_push(&buf,(unsigned char
     * *)&header,sizeof(DltUserHeader))); */
...
TEST(t_dlt_buffer_pull, nullpointer)
{
    ...
    EXPECT_GE(-1, dlt_buffer_pull(NULL, (unsigned char *)&header,
                                  sizeof(DltUserHeader)));
    EXPECT_LE(DLT_RETURN_OK,
              dlt_buffer_init_dynamic(&buf, DLT_USER_RINGBUFFER_MIN_SIZE,
                                      DLT_USER_RINGBUFFER_MAX_SIZE,
...
TEST(t_dlt_buffer_remove, normal)
{
    ...
    EXPECT_LE(DLT_RETURN_OK,
              dlt_buffer_init_dynamic(&buf, DLT_USER_RINGBUFFER_MIN_SIZE,
                                      ...
                                      DLT_USER_RINGBUFFER_STEP_SIZE));
    EXPECT_LE(DLT_RETURN_OK,
              dlt_buffer_push(&buf, (unsigned char *)&header, size));
...
TEST(t_dlt_buffer_copy, nullpointer)
{
    ...
    EXPECT_LE(DLT_RETURN_WRONG_PARAMETER,
              dlt_buffer_copy(NULL, (unsigned char *)&header, size));
    EXPECT_LE(DLT_RETURN_WRONG_PARAMETER,
              dlt_buffer_copy(NULL, (unsigned char *)&header, 0));
    EXPECT_LE(DLT_RETURN_OK,
              dlt_buffer_init_dynamic(&buf, DLT_USER_RINGBUFFER_MIN_SIZE,
                                      DLT_USER_RINGBUFFER_MAX_SIZE,
                                      DLT_USER_RINGBUFFER_STEP_SIZE));
...
TEST(t_dlt_buffer_get, abnormal)
{
    ...
    /*    EXPECT_LE(DLT_RETURN_OK, dlt_buffer_push(&buf,(unsigned char
     * *)&header,size)); */
    /*    printf("#### %i\n", dlt_buffer_get(&buf,(unsigned
     * char*)&header,size,0)); */
    /*    EXPECT_LE(0, dlt_buffer_get(&buf,(unsigned
     * char*)&header,size,12345678)); */
    /*    EXPECT_LE(DLT_RETURN_OK, dlt_buffer_free_dynamic(&buf)); */
}
...
TEST(t_dlt_buffer_get_message_count, normal)
{
    ...
    /* Pushing DLT_USER_RINGBUFFER_MIN_SIZE / (sizeof(DltUserHeader) +
     * sizeof(DltBufferBlockHead)) mesages */
    EXPECT_LE(DLT_RETURN_OK,
              dlt_buffer_init_dynamic(&buf, DLT_USER_RINGBUFFER_MIN_SIZE,
                                      DLT_USER_RINGBUFFER_MAX_SIZE,
                                      DLT_USER_RINGBUFFER_STEP_SIZE));
    ...
    for (unsigned int i = 1;
         i <= DLT_USER_RINGBUFFER_MIN_SIZE /
                  (sizeof(DltUserHeader) + sizeof(DltBufferBlockHead));
         i++) {
        EXPECT_LE(DLT_RETURN_OK, dlt_buffer_push(&buf, (unsigned char *)&header,
                                                 sizeof(DltUserHeader)));
        /*printf("#### %i\n", dlt_buffer_get_message_count(&buf)); */
...
TEST(t_dlt_buffer_get_total_size, normal)
{
    ...
    /* Normal Use-Case, 1st pushing data, expected max buffer size
     * (DLT_USER_RINGBUFFER_MAX_SIZE) */
    EXPECT_LE(DLT_RETURN_OK,
              dlt_buffer_init_dynamic(&buf, DLT_USER_RINGBUFFER_MIN_SIZE,
                                      DLT_USER_RINGBUFFER_MAX_SIZE,
...
TEST(t_dlt_buffer_get_used_size, normal)
{
    ...
    /* Normal Use-Case with pushing data, expected sum of DltUserHeader and
     * DltBufferBlockHead */
    EXPECT_LE(DLT_RETURN_OK,
              dlt_buffer_init_dynamic(&buf, DLT_USER_RINGBUFFER_MIN_SIZE,
                                      DLT_USER_RINGBUFFER_MAX_SIZE,
    ...
    EXPECT_LE(DLT_RETURN_OK, dlt_buffer_push(&buf, (unsigned char *)&header,
                                             sizeof(DltUserHeader)));
    /*printf("##### %i\n", dlt_buffer_get_used_size(&buf)); */
    EXPECT_EQ(sizeof(DltUserHeader) + sizeof(DltBufferBlockHead),
              dlt_buffer_get_used_size(&buf));
    EXPECT_LE(DLT_RETURN_OK, dlt_buffer_free_dynamic(&buf));

    /* Normal Use-Case with pushing DLT_USER_RINGBUFFER_MIN_SIZE /
     * (sizeof(DltUserHeader) + sizeof(DltBufferBlockHead)) data */
    EXPECT_LE(DLT_RETURN_OK,
              dlt_buffer_init_dynamic(&buf, DLT_USER_RINGBUFFER_MIN_SIZE,
                                      DLT_USER_RINGBUFFER_MAX_SIZE,
                                      DLT_USER_RINGBUFFER_STEP_SIZE));
...
TEST(t_dlt_buffer_write_block, abnormal)
{
    ...
    EXPECT_LE(DLT_RETURN_OK,
              dlt_buffer_init_dynamic(&buf, DLT_USER_RINGBUFFER_MIN_SIZE,
                                      ...
                                      DLT_USER_RINGBUFFER_STEP_SIZE));
    EXPECT_NO_THROW(
        dlt_buffer_write_block(&buf, &write, (unsigned char *)&data, size));
...
TEST(t_dlt_buffer_write_block, nullpointer)
{
    ...
    EXPECT_NO_THROW(
        dlt_buffer_write_block(NULL, NULL, (unsigned char *)&data, 0));
    EXPECT_NO_THROW(
        dlt_buffer_write_block(NULL, NULL, (unsigned char *)&data, test1));
    EXPECT_NO_THROW(dlt_buffer_write_block(NULL, &write, NULL, 0));
    EXPECT_NO_THROW(dlt_buffer_write_block(NULL, &write, NULL, test1));
    EXPECT_NO_THROW(
        dlt_buffer_write_block(NULL, &write, (unsigned char *)&data, 0));
    EXPECT_NO_THROW(
        dlt_buffer_write_block(NULL, &write, (unsigned char *)&data, test1));
    EXPECT_LE(DLT_RETURN_OK,
              dlt_buffer_init_dynamic(&buf, DLT_USER_RINGBUFFER_MIN_SIZE,
                                      DLT_USER_RINGBUFFER_MAX_SIZE,
                                      DLT_USER_RINGBUFFER_STEP_SIZE));
    ...
    EXPECT_LE(DLT_RETURN_OK,
              dlt_buffer_init_dynamic(&buf, DLT_USER_RINGBUFFER_MIN_SIZE,
                                      ...
                                      DLT_USER_RINGBUFFER_STEP_SIZE));
    EXPECT_NO_THROW(
        dlt_buffer_write_block(&buf, NULL, (unsigned char *)&data, 0));
    EXPECT_LE(DLT_RETURN_OK, dlt_buffer_free_dynamic(&buf));
    EXPECT_LE(DLT_RETURN_OK,
              dlt_buffer_init_dynamic(&buf, DLT_USER_RINGBUFFER_MIN_SIZE,
                                      DLT_USER_RINGBUFFER_MAX_SIZE,
                                      DLT_USER_RINGBUFFER_STEP_SIZE));
    EXPECT_NO_THROW(
        dlt_buffer_write_block(&buf, NULL, (unsigned char *)&data, test1));
    EXPECT_LE(DLT_RETURN_OK, dlt_buffer_free_dynamic(&buf));
    EXPECT_LE(DLT_RETURN_OK,
              dlt_buffer_init_dynamic(&buf, DLT_USER_RINGBUFFER_MIN_SIZE,
                                      DLT_USER_RINGBUFFER_MAX_SIZE,
...
TEST(t_dlt_buffer_read_block, nullpointer)
{
    ...
    EXPECT_NO_THROW(
        dlt_buffer_read_block(NULL, NULL, (unsigned char *)&data, 0));
    EXPECT_NO_THROW(
        dlt_buffer_read_block(NULL, NULL, (unsigned char *)&data, test1));
    EXPECT_NO_THROW(dlt_buffer_read_block(NULL, &read, NULL, 0));
    EXPECT_NO_THROW(dlt_buffer_read_block(NULL, &read, NULL, test1));
    EXPECT_NO_THROW(
        dlt_buffer_read_block(NULL, &read, (unsigned char *)&data, 0));
    EXPECT_NO_THROW(
        dlt_buffer_read_block(NULL, &read, (unsigned char *)&data, test1));
    EXPECT_LE(DLT_RETURN_OK,
              dlt_buffer_init_dynamic(&buf, DLT_USER_RINGBUFFER_MIN_SIZE,
                                      DLT_USER_RINGBUFFER_MAX_SIZE,
                                      DLT_USER_RINGBUFFER_STEP_SIZE));
    ...
    EXPECT_LE(DLT_RETURN_OK,
              dlt_buffer_init_dynamic(&buf, DLT_USER_RINGBUFFER_MIN_SIZE,
                                      ...
                                      DLT_USER_RINGBUFFER_STEP_SIZE));
    EXPECT_NO_THROW(
        dlt_buffer_read_block(&buf, NULL, (unsigned char *)&data, 0));
    EXPECT_LE(DLT_RETURN_OK, dlt_buffer_free_dynamic(&buf));
    EXPECT_LE(DLT_RETURN_OK,
              dlt_buffer_init_dynamic(&buf, DLT_USER_RINGBUFFER_MIN_SIZE,
                                      DLT_USER_RINGBUFFER_MAX_SIZE,
                                      DLT_USER_RINGBUFFER_STEP_SIZE));
    EXPECT_NO_THROW(
        dlt_buffer_read_block(&buf, NULL, (unsigned char *)&data, test1));
    EXPECT_LE(DLT_RETURN_OK, dlt_buffer_free_dynamic(&buf));
    EXPECT_LE(DLT_RETURN_OK,
              dlt_buffer_init_dynamic(&buf, DLT_USER_RINGBUFFER_MIN_SIZE,
                                      DLT_USER_RINGBUFFER_MAX_SIZE,
    ...
    EXPECT_LE(DLT_RETURN_OK,
              dlt_buffer_init_dynamic(&buf, DLT_USER_RINGBUFFER_MIN_SIZE,
                                      ...
                                      DLT_USER_RINGBUFFER_STEP_SIZE));
    EXPECT_NO_THROW(
        dlt_buffer_read_block(&buf, &read, (unsigned char *)&data, 0));
    EXPECT_LE(DLT_RETURN_OK, dlt_buffer_free_dynamic(&buf));
}
...
TEST(t_dlt_file_open, abnormal)
{
    ...
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_file_open(&file, "This Path doesn't
     * exist!!", 0)); */
}
...
TEST(t_dlt_file_quick_parsing, normal)
{
    ...
    EXPECT_LE(DLT_RETURN_OK, dlt_file_open(&file, openfile, 0));
    EXPECT_LE(DLT_RETURN_OK,
              dlt_file_quick_parsing(&file, output, DLT_OUTPUT_ASCII, 0));
    EXPECT_LE(DLT_RETURN_OK, dlt_file_free(&file, 0));
    unlink(output);
}
...
TEST(t_dlt_file_quick_parsing, abnormal)
{
    ...
    EXPECT_LE(DLT_RETURN_OK, dlt_file_init(&file, 0));
    EXPECT_LE(DLT_RETURN_OK, dlt_file_open(&file, openfile, 0));
    EXPECT_GE(DLT_RETURN_WRONG_PARAMETER,
              dlt_file_quick_parsing(&file, NULL, DLT_OUTPUT_ASCII, 0));
    EXPECT_LE(DLT_RETURN_OK, dlt_file_free(&file, 0));

    EXPECT_LE(DLT_RETURN_OK, dlt_file_init(&file, 0));
    EXPECT_LE(DLT_RETURN_OK, dlt_file_open(&file, openfile, 0));
    EXPECT_GE(DLT_RETURN_WRONG_PARAMETER,
              dlt_file_quick_parsing(NULL, output, DLT_OUTPUT_ASCII, 0));
    EXPECT_LE(DLT_RETURN_OK, dlt_file_free(&file, 0));
}
...
TEST(t_dlt_message_print_ascii, normal)
{
    ...
    for (int i = 0; i < file.counter; i++) {
        ...
        EXPECT_LE(DLT_RETURN_OK, dlt_message_print_ascii(
                                     &file.msg, text, DLT_DAEMON_TEXTSIZE, 0));
    }

    for (int i = 0; i < file.counter; i++) {
        EXPECT_LE(DLT_RETURN_OK, dlt_file_message(&file, i, 0));
        EXPECT_LE(DLT_RETURN_OK, dlt_message_print_ascii(
                                     &file.msg, text, DLT_DAEMON_TEXTSIZE, 1));
    }
...
TEST(t_dlt_message_print_ascii, abnormal)
{
    ...
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_print_ascii(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, 0)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_print_ascii(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, 1)); */

    /* Set verbose to 12345678 */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_print_ascii(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, 12345678)); */
    ...
    /*    { */
    /*        EXPECT_LE(DLT_RETURN_OK, dlt_file_message(&file, i, 0)); */
    /*        EXPECT_GE(DLT_RETURN_ERROR, dlt_message_print_ascii(&file.msg,
     * text, DLT_DAEMON_TEXTSIZE, 12345678)); */
    /*    } */
    /*    EXPECT_LE(DLT_RETURN_OK, dlt_file_free(&file, 0)); */
}
TEST(t_dlt_message_print_ascii, nullpointer)
{
    ...
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_print_ascii(NULL, NULL, DLT_DAEMON_TEXTSIZE, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_print_ascii(NULL, NULL, DLT_DAEMON_TEXTSIZE, 1));
    ...
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_print_ascii(NULL, text, DLT_DAEMON_TEXTSIZE, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_print_ascii(NULL, text, DLT_DAEMON_TEXTSIZE, 1));
    ...
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_print_ascii(&file.msg, NULL, DLT_DAEMON_TEXTSIZE, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_print_ascii(&file.msg, NULL, DLT_DAEMON_TEXTSIZE, 1));
}
...
TEST(t_dlt_message_print_header, normal)
{
    ...
    for (int i = 0; i < file.counter; i++) {
        EXPECT_LE(DLT_RETURN_OK, dlt_file_message(&file, i, 0));
        EXPECT_LE(DLT_RETURN_OK, dlt_message_print_header(
                                     &file.msg, text, DLT_DAEMON_TEXTSIZE, 0));
    }
...
TEST(t_dlt_message_print_header, abnormal)
{
    ...
    /* No messages read, expected -1 */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_print_header(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, 0)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_print_header(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, 1)); */
    ...
    /*    { */
    /*        EXPECT_LE(DLT_RETURN_OK, dlt_file_message(&file, i, 0)); */
    /*        EXPECT_GE(DLT_RETURN_ERROR, dlt_message_print_header(&file.msg,
     * text, DLT_DAEMON_TEXTSIZE, 12345678)); */
    /*    } */
    /*    EXPECT_LE(DLT_RETURN_OK, dlt_file_free(&file, 0)); */
}
TEST(t_dlt_message_print_header, nullpointer)
{
    ...
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_print_header(NULL, NULL, DLT_DAEMON_TEXTSIZE, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_print_header(NULL, NULL, DLT_DAEMON_TEXTSIZE, 1));
    ...
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_print_header(NULL, text, DLT_DAEMON_TEXTSIZE, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_print_header(NULL, text, DLT_DAEMON_TEXTSIZE, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_print_header(&file.msg, NULL, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_print_header(&file.msg, NULL, 0, 1));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_print_header(
                                    &file.msg, NULL, DLT_DAEMON_TEXTSIZE, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_print_header(
                                    &file.msg, NULL, DLT_DAEMON_TEXTSIZE, 1));
}
...
TEST(t_dlt_message_print_hex, normal)
{
    ...
    for (int i = 0; i < file.counter; i++) {
        EXPECT_LE(DLT_RETURN_OK, dlt_file_message(&file, i, 0));
        EXPECT_LE(DLT_RETURN_OK, dlt_message_print_hex(&file.msg, text,
                                                       DLT_DAEMON_TEXTSIZE, 0));
    }
...
TEST(t_dlt_message_print_hex, abnormal)
{
    ...
    /* No messages read, expected -1 */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_print_hex(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, 0)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_print_hex(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, 1)); */
...
TEST(t_dlt_message_print_hex, nullpointer)
{
    ...
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_print_hex(NULL, NULL, DLT_DAEMON_TEXTSIZE, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_print_hex(NULL, NULL, DLT_DAEMON_TEXTSIZE, 1));
    ...
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_print_hex(NULL, text, DLT_DAEMON_TEXTSIZE, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_print_hex(NULL, text, DLT_DAEMON_TEXTSIZE, 1));
    ...
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_print_hex(&file.msg, NULL, DLT_DAEMON_TEXTSIZE, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_print_hex(&file.msg, NULL, DLT_DAEMON_TEXTSIZE, 1));
}
...
TEST(t_dlt_message_print_mixed_plain, normal)
{
    ...
    for (int i = 0; i < file.counter; i++) {
        EXPECT_LE(DLT_RETURN_OK, dlt_file_message(&file, i, 0));
        EXPECT_LE(DLT_RETURN_OK, dlt_message_print_mixed_plain(
                                     &file.msg, text, DLT_DAEMON_TEXTSIZE, 0));
    }
...
TEST(t_dlt_message_print_mixed_plain, abnormal)
{
    ...
    /* No messages read, expected -1 */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_print_mixed_plain(&file.msg,
     * text, DLT_DAEMON_TEXTSIZE, 0)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_print_mixed_plain(&file.msg,
     * text, DLT_DAEMON_TEXTSIZE, 1)); */
    ...
    /*        EXPECT_LE(DLT_RETURN_OK, dlt_file_message(&file, i, 0)); */
    /*        EXPECT_GE(DLT_RETURN_ERROR,
     * dlt_message_print_mixed_plain(&file.msg, text, DLT_DAEMON_TEXTSIZE,
     * 12345678)); */
    /*    } */
    /*    EXPECT_LE(DLT_RETURN_OK, dlt_file_free(&file, 0)); */
}
TEST(t_dlt_message_print_mixed_plain, nullpointer)
{
    ...
    /* NULL-Pointer, expected -1 */
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_print_mixed_plain(NULL, NULL, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_print_mixed_plain(NULL, NULL, 0, 1));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_print_mixed_plain(
                                    NULL, NULL, DLT_DAEMON_TEXTSIZE, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_print_mixed_plain(
                                    NULL, NULL, DLT_DAEMON_TEXTSIZE, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_print_mixed_plain(NULL, text, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_print_mixed_plain(NULL, text, 0, 1));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_print_mixed_plain(
                                    NULL, text, DLT_DAEMON_TEXTSIZE, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_print_mixed_plain(
                                    NULL, text, DLT_DAEMON_TEXTSIZE, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_print_mixed_plain(&file.msg, NULL, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_print_mixed_plain(&file.msg, NULL, 0, 1));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_print_mixed_plain(
                                    &file.msg, NULL, DLT_DAEMON_TEXTSIZE, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_print_mixed_plain(
                                    &file.msg, NULL, DLT_DAEMON_TEXTSIZE, 1));
...
TEST(t_dlt_message_print_mixed_html, normal)
{
    ...
    for (int i = 0; i < file.counter; i++) {
        EXPECT_LE(DLT_RETURN_OK, dlt_file_message(&file, i, 0));
        EXPECT_LE(DLT_RETURN_OK, dlt_message_print_mixed_html(
                                     &file.msg, text, DLT_DAEMON_TEXTSIZE, 0));
    }
...
TEST(t_dlt_message_print_mixed_html, abnormal)
{
    ...
    /* No messages read, expected -1 */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_print_mixed_html(&file.msg,
     * text, DLT_DAEMON_TEXTSIZE, 0)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_print_mixed_html(&file.msg,
     * text, DLT_DAEMON_TEXTSIZE, 1)); */
    ...
    /*        EXPECT_LE(DLT_RETURN_OK, dlt_file_message(&file, i, 0)); */
    /*        EXPECT_GE(DLT_RETURN_ERROR,
     * dlt_message_print_mixed_html(&file.msg, text, DLT_DAEMON_TEXTSIZE,
     * 12345678)); */
    /*    } */
    /*    EXPECT_LE(DLT_RETURN_OK, dlt_file_free(&file, 0)); */
}
TEST(t_dlt_message_print_mixed_html, nullpointer)
{
    ...
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_print_mixed_html(NULL, NULL, DLT_DAEMON_TEXTSIZE, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_print_mixed_html(NULL, NULL, DLT_DAEMON_TEXTSIZE, 1));
    ...
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_print_mixed_html(NULL, text, DLT_DAEMON_TEXTSIZE, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_print_mixed_html(NULL, text, DLT_DAEMON_TEXTSIZE, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_print_mixed_html(&file.msg, NULL, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_print_mixed_html(&file.msg, NULL, 0, 1));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_print_mixed_html(
                                    &file.msg, NULL, DLT_DAEMON_TEXTSIZE, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_print_mixed_html(
                                    &file.msg, NULL, DLT_DAEMON_TEXTSIZE, 1));
}
...
TEST(t_dlt_message_filter_check, normal)
{
    ...
    for (int i = 0; i < file.counter; i++) {
        EXPECT_LE(DLT_RETURN_OK, dlt_file_message(&file, i, 0));
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_filter_check(&file.msg, &filter, 0));
    }

    for (int i = 0; i < file.counter; i++) {
        EXPECT_LE(DLT_RETURN_OK, dlt_file_message(&file, i, 0));
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_filter_check(&file.msg, &filter, 1));
    }
...
TEST(t_dlt_message_filter_check, abnormal)
{
    ...
    /* No messages read, expected -1 */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_filter_check(&file.msg,
     * &filter, 0)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_filter_check(&file.msg,
     * &filter, 1)); */
...
TEST(t_dlt_message_get_extraparamters, abnormal)
{
    ...
    /* Uninizialised, expected -1 */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_get_extraparameters(&file.msg,
     * 0)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_get_extraparameters(&file.msg,
     * 1)); */
    ...
    /*        EXPECT_LE(DLT_RETURN_OK, dlt_file_message(&file, i, 0)); */
    /*        EXPECT_GE(DLT_RETURN_ERROR,
     * dlt_message_get_extraparameters(&file.msg, 12345678)); */
    /*    } */
    /*    EXPECT_LE(DLT_RETURN_OK, dlt_file_free(&file, 0)); */
}
...
TEST(t_dlt_message_header, normal)
{
    ...
    for (int i = 0; i < file.counter; i++) {
        EXPECT_LE(DLT_RETURN_OK, dlt_file_message(&file, i, 0));
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_header(&file.msg, text, DLT_DAEMON_TEXTSIZE, 0));
        printf("%s \n", text);
    }
    ...
    for (int i = 0; i < file.counter; i++) {
        EXPECT_LE(DLT_RETURN_OK, dlt_file_message(&file, i, 0));
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_header(&file.msg, text, DLT_DAEMON_TEXTSIZE, 1));
        printf("%s \n", text);
    }
...
TEST(t_dlt_message_header, abnormal)
{
    ...
    /* Uninizialised, expected -1 */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, 0)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, 1)); */
...
TEST(t_dlt_message_header, nullpointer)
{
    ...
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header(NULL, NULL, DLT_DAEMON_TEXTSIZE, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header(NULL, NULL, DLT_DAEMON_TEXTSIZE, 1));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header(NULL, text, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header(NULL, text, 0, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header(NULL, text, DLT_DAEMON_TEXTSIZE, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header(NULL, text, DLT_DAEMON_TEXTSIZE, 1));
    ...
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header(&file.msg, NULL, DLT_DAEMON_TEXTSIZE, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header(&file.msg, NULL, DLT_DAEMON_TEXTSIZE, 1));
...
TEST(t_dlt_message_header_flags, normal)
{
    ...
    for (int i = 0; i < file.counter; i++) {
        EXPECT_LE(DLT_RETURN_OK, dlt_file_message(&file, i, 0));
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_header_flags(&file.msg, text, DLT_DAEMON_TEXTSIZE,
                                           DLT_HEADER_SHOW_NONE, 0));
        printf("%s \n", text);
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_header_flags(&file.msg, text, DLT_DAEMON_TEXTSIZE,
                                           DLT_HEADER_SHOW_TIME, 0));
        printf("%s \n", text);
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_header_flags(&file.msg, text, DLT_DAEMON_TEXTSIZE,
                                           DLT_HEADER_SHOW_TMSTP, 0));
        printf("%s \n", text);
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_header_flags(&file.msg, text, DLT_DAEMON_TEXTSIZE,
                                           DLT_HEADER_SHOW_MSGCNT, 0));
        printf("%s \n", text);
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_header_flags(&file.msg, text, DLT_DAEMON_TEXTSIZE,
                                           DLT_HEADER_SHOW_ECUID, 0));
        printf("%s \n", text);
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_header_flags(&file.msg, text, DLT_DAEMON_TEXTSIZE,
                                           DLT_HEADER_SHOW_APID, 0));
        printf("%s \n", text);
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_header_flags(&file.msg, text, DLT_DAEMON_TEXTSIZE,
                                           DLT_HEADER_SHOW_CTID, 0));
        printf("%s \n", text);
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_header_flags(&file.msg, text, DLT_DAEMON_TEXTSIZE,
                                           DLT_HEADER_SHOW_MSGTYPE, 0));
        printf("%s \n", text);
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_header_flags(&file.msg, text, DLT_DAEMON_TEXTSIZE,
                                           DLT_HEADER_SHOW_MSGSUBTYPE, 0));
        printf("%s \n", text);
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_header_flags(&file.msg, text, DLT_DAEMON_TEXTSIZE,
                                           DLT_HEADER_SHOW_VNVSTATUS, 0));
        printf("%s \n", text);
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_header_flags(&file.msg, text, DLT_DAEMON_TEXTSIZE,
                                           DLT_HEADER_SHOW_NOARG, 0));
        printf("%s \n", text);
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_header_flags(&file.msg, text, DLT_DAEMON_TEXTSIZE,
                                           DLT_HEADER_SHOW_ALL, 0));
        printf("%s \n", text);
    }
    ...
    for (int i = 0; i < file.counter; i++) {
        ...
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_header_flags(&file.msg, text, DLT_DAEMON_TEXTSIZE,
                                           DLT_HEADER_SHOW_NONE, 1));
        printf("%s \n", text);
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_header_flags(&file.msg, text, DLT_DAEMON_TEXTSIZE,
                                           DLT_HEADER_SHOW_TIME, 1));
        printf("%s \n", text);
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_header_flags(&file.msg, text, DLT_DAEMON_TEXTSIZE,
                                           DLT_HEADER_SHOW_TMSTP, 1));
        printf("%s \n", text);
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_header_flags(&file.msg, text, DLT_DAEMON_TEXTSIZE,
                                           DLT_HEADER_SHOW_MSGCNT, 1));
        printf("%s \n", text);
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_header_flags(&file.msg, text, DLT_DAEMON_TEXTSIZE,
                                           DLT_HEADER_SHOW_ECUID, 1));
        printf("%s \n", text);
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_header_flags(&file.msg, text, DLT_DAEMON_TEXTSIZE,
                                           DLT_HEADER_SHOW_APID, 1));
        printf("%s \n", text);
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_header_flags(&file.msg, text, DLT_DAEMON_TEXTSIZE,
                                           DLT_HEADER_SHOW_CTID, 1));
        printf("%s \n", text);
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_header_flags(&file.msg, text, DLT_DAEMON_TEXTSIZE,
                                           DLT_HEADER_SHOW_MSGTYPE, 1));
        printf("%s \n", text);
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_header_flags(&file.msg, text, DLT_DAEMON_TEXTSIZE,
                                           DLT_HEADER_SHOW_MSGSUBTYPE, 1));
        printf("%s \n", text);
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_header_flags(&file.msg, text, DLT_DAEMON_TEXTSIZE,
                                           DLT_HEADER_SHOW_VNVSTATUS, 1));
        printf("%s \n", text);
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_header_flags(&file.msg, text, DLT_DAEMON_TEXTSIZE,
                                           DLT_HEADER_SHOW_NOARG, 1));
        printf("%s \n", text);
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_header_flags(&file.msg, text, DLT_DAEMON_TEXTSIZE,
                                           DLT_HEADER_SHOW_ALL, 1));
        printf("%s \n", text);
    }
...
TEST(t_dlt_message_header_flags, abnormal)
{
    ...
    /* Uninizialised, expected -1 */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, DLT_HEADER_SHOW_NONE, 0)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, DLT_HEADER_SHOW_TIME, 0)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, DLT_HEADER_SHOW_TMSTP, 0)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, DLT_HEADER_SHOW_MSGCNT, 0)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, DLT_HEADER_SHOW_ECUID, 0)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, DLT_HEADER_SHOW_APID, 0)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, DLT_HEADER_SHOW_CTID, 0)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, DLT_HEADER_SHOW_MSGTYPE, 0)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, DLT_HEADER_SHOW_MSGSUBTYPE, 0)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, DLT_HEADER_SHOW_VNVSTATUS, 0)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, DLT_HEADER_SHOW_NOARG, 0)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, DLT_HEADER_SHOW_ALL, 0)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, DLT_HEADER_SHOW_NONE, 1)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, DLT_HEADER_SHOW_TIME, 1)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, DLT_HEADER_SHOW_TMSTP, 1)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, DLT_HEADER_SHOW_MSGCNT, 1)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, DLT_HEADER_SHOW_ECUID, 1)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, DLT_HEADER_SHOW_APID, 1)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, DLT_HEADER_SHOW_CTID, 1)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, DLT_HEADER_SHOW_MSGTYPE, 1)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, DLT_HEADER_SHOW_MSGSUBTYPE, 1)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, DLT_HEADER_SHOW_VNVSTATUS, 1)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, DLT_HEADER_SHOW_NOARG, 1)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, DLT_HEADER_SHOW_ALL, 1)); */

    /* USE own DLT_HEADER_SHOW , expected -1 */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, 0x1234, 0)); */
    ...
    /*    { */
    /*        EXPECT_LE(DLT_RETURN_OK, dlt_file_message(&file, i, 0)); */
    /*        EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(&file.msg,
     * text, DLT_DAEMON_TEXTSIZE, 0x1234, 0)); */
    /*        printf("%s \n",text); */
    /*    } */
    /*    EXPECT_LE(DLT_RETURN_OK, dlt_file_free(&file, 0)); */
}
TEST(t_dlt_message_header_flags, nullpointer)
{
    ...
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(NULL, NULL, 0, DLT_HEADER_SHOW_NONE, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(NULL, NULL, 0, DLT_HEADER_SHOW_TIME, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(
                                    NULL, NULL, 0, DLT_HEADER_SHOW_TMSTP, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(
                                    NULL, NULL, 0, DLT_HEADER_SHOW_MSGCNT, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(
                                    NULL, NULL, 0, DLT_HEADER_SHOW_ECUID, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(NULL, NULL, 0, DLT_HEADER_SHOW_APID, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(NULL, NULL, 0, DLT_HEADER_SHOW_CTID, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(
                                    NULL, NULL, 0, DLT_HEADER_SHOW_MSGTYPE, 0));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(NULL, NULL, 0, DLT_HEADER_SHOW_MSGSUBTYPE, 0));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(NULL, NULL, 0, DLT_HEADER_SHOW_VNVSTATUS, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(
                                    NULL, NULL, 0, DLT_HEADER_SHOW_NOARG, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(NULL, NULL, 0, DLT_HEADER_SHOW_ALL, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(NULL, NULL, 0, DLT_HEADER_SHOW_NONE, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(NULL, NULL, 0, DLT_HEADER_SHOW_TIME, 1));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(
                                    NULL, NULL, 0, DLT_HEADER_SHOW_TMSTP, 1));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(
                                    NULL, NULL, 0, DLT_HEADER_SHOW_MSGCNT, 1));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(
                                    NULL, NULL, 0, DLT_HEADER_SHOW_ECUID, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(NULL, NULL, 0, DLT_HEADER_SHOW_APID, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(NULL, NULL, 0, DLT_HEADER_SHOW_CTID, 1));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(
                                    NULL, NULL, 0, DLT_HEADER_SHOW_MSGTYPE, 1));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(NULL, NULL, 0, DLT_HEADER_SHOW_MSGSUBTYPE, 1));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(NULL, NULL, 0, DLT_HEADER_SHOW_VNVSTATUS, 1));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(
                                    NULL, NULL, 0, DLT_HEADER_SHOW_NOARG, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(NULL, NULL, 0, DLT_HEADER_SHOW_ALL, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(NULL, NULL, DLT_DAEMON_TEXTSIZE, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(NULL, NULL, DLT_DAEMON_TEXTSIZE, 0, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(NULL, NULL, DLT_DAEMON_TEXTSIZE,
                                       DLT_HEADER_SHOW_NONE, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(NULL, NULL, DLT_DAEMON_TEXTSIZE,
                                       DLT_HEADER_SHOW_TIME, 0));
    ...
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(NULL, text, 0, DLT_HEADER_SHOW_NONE, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(NULL, text, 0, DLT_HEADER_SHOW_TIME, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(
                                    NULL, text, 0, DLT_HEADER_SHOW_TMSTP, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(
                                    NULL, text, 0, DLT_HEADER_SHOW_MSGCNT, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(
                                    NULL, text, 0, DLT_HEADER_SHOW_ECUID, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(NULL, text, 0, DLT_HEADER_SHOW_APID, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(NULL, text, 0, DLT_HEADER_SHOW_CTID, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(
                                    NULL, text, 0, DLT_HEADER_SHOW_MSGTYPE, 0));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(NULL, text, 0, DLT_HEADER_SHOW_MSGSUBTYPE, 0));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(NULL, text, 0, DLT_HEADER_SHOW_VNVSTATUS, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(
                                    NULL, text, 0, DLT_HEADER_SHOW_NOARG, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(NULL, text, 0, DLT_HEADER_SHOW_ALL, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(NULL, text, 0, DLT_HEADER_SHOW_NONE, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(NULL, text, 0, DLT_HEADER_SHOW_TIME, 1));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(
                                    NULL, text, 0, DLT_HEADER_SHOW_TMSTP, 1));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(
                                    NULL, text, 0, DLT_HEADER_SHOW_MSGCNT, 1));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(
                                    NULL, text, 0, DLT_HEADER_SHOW_ECUID, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(NULL, text, 0, DLT_HEADER_SHOW_APID, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(NULL, text, 0, DLT_HEADER_SHOW_CTID, 1));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(
                                    NULL, text, 0, DLT_HEADER_SHOW_MSGTYPE, 1));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(NULL, text, 0, DLT_HEADER_SHOW_MSGSUBTYPE, 1));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(NULL, text, 0, DLT_HEADER_SHOW_VNVSTATUS, 1));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_header_flags(
                                    NULL, text, 0, DLT_HEADER_SHOW_NOARG, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(NULL, text, 0, DLT_HEADER_SHOW_ALL, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(NULL, text, DLT_DAEMON_TEXTSIZE, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(NULL, text, DLT_DAEMON_TEXTSIZE, 0, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(NULL, text, DLT_DAEMON_TEXTSIZE,
                                       DLT_HEADER_SHOW_NONE, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(NULL, text, DLT_DAEMON_TEXTSIZE,
                                       DLT_HEADER_SHOW_TIME, 0));
    ...
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(NULL, text, DLT_DAEMON_TEXTSIZE,
                                       DLT_HEADER_SHOW_ALL, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(&file.msg, NULL, 0, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(&file.msg, NULL, 0, 0, 1));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, NULL, 0, DLT_HEADER_SHOW_NONE, 0));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, NULL, 0, DLT_HEADER_SHOW_TIME, 0));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, NULL, 0, DLT_HEADER_SHOW_TMSTP, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(&file.msg, NULL, 0,
                                       DLT_HEADER_SHOW_MSGCNT, 0));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, NULL, 0, DLT_HEADER_SHOW_ECUID, 0));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, NULL, 0, DLT_HEADER_SHOW_APID, 0));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, NULL, 0, DLT_HEADER_SHOW_CTID, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(&file.msg, NULL, 0,
                                       DLT_HEADER_SHOW_MSGTYPE, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
    ...
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, NULL, 0, DLT_HEADER_SHOW_NOARG, 0));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, NULL, 0, DLT_HEADER_SHOW_ALL, 0));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, NULL, 0, DLT_HEADER_SHOW_NONE, 1));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, NULL, 0, DLT_HEADER_SHOW_TIME, 1));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, NULL, 0, DLT_HEADER_SHOW_TMSTP, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(&file.msg, NULL, 0,
                                       DLT_HEADER_SHOW_MSGCNT, 1));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, NULL, 0, DLT_HEADER_SHOW_ECUID, 1));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, NULL, 0, DLT_HEADER_SHOW_APID, 1));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, NULL, 0, DLT_HEADER_SHOW_CTID, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(&file.msg, NULL, 0,
                                       DLT_HEADER_SHOW_MSGTYPE, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
    ...
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, NULL, 0, DLT_HEADER_SHOW_NOARG, 1));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, NULL, 0, DLT_HEADER_SHOW_ALL, 1));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, NULL, DLT_DAEMON_TEXTSIZE, 0, 0));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, NULL, DLT_DAEMON_TEXTSIZE, 0, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(&file.msg, NULL, DLT_DAEMON_TEXTSIZE,
                                       DLT_HEADER_SHOW_NONE, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(&file.msg, NULL, DLT_DAEMON_TEXTSIZE,
                                       DLT_HEADER_SHOW_TIME, 0));
    ...
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(&file.msg, NULL, DLT_DAEMON_TEXTSIZE,
                                       DLT_HEADER_SHOW_ALL, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(&file.msg, text, 0, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(&file.msg, text, 0, 0, 1));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, text, 0, DLT_HEADER_SHOW_NONE, 0));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, text, 0, DLT_HEADER_SHOW_TIME, 0));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, text, 0, DLT_HEADER_SHOW_TMSTP, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(&file.msg, text, 0,
                                       DLT_HEADER_SHOW_MSGCNT, 0));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, text, 0, DLT_HEADER_SHOW_ECUID, 0));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, text, 0, DLT_HEADER_SHOW_APID, 0));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, text, 0, DLT_HEADER_SHOW_CTID, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(&file.msg, text, 0,
                                       DLT_HEADER_SHOW_MSGTYPE, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
    ...
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, text, 0, DLT_HEADER_SHOW_NOARG, 0));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, text, 0, DLT_HEADER_SHOW_ALL, 0));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, text, 0, DLT_HEADER_SHOW_NONE, 1));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, text, 0, DLT_HEADER_SHOW_TIME, 1));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, text, 0, DLT_HEADER_SHOW_TMSTP, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(&file.msg, text, 0,
                                       DLT_HEADER_SHOW_MSGCNT, 1));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, text, 0, DLT_HEADER_SHOW_ECUID, 1));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, text, 0, DLT_HEADER_SHOW_APID, 1));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, text, 0, DLT_HEADER_SHOW_CTID, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_header_flags(&file.msg, text, 0,
                                       DLT_HEADER_SHOW_MSGTYPE, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
    ...
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, text, 0, DLT_HEADER_SHOW_NOARG, 1));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_header_flags(&file.msg, text, 0, DLT_HEADER_SHOW_ALL, 1));
}
...
TEST(t_dlt_message_payload, normal)
{
    ...
    for (int i = 0; i < file.counter; i++) {
        EXPECT_LE(DLT_RETURN_OK, dlt_file_message(&file, i, 0));
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_payload(&file.msg, text, DLT_DAEMON_TEXTSIZE,
                                      DLT_OUTPUT_HEX, 0));
        printf("%s \n", text);
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_payload(&file.msg, text, DLT_DAEMON_TEXTSIZE,
        ...
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_payload(&file.msg, text, DLT_DAEMON_TEXTSIZE,
                                      DLT_OUTPUT_MIXED_FOR_PLAIN, 0));
        printf("%s \n", text);
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_payload(&file.msg, text, DLT_DAEMON_TEXTSIZE,
                                      DLT_OUTPUT_MIXED_FOR_HTML, 0));
        printf("%s \n", text);
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_payload(&file.msg, text, DLT_DAEMON_TEXTSIZE,
                                      DLT_OUTPUT_ASCII_LIMITED, 0));
        printf("%s \n", text);
    }
    ...
    for (int i = 0; i < file.counter; i++) {
        ...
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_payload(&file.msg, text, DLT_DAEMON_TEXTSIZE,
                                      DLT_OUTPUT_MIXED_FOR_PLAIN, 1));
        printf("%s \n", text);
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_payload(&file.msg, text, DLT_DAEMON_TEXTSIZE,
                                      DLT_OUTPUT_MIXED_FOR_HTML, 1));
        printf("%s \n", text);
        EXPECT_LE(DLT_RETURN_OK,
                  dlt_message_payload(&file.msg, text, DLT_DAEMON_TEXTSIZE,
                                      DLT_OUTPUT_ASCII_LIMITED, 1));
        printf("%s \n", text);
    }
...
TEST(t_dlt_message_payload, abnormal)
{
    ...
    /* USE own DLT_HEADER_SHOW , expected -1 */
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(&file.msg, text, DLT_DAEMON_TEXTSIZE, 99, 0));
...
TEST(t_dlt_message_payload, nullpointer)
{
    ...
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(NULL, NULL, 0, DLT_OUTPUT_HEX, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(NULL, NULL, 0, DLT_OUTPUT_ASCII, 0));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_payload(NULL, NULL, 0, DLT_OUTPUT_MIXED_FOR_PLAIN, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(NULL, NULL, 0, DLT_OUTPUT_MIXED_FOR_HTML, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(NULL, NULL, 0, DLT_OUTPUT_ASCII_LIMITED, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(NULL, NULL, 0, DLT_OUTPUT_HEX, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(NULL, NULL, 0, DLT_OUTPUT_ASCII, 1));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_payload(NULL, NULL, 0, DLT_OUTPUT_MIXED_FOR_PLAIN, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(NULL, NULL, 0, DLT_OUTPUT_MIXED_FOR_HTML, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(NULL, NULL, 0, DLT_OUTPUT_ASCII_LIMITED, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(NULL, NULL, DLT_DAEMON_TEXTSIZE, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(NULL, NULL, DLT_DAEMON_TEXTSIZE, 0, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(NULL, NULL, DLT_DAEMON_TEXTSIZE,
                                  DLT_OUTPUT_HEX, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(NULL, NULL, DLT_DAEMON_TEXTSIZE,
                                  DLT_OUTPUT_ASCII, 0));
    ...
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(NULL, text, 0, DLT_OUTPUT_HEX, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(NULL, text, 0, DLT_OUTPUT_ASCII, 0));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_payload(NULL, text, 0, DLT_OUTPUT_MIXED_FOR_PLAIN, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(NULL, text, 0, DLT_OUTPUT_MIXED_FOR_HTML, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(NULL, text, 0, DLT_OUTPUT_ASCII_LIMITED, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(NULL, text, 0, DLT_OUTPUT_HEX, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(NULL, text, 0, DLT_OUTPUT_ASCII, 1));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_payload(NULL, text, 0, DLT_OUTPUT_MIXED_FOR_PLAIN, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(NULL, text, 0, DLT_OUTPUT_MIXED_FOR_HTML, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(NULL, text, 0, DLT_OUTPUT_ASCII_LIMITED, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(NULL, text, DLT_DAEMON_TEXTSIZE, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(NULL, text, DLT_DAEMON_TEXTSIZE, 0, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(NULL, text, DLT_DAEMON_TEXTSIZE,
                                  DLT_OUTPUT_HEX, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(NULL, text, DLT_DAEMON_TEXTSIZE,
                                  DLT_OUTPUT_ASCII, 0));
    ...
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(&file.msg, NULL, 0, DLT_OUTPUT_HEX, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(&file.msg, NULL, 0, DLT_OUTPUT_ASCII, 0));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_payload(&file.msg, NULL, 0, DLT_OUTPUT_MIXED_FOR_PLAIN, 0));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_payload(&file.msg, NULL, 0, DLT_OUTPUT_MIXED_FOR_HTML, 0));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_payload(&file.msg, NULL, 0, DLT_OUTPUT_ASCII_LIMITED, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(&file.msg, NULL, 0, DLT_OUTPUT_HEX, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(&file.msg, NULL, 0, DLT_OUTPUT_ASCII, 1));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_payload(&file.msg, NULL, 0, DLT_OUTPUT_MIXED_FOR_PLAIN, 1));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_payload(&file.msg, NULL, 0, DLT_OUTPUT_MIXED_FOR_HTML, 1));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_payload(&file.msg, NULL, 0, DLT_OUTPUT_ASCII_LIMITED, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(&file.msg, NULL, DLT_DAEMON_TEXTSIZE, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(&file.msg, NULL, DLT_DAEMON_TEXTSIZE, 0, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(&file.msg, NULL, DLT_DAEMON_TEXTSIZE,
                                  DLT_OUTPUT_HEX, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(&file.msg, NULL, DLT_DAEMON_TEXTSIZE,
                                  DLT_OUTPUT_ASCII, 0));
    ...
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(&file.msg, text, 0, DLT_OUTPUT_HEX, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(&file.msg, text, 0, DLT_OUTPUT_ASCII, 0));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_payload(&file.msg, text, 0, DLT_OUTPUT_MIXED_FOR_PLAIN, 0));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_payload(&file.msg, text, 0, DLT_OUTPUT_MIXED_FOR_HTML, 0));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_payload(&file.msg, text, 0, DLT_OUTPUT_ASCII_LIMITED, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(&file.msg, text, 0, DLT_OUTPUT_HEX, 1));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_payload(&file.msg, text, 0, DLT_OUTPUT_ASCII, 1));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_payload(&file.msg, text, 0, DLT_OUTPUT_MIXED_FOR_PLAIN, 1));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_payload(&file.msg, text, 0, DLT_OUTPUT_MIXED_FOR_HTML, 1));
    EXPECT_GE(
        DLT_RETURN_ERROR,
        dlt_message_payload(&file.msg, text, 0, DLT_OUTPUT_ASCII_LIMITED, 1));

    /* file.msg is not initialised which causes problems when textsize is > 0
     * but */
    /* we don't have text: */
    /* dlt_common.c line 943: ptr = msg->databuffer; */
    /* (gdb) p ptr */
    /*    $28 = (uint8_t *) 0x5124010337d46c00 <error: Cannot access memory at
     * address 0x5124010337d46c00> */

    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_payload(&file.msg, text,
     * DLT_DAEMON_TEXTSIZE, 0, 0)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_payload(&file.msg, text,
...
TEST(t_dlt_message_set_extraparamters, abnormal)
{
    ...
    /* Uninizialised, expected -1 */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_set_extraparameters(&file.msg,
     * 0)); */
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_set_extraparameters(&file.msg,
     * 1)); */
    ...
    /*        EXPECT_LE(DLT_RETURN_OK, dlt_file_message(&file, i, 0)); */
    /*        EXPECT_GE(DLT_RETURN_ERROR,
     * dlt_message_set_extraparameters(&file.msg, 12345678)); */
    /*    } */
    /*    EXPECT_LE(DLT_RETURN_OK, dlt_file_free(&file, 0)); */
}
...
TEST(t_dlt_message_read, normal)
{
    ...
    for (int i = 0; i < file.counter; i++) {
        ...
        EXPECT_LE(
            DLT_RETURN_ERROR,
            dlt_message_read(&file.msg, (unsigned char *)buffer, 255, 0, 1));
    }
    ...
    for (int i = 0; i < file.counter; i++) {
        ...
        EXPECT_LE(
            DLT_RETURN_ERROR,
            dlt_message_read(&file.msg, (unsigned char *)buffer, 255, 1, 1));
    }
...
TEST(t_dlt_message_read, nullpointer)
{
    ...
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_read(NULL, (uint8_t *)&buf, 0, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_read(&file.msg, NULL, 0, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_read(&file.msg, (uint8_t *)&buf, 0, 0, 0));
}
...
TEST(t_dlt_message_argument_print, normal)
{
    ...
    for (int i = 0; i < file.counter; i++) {
        ...
        EXPECT_GE(DLT_RETURN_OK,
                  dlt_message_argument_print(&file.msg, DLT_TYPE_INFO_BOOL,
                                             pptr, pdatalength, text,
                                             DLT_DAEMON_TEXTSIZE, 0, 1));
        /*printf("### ARGUMENT:%s\n", text); */
    }
    ...
    for (int i = 0; i < file.counter; i++) {
        ...
        pdatalength = &datalength;
        EXPECT_GE(DLT_RETURN_OK,
                  dlt_message_argument_print(&file.msg, DLT_TYPE_INFO_RAWD,
                                             pptr, pdatalength, text,
                                             DLT_DAEMON_TEXTSIZE, 0, 1));
        /*printf("### ARGUMENT:%s\n", text); */
    }
...
TEST(t_dlt_message_argument_print, abnormal)
{
    ...
    /* Uninizialised, expected -1 */
    /*    EXPECT_GE(DLT_RETURN_ERROR,
     * dlt_message_argument_print(&file.msg,12345678,pptr,pdatalength,text,DLT_DAEMON_TEXTSIZE,0,1));
     */
...
TEST(t_dlt_message_argument_print, nullpointer)
{
    ...
    /* NULL-Pointer, expected -1 */
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_argument_print(NULL, 0, NULL, NULL, NULL, 0, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_argument_print(NULL, 0, NULL, NULL, text, 0, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_argument_print(
                                    NULL, 0, NULL, pdatalength, NULL, 0, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_argument_print(
                                    NULL, 0, NULL, pdatalength, text, 0, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_argument_print(NULL, 0, pptr, NULL, NULL, 0, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_argument_print(NULL, 0, pptr, NULL, text, 0, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_argument_print(
                                    NULL, 0, pptr, pdatalength, NULL, 0, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_argument_print(
                                    NULL, 0, pptr, pdatalength, text, 0, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_argument_print(
                                    &file.msg, 0, NULL, NULL, NULL, 0, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_argument_print(
                                    &file.msg, 0, NULL, NULL, text, 0, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_argument_print(&file.msg, 0, NULL, pdatalength, NULL,
                                         0, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_argument_print(&file.msg, 0, NULL, pdatalength, text,
                                         0, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_argument_print(
                                    &file.msg, 0, pptr, NULL, NULL, 0, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR, dlt_message_argument_print(
                                    &file.msg, 0, pptr, NULL, text, 0, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_argument_print(&file.msg, 0, pptr, pdatalength, NULL,
                                         0, 0, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_message_argument_print(&file.msg, 0, pptr, pdatalength, text,
                                         0, 0, 0));
}
...
TEST(t_dlt_log, abnormal)
{
    ...
    /*    EXPECT_GE(DLT_RETURN_ERROR, dlt_log(DLT_LOG_DONT_EXISTS, (char *)
     * EXIST)); */
}
...
TEST(t_dlt_uptime, normal) { EXPECT_LE(1, dlt_uptime()); }
...
TEST(t_dlt_print_hex_string, normal)
{
    ...
    EXPECT_LE(DLT_RETURN_OK,
              dlt_print_hex_string(
                  text1, DLT_DAEMON_TEXTSIZE,
                  const_cast<unsigned char *>(
                      reinterpret_cast<const unsigned char *>(test1)),
                  (int)strlen(test1)));
    /*printf("text:%s\n", text1); */
    ...
    EXPECT_LE(DLT_RETURN_OK,
              dlt_print_hex_string(
                  ...
                  const_cast<unsigned char *>(
                      reinterpret_cast<const unsigned char *>(test2)),
                  (int)strlen(test2)));
    /*printf("text:%s\n", text2); */
...
TEST(t_dlt_print_hex_string, abnormal)
{
    ...
    /*    EXPECT_LE(DLT_RETURN_OK,
     * dlt_print_hex_string(text3,DLT_DAEMON_TEXTSIZE,(unsigned char *)test3,
     * strlen(test3))); */
    /*printf("text:%s\n", text3); */
    ...
    /*    EXPECT_LE(DLT_RETURN_OK,
     * dlt_print_hex_string(text4,DLT_DAEMON_TEXTSIZE,(unsigned char *)test4,
     * strlen(test4))); */
    /*printf("text:%s\n", text4); */
...
TEST(t_dlt_print_hex_string, nullpointer)
{
    ...
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_print_hex_string(
                  NULL, 0,
                  const_cast<unsigned char *>(
                      reinterpret_cast<const unsigned char *>(test5)),
                  0));
...
TEST(t_dlt_print_mixed_string, normal)
{
    ...
    EXPECT_LE(DLT_RETURN_OK,
              dlt_print_mixed_string(
                  text3, DLT_DAEMON_TEXTSIZE,
                  const_cast<unsigned char *>(
                      reinterpret_cast<const unsigned char *>(test3)),
                  (int)strlen(test3), 0));
    printf("%s\n", text3);
    ...
    EXPECT_LE(DLT_RETURN_OK,
              dlt_print_mixed_string(
                  text4, DLT_DAEMON_TEXTSIZE,
                  const_cast<unsigned char *>(
                      reinterpret_cast<const unsigned char *>(test4)),
                  (int)strlen(test4), 1));
    printf("%s\n", text4);
}
TEST(t_dlt_print_mixed_string, abnormal)
{
    ...
    EXPECT_LE(DLT_RETURN_OK,
              dlt_print_mixed_string(
                  text6, DLT_DAEMON_TEXTSIZE,
                  const_cast<unsigned char *>(
                      reinterpret_cast<const unsigned char *>(test6)),
                  (int)strlen(test6), 1));
    /*    printf("%s\n", text6); */
    ...
    EXPECT_LE(DLT_RETURN_OK,
              dlt_print_mixed_string(
                  text7, DLT_DAEMON_TEXTSIZE,
                  const_cast<unsigned char *>(
                      reinterpret_cast<const unsigned char *>(test7)),
                  (int)strlen(test7), 0));
    /*    printf("%s\n", text7); */
    ...
    EXPECT_LE(DLT_RETURN_OK,
              dlt_print_mixed_string(
                  text8, DLT_DAEMON_TEXTSIZE,
                  const_cast<unsigned char *>(
                      reinterpret_cast<const unsigned char *>(test8)),
                  (int)strlen(test8), 1));
    /*    printf("%s\n", text8); */
}
TEST(t_dlt_print_mixed_string, nullpointer)
{
    ...
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_print_mixed_string(
                  NULL, 0,
                  const_cast<unsigned char *>(
                      reinterpret_cast<const unsigned char *>(test9)),
                  0, 0));
    EXPECT_GE(DLT_RETURN_ERROR,
              dlt_print_mixed_string(
                  NULL, 0,
...
TEST(t_dlt_print_char_string, abnormal)
{
    ...
    /*    char * ptr3 = text3; */
    /*    EXPECT_LE(DLT_RETURN_OK,
     * dlt_print_char_string(&ptr3,DLT_DAEMON_TEXTSIZE,(unsigned char *)test3,
     * strlen(test3))); */
    /*    printf("text:%s\n", text3); */
    ...
    /*    char * ptr4 = text4; */
    /*    EXPECT_LE(DLT_RETURN_OK,
     * dlt_print_char_string(&ptr4,DLT_DAEMON_TEXTSIZE,(unsigned char *)test4,
     * strlen(test4))); */
    /*    printf("text:%s\n", text4); */
...
{
    char input[] =
        "get_log_info, 07, 02 00 4c 4f 47 00 03 00 54 45 53 54 ff ff 18 00 54 "
        "65 73 74 20 43 6f 6e 74 65 78 74 20 66 6f 72 20 4c 6f 67 67 69 6e 67 "
        "54 53 31 00 ff ff 1b 00 54 65 73 74 20 43 6f 6e 74 65 78 74 31 20 66 "
        "6f 72 20 69 6e 6a 65 63 74 69 6f 6e 54 53 32 00 ff ff 1b 00 54 65 73 "
        "74 20 43 6f 6e 74 65 78 74 32 20 66 6f 72 20 69 6e 6a 65 63 74 69 6f "
        "6e 1c 00 54 65 73 74 20 41 70 70 6c 69 63 61 74 69 6f 6e 20 66 6f 72 "
        "20 4c 6f 67 67 69 6e 67 53 59 53 00 02 00 4a 4f 55 52 ff ff 0f 00 4a "
        "6f 75 72 6e 61 6c 20 41 64 61 70 74 65 72 4d 47 52 00 ff ff 22 00 43 "
        "6f 6e 74 65 78 74 20 6f 66 20 6d 61 69 6e 20 64 6c 74 20 73 79 73 74 "
        "65 6d 20 6d 61 6e 61 67 65 72 12 00 44 4c 54 20 53 79 73 74 65 6d 20 "
        "4d 61 6e 61 67 65 72 72 65 6d 6f";
    /* expected output:
    ...
    DltServiceGetLogInfoResponse *resp = (DltServiceGetLogInfoResponse *)malloc(
    ...
    EXPECT_EQ(0, strcmp("Test Application for Logging",
                        resp->log_info_type.app_ids[0].app_description));
    ...
    EXPECT_EQ(
        0, memcmp("TEST",
                  resp->log_info_type.app_ids[0].context_id_info[0].context_id,
                  4));
    EXPECT_EQ(0,
              strcmp("Test Context for Logging", resp->log_info_type.app_ids[0]
                                                     .context_id_info[0]
                                                     .context_description));
    EXPECT_EQ(24, resp->log_info_type.app_ids[0]
                      .context_id_info[0]
                      .len_context_description);
    EXPECT_EQ(-1, resp->log_info_type.app_ids[0].context_id_info[0].log_level);
    EXPECT_EQ(-1,
              resp->log_info_type.app_ids[0].context_id_info[0].trace_status);

    EXPECT_EQ(
        0, memcmp("TS1",
                  resp->log_info_type.app_ids[0].context_id_info[1].context_id,
                  4));
    EXPECT_EQ(
        0, strcmp("Test Context1 for injection", resp->log_info_type.app_ids[0]
                                                     .context_id_info[1]
                                                     .context_description));
    EXPECT_EQ(27, resp->log_info_type.app_ids[0]
                      .context_id_info[1]
                      .len_context_description);
    EXPECT_EQ(-1, resp->log_info_type.app_ids[0].context_id_info[1].log_level);
    EXPECT_EQ(-1,
              resp->log_info_type.app_ids[0].context_id_info[1].trace_status);

    EXPECT_EQ(
        0, memcmp("TS2",
                  resp->log_info_type.app_ids[0].context_id_info[2].context_id,
                  4));
    EXPECT_EQ(
        0, strcmp("Test Context2 for injection", resp->log_info_type.app_ids[0]
                                                     .context_id_info[2]
                                                     .context_description));
    EXPECT_EQ(27, resp->log_info_type.app_ids[0]
                      .context_id_info[2]
    ...
    EXPECT_EQ(-1,
              resp->log_info_type.app_ids[0].context_id_info[2].trace_status);

    EXPECT_EQ(0, memcmp("SYS", resp->log_info_type.app_ids[1].app_id, 4));
    EXPECT_EQ(0, strcmp("DLT System Manager",
                        resp->log_info_type.app_ids[1].app_description));
    ...
    EXPECT_EQ(
        0, memcmp("JOUR",
                  resp->log_info_type.app_ids[1].context_id_info[0].context_id,
                  4));
    EXPECT_EQ(0, strcmp("Journal Adapter", resp->log_info_type.app_ids[1]
                                               .context_id_info[0]
                                               .context_description));
    EXPECT_EQ(15, resp->log_info_type.app_ids[1]
                      .context_id_info[0]
                      .len_context_description);
    EXPECT_EQ(-1, resp->log_info_type.app_ids[1].context_id_info[0].log_level);
    EXPECT_EQ(-1,
              resp->log_info_type.app_ids[1].context_id_info[0].trace_status);

    EXPECT_EQ(
        0, memcmp("MGR",
                  resp->log_info_type.app_ids[1].context_id_info[1].context_id,
                  4));
    EXPECT_EQ(0, strcmp("Context of main dlt system manager",
                        resp->log_info_type.app_ids[1]
                            .context_id_info[1]
                            .context_description));
    EXPECT_EQ(34, resp->log_info_type.app_ids[1]
                      .context_id_info[1]
                      .len_context_description);
    EXPECT_EQ(-1, resp->log_info_type.app_ids[1].context_id_info[1].log_level);
    EXPECT_EQ(-1,
              resp->log_info_type.app_ids[1].context_id_info[1].trace_status);
...
TEST(dlt_getloginfo_conv_ascii_to_string, normal)
{
    ...
    int rp_count = 0;
    char rp_str[] = {0x72, 0x65, 0x6d, 0x6f, 0x00};
    char wp[5];

...
DltReturnValue dlt_shm_init_server(DltShm* buf, const char* name, int size)
{
    ...
    ptr = (unsigned char*)mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED, buf->shmfd, 0);
    if (ptr == MAP_FAILED) {
...
DltReturnValue dlt_shm_init_client(DltShm* buf, const char* name)
{
    ...
    /* Now we attach the segment to our data space. */
    ptr = (unsigned char*)mmap(NULL, shm_buf.st_size, PROT_READ | PROT_WRITE, MAP_SHARED, buf->shmfd, 0);
    if (ptr == MAP_FAILED) {
        dlt_vlog(LOG_ERR, "%s: mmap() failed: %s\n", __func__, strerror(errno));
        return DLT_RETURN_ERROR; /* ERROR */
    }

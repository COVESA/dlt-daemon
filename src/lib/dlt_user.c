...
DltReturnValue dlt_init_common(void)
{
    ...
    dlt_set_id_v2(&dlt_user.ecuID2, DLT_USER_DEFAULT_ECU_ID, (int8_t)dlt_user.ecuID2len);
...
DltReturnValue dlt_register_app_v2(const char *apid, const char *description)
{
    ...
    int8_t apidlen = (int8_t)apidlen_sz;
    ...
    dlt_user.appID2len = (uint8_t)apidlen;
    if (dlt_user.application_description != NULL)
        free(dlt_user.application_description);
                                                                                       ...
                                                                                       uint8_t trace_status))
{
    ...
    int8_t contextidlen = (int8_t)contextidlen_sz;
...
DltReturnValue dlt_with_tags(const char *firstTag, ...) {
    ...
    va_start(args, firstTag);
    while (currentTag != NULL) {
        tag[i].taglen = (uint8_t)strlen(currentTag);
        tag[i].tagname = (char *)malloc((size_t)(tag[i].taglen + 1));
        if (tag[i].tagname != NULL) {
            memcpy(tag[i].tagname, currentTag, tag[i].taglen);
            tag[i].tagname[tag[i].taglen] = '\0';
        }
...
*/
    ...
    if (DLT_IS_HTYP2_WEID(msg.baseheaderv2->htyp2)) {
        ...
        char *ecid = msg.extendedheaderv2.ecid;
        dlt_set_id_v2(&ecid, dlt_user.ecuID2, (int8_t)msg.extendedheaderv2.ecidlen);
        memcpy(msg.extendedheaderv2.ecid, ecid, msg.extendedheaderv2.ecidlen);
    }

    if (DLT_IS_HTYP2_WACID(msg.baseheaderv2->htyp2)) {
        msg.extendedheaderv2.apidlen = dlt_user.appID2len;
        char *apid = msg.extendedheaderv2.apid;
        dlt_set_id_v2(&apid, dlt_user.appID2, (int8_t)msg.extendedheaderv2.apidlen);
        memcpy(msg.extendedheaderv2.apid, apid, msg.extendedheaderv2.apidlen);
        msg.extendedheaderv2.ctidlen = log->handle->contextID2len;
        char *ctid = msg.extendedheaderv2.ctid;
        dlt_set_id_v2(&ctid, log->handle->contextID2, (int8_t)msg.extendedheaderv2.ctidlen);
        memcpy(msg.extendedheaderv2.ctid, ctid, msg.extendedheaderv2.ctidlen);
    }
    ...
    if (DLT_IS_HTYP2_WSFLN(msg.baseheaderv2->htyp2)) {
        ...
        char *fina = msg.extendedheaderv2.fina;
        dlt_set_id_v2(&fina, dlt_user.filename, (int8_t)msg.extendedheaderv2.finalen);
        memcpy(msg.extendedheaderv2.fina, fina, msg.extendedheaderv2.finalen);
...
DltReturnValue dlt_user_log_send_register_application_v2(void)
{
    ...
    /* set usercontext */
    char apid_buf[DLT_ID_SIZE];
    dlt_set_id_v2((char **)&apid_buf, dlt_user.appID2, (int8_t)dlt_user.appID2len); /* application id */
    usercontext.apid = (char *)apid_buf;
...
DltReturnValue dlt_user_log_send_register_context_v2(DltContextData *log)
{
    ...
    /* set usercontext */
    char apid_buf[DLT_ID_SIZE];
    char ctid_buf[DLT_ID_SIZE];
    dlt_set_id_v2((char **)&apid_buf, dlt_user.appID2, (int8_t)dlt_user.appID2len); /* application id */
    usercontext.apid = (char *)apid_buf;
    usercontext.apidlen = dlt_user.appID2len;
    dlt_set_id_v2((char **)&ctid_buf, log->handle->contextID2, (int8_t)log->handle->contextID2len); /* context id */
    usercontext.ctid = (char *)ctid_buf;
...
DltReturnValue dlt_send_app_ll_ts_limit_v2(const char *apid, DltLogLevelType loglevel, DltTraceStatusType tracestatus)
{
    ...
    char apid_buf[DLT_ID_SIZE];
    dlt_set_id_v2((char **)&apid_buf, apid, (int8_t)usercontext.apidlen); /* application id */
    usercontext.apid = (char *)apid_buf;
    usercontext.log_level = loglevel;
    usercontext.trace_status = tracestatus;
                            ...
                            if (dlt_user.dlt_ll_ts) {
                                ...
                                if (version == DLT_VERSION1) {
                                    ...
                                    dlt_set_id_v2(&(delayed_log_level_changed_callback.contextID2),
                                                  ...
                                                  (int8_t)dlt_user.dlt_ll_ts[usercontextll->log_level_pos].contextID2len);
            ...
            if (size > 0) {
                ...
                if (dlt_user_check_userheader(userheader)) {
                    ...
                    case DLT_USER_MESSAGE_REGISTER_CONTEXT:
                    {
                        ...
                        dlt_set_id_v2((char **)&apid_buf, dlt_user.appID2, (int8_t)dlt_user.appID2len);
                        usercontextv2.apid = (char *)apid_buf;
                    ...
                    case DLT_USER_MESSAGE_LOG:
                    {
                        ...
                        dlt_set_id_v2((char **)&apid_buf, dlt_user.appID2, (int8_t)dlt_user.appID2len);
                        extendedheaderv2.apid = (char *)apid_buf;
                        if (dlt_user.with_ecu_id) {
...
void dlt_user_log_reattach_to_daemon(void)
{
    ...
    if (dlt_user.dlt_log_handle < 0) {
        ...
        for (num = 0; num < dlt_user.dlt_ll_ts_num_entries; num++)
            ...
            /* Register DLTv2 apids and context if present*/
            else if ((dlt_user.appID2len != 0) && (dlt_user.dlt_ll_ts) && (dlt_user.dlt_ll_ts[num].contextID2[0] != 0))
            {
                ...
                dlt_set_id_v2(&(handle.contextID2), dlt_user.dlt_ll_ts[num].contextID2, (int8_t)handle.contextID2len);
                handle.log_level_pos = (int32_t) num;
                log_new.context_description = dlt_user.dlt_ll_ts[num].context_description;

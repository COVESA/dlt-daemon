...
/*******************************************************************************
**                                                                            **
**  SRC-MODULE: dlt-example-user-func-v2.c                                    **
**                                                                            **
**  TARGET    : linux                                                         **
**                                                                            **
**  PROJECT   : DLT                                                           **
...
void usage()
{
    ...
    printf("Usage: dlt-example-user-func-v2 [options] message\n");
...
int main(int argc, char *argv[])
{
    ...
    dlt_with_ecu_id(1);
    dlt_with_filename_and_line_number(FILENAME_BASE, __LINE__);
    dlt_with_prlv(32);
    ...
    dlt_register_context_v2(&mycontext, "TESTER", "Test Context for Logging");

    dlt_register_injection_callback(&mycontext, 0xFFF,
    ...
    if (gflag) {
        ...
        // if (dlt_user_log_write_start_id(&mycontext, &mycontextdata,
        // DLT_LOG_INFO, 10) > 0)
    ...
    for (num = 0; num < maxnum; num++) {
        ...
        if (gflag) {
            ...
            /* Verbose mode */
            if (dlt_user_log_write_start(&mycontext, &mycontextdata,
                                         DLT_LOG_WARN) > 0) {
                dlt_user_log_write_int(&mycontextdata, num);
                dlt_user_log_write_string(&mycontextdata, text);

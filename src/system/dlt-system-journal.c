...
int dlt_system_journal_get(sd_journal *j, char *target, const char *field,
                           size_t max_size)
{
    char *data;
    size_t length;
                                      ...
                                      MessageTimestamp *timestamp)
{
    ...
    /* Try to get realtime from message source and if not successful try to get
     * realtime from journal entry */
    ret = dlt_system_journal_get(journal, buffer_realtime,
    ...
    strftime(buffer_realtime_formatted, sizeof(buffer_realtime_formatted),
             "%Y/%m/%d %H:%M:%S", &timeinfo);

    snprintf(timestamp->real, sizeof(timestamp->real), "%s.%06" PRIu64,
             buffer_realtime_formatted, time_usecs % 1000000);

    /* Try to get monotonic time from message source and if not successful try
     * to get monotonic time from journal entry */
    ret = dlt_system_journal_get(journal, buffer_monotime,
    ...
    else if ((ret = sd_journal_get_monotonic_usec(journal, &time_usecs, NULL)) <
             0) {
        DLT_LOG(dltsystem, DLT_LOG_WARN,
    ...
    snprintf(timestamp->monotonic, sizeof(timestamp->monotonic),
             "%" PRId64 ".%06" PRIu64, time_usecs / 1000000,
             time_usecs % 1000000);
}
...
void get_journal_msg(sd_journal *j, DltSystemConfiguration *config)
{
    ...
    for (;;) {
        ...
        if (r < 0) {
            DLT_LOG(dltsystem, DLT_LOG_ERROR,
                    DLT_STRING("dlt-system-journal failed to get next entry:"),
    ...
    defined(DLT_SYSTEMD_JOURNAL_ENABLE)
        ...
        dlt_system_journal_get(j, buffer_priority, "PRIORITY",
                               sizeof(buffer_priority));
        dlt_system_journal_get(j, buffer_message, "MESSAGE",
                               sizeof(buffer_message));
        dlt_system_journal_get(j, buffer_transport, "_TRANSPORT",
        ...
        if (strcmp(buffer_transport, "kernel") == 0)
            ...
            snprintf(buffer_process, DLT_SYSTEM_JOURNAL_BUFFER_SIZE,
        ...
        if ((systemd_loglevel >= 0) && (systemd_loglevel <= 7))
            snprintf(buffer_priority, DLT_SYSTEM_JOURNAL_BUFFER_SIZE,
        ...
        if (config->Journal.UseUptimeOnly == 1) {
            ...
            DLT_LOG(journalContext, loglevel, DLT_STRING(timestamp.monotonic),
                    DLT_STRING(buffer_process), DLT_STRING(buffer_priority),
                    DLT_STRING(buffer_message));
        }
        else {
            /* write log entry (including timestamp) */
            if (config->Journal.UseOriginalTimestamp == 0) {
                DLT_LOG(journalContext, loglevel, DLT_STRING(timestamp.real),
                        DLT_STRING(timestamp.monotonic),
                        DLT_STRING(buffer_process), DLT_STRING(buffer_priority),
                        DLT_STRING(buffer_message));
            }
            else {
                /* since we are talking about points in time, I'd prefer
                 * truncating over arithmetic rounding */
                ts = (uint32_t)(atof(timestamp.monotonic) * 10000);
                DLT_LOG_TS(
                    journalContext, loglevel, ts, DLT_STRING(timestamp.real),
                    DLT_STRING(buffer_process), DLT_STRING(buffer_priority),
                    DLT_STRING(buffer_message));
            }
        }
...
{
    DLT_REGISTER_CONTEXT(journalContext, config->Journal.ContextId,
                         "Journal Adapter");
    sd_journal *j_tmp;
    ...
    r = sd_journal_open(
        &j_tmp,
        SD_JOURNAL_LOCAL_ONLY /*SD_JOURNAL_LOCAL_ONLY|SD_JOURNAL_RUNTIME_ONLY*/);
    ...
    if (config->Journal.CurrentBoot) {
        ...
        if (r < 0) {
            DLT_LOG(dltsystem, DLT_LOG_ERROR,
                    DLT_STRING("dlt-system-journal failed to get boot id:"),
                    DLT_STRING(strerror(-r)));
        ...
        if (r < 0) {
            DLT_LOG(dltsystem, DLT_LOG_ERROR,
                    DLT_STRING("dlt-system-journal failed to get match:"),
                    DLT_STRING(strerror(-r)));
            sd_journal_close(j_tmp);
            j_tmp = NULL;
        }
    ...
    if (config->Journal.Follow) {
        ...
        if (r < 0) {
            DLT_LOG(dltsystem, DLT_LOG_ERROR,
                    DLT_STRING("dlt-system-journal failed to seek to tail:"),
                    DLT_STRING(strerror(-r)));
            sd_journal_close(j_tmp);
            j_tmp = NULL;
        }
        ...
        if (r < 0) {
            DLT_LOG(dltsystem, DLT_LOG_ERROR,
                    DLT_STRING(
                        "dlt-system-journal failed to seek back 10 entries:"),
                    DLT_STRING(strerror(-r)));
            sd_journal_close(j_tmp);
            j_tmp = NULL;
        }
...
void *journal_thread(void *journalParams)
{
    struct journal_fd_params *params =
        (struct journal_fd_params *)journalParams;

                                           ...
                                           bool is_verbose)
{
    ...
    if (!is_verbose_mode(&dlt_user, log)) {
...
DltReturnValue dlt_user_log_write_constant_string(DltContextData *log, const char *text)
{
    ...
    return is_verbose_mode(&dlt_user, log) ? dlt_user_log_write_string(log, text) : DLT_RETURN_OK;
}

DltReturnValue dlt_user_log_write_constant_string_attr(DltContextData *log, const char *text, const char *name)
{
    /* Send parameter only in verbose mode */
    return is_verbose_mode(&dlt_user, log) ? dlt_user_log_write_string_attr(log, text, name) : DLT_RETURN_OK;
}

DltReturnValue dlt_user_log_write_sized_constant_string(DltContextData *log, const char *text, uint16_t length)
{
    /* Send parameter only in verbose mode */
    return is_verbose_mode(&dlt_user, log) ? dlt_user_log_write_sized_string(log, text, length) : DLT_RETURN_OK;
}
...
{
    /* Send parameter only in verbose mode */
    return is_verbose_mode(&dlt_user, log) ? dlt_user_log_write_sized_string_attr(log, text, length, name) : DLT_RETURN_OK;
}
...
{
    if (!is_verbose_mode(&dlt_user, NULL))
        return DLT_RETURN_ERROR;

...
void dlt_lock_mutex(pthread_mutex_t* mutex)
{
    ...
    if (lock_mutex_result != 0)
        dlt_vlog(LOG_ERR, "Mutex lock failed unexpected pid=%i with result %i!\n", getpid(), lock_mutex_result);
}
...
static DltReturnValue dlt_user_log_send_log_v2(
    DltContextData* log, const int mtype, DltHtyp2ContentType msgcontent, int* const sent_size);
...
static DltReturnValue dlt_user_log_send_register_context_v2(DltContextData* log);
static DltReturnValue dlt_user_log_send_unregister_context(DltContextData* log);
static DltReturnValue dlt_user_log_send_unregister_context_v2(DltContextData* log);
static DltReturnValue dlt_send_app_ll_ts_limit(
    const char* apid, DltLogLevelType loglevel, DltTraceStatusType tracestatus);
static DltReturnValue dlt_send_app_ll_ts_limit_v2(
    const char* apid, DltLogLevelType loglevel, DltTraceStatusType tracestatus);
...
static DltReturnValue dlt_user_log_out_error_handling(
    void* ptr1, size_t len1, void* ptr2, size_t len2, void* ptr3, size_t len3);
...
static DltReturnValue dlt_user_log_write_string_utils_attr(
    DltContextData* log, const char* text, const enum StringType type, const char* name, bool with_var_info);
static DltReturnValue dlt_user_log_write_sized_string_utils_attr(
    DltContextData* log, const char* text, size_t length, const enum StringType type, const char* name,
    bool with_var_info);
...
static int dlt_get_extendedheadersize_v2(DltUser dlt_user_param, int contextIDSize);
...
DltReturnValue dlt_user_check_library_version(const char* user_major_version, const char* user_minor_version)
{
    ...
    if ((strcmp(lib_major_version, user_major_version) != 0) || (strcmp(lib_minor_version, user_minor_version) != 0)) {
        dlt_vnlog(
            ...
            "DLT Library version check failed! Installed DLT library version is %s.%s - Application using DLT library "
            "version %s.%s\n",
...
static DltReturnValue dlt_initialize_socket_connection(void)
{
    ...
    if (connect(sockfd, (struct sockaddr*)&remote, sizeof(remote)) == -1) {
        if (dlt_user.connection_state != DLT_USER_RETRY_CONNECT) {
            dlt_vlog(LOG_INFO, "Socket %s cannot be opened (errno=%d). Retrying later...\n", dltSockBaseDir, errno);
        ...
        if (dlt_receiver_init(&(dlt_user.receiver), sockfd, DLT_RECEIVE_SOCKET, DLT_USER_RCVBUF_MAX_SIZE)
            == DLT_RETURN_ERROR) {
...
static DltReturnValue dlt_initialize_fifo_connection(void)
{
    ...
    if (ret == 0) {
        ...
        ret = chmod(
            dlt_user_dir, S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IWGRP | S_IXGRP | S_IROTH | S_IWOTH | S_IXOTH
                              | S_ISGID | S_ISVTX);
...
DltReturnValue dlt_init(void)
{
    ...
    if (dlt_receiver_init(&(dlt_user.receiver), dlt_user.dlt_user_handle, DLT_RECEIVE_FD, DLT_USER_RCVBUF_MAX_SIZE)
        == DLT_RETURN_ERROR) {
        dlt_user_init_state = INIT_ERROR;
        dlt_free();
        return DLT_RETURN_ERROR;
...
DltReturnValue dlt_init_message_queue(void)
{
    ...
    if ((dlt_user.dlt_segmented_queue_read_handle >= 0) && (dlt_user.dlt_segmented_queue_write_handle >= 0)) {
        /* Already intialized */
    ...
    dlt_user.dlt_segmented_queue_read_handle = mq_open(
        queue_name, O_CREAT | O_RDONLY | O_EXCL, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH, &mqatr);
    ...
    if (dlt_user.dlt_segmented_queue_read_handle < 0) {
        if (errno == EEXIST) {
            ...
            if (mq_unlink(queue_name) < 0)
                ...
                dlt_user.dlt_segmented_queue_read_handle = mq_open(
                    queue_name, O_CREAT | O_RDONLY | O_EXCL, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH,
                    &mqatr);
        }
...
DltReturnValue dlt_init_common(void)
{
     ...
     */
    header_size = sizeof(DltUserHeader) + sizeof(DltStandardHeader) + sizeof(DltStandardHeaderExtra);

    /* Use extended header for non verbose is enabled by default */
    dlt_user.use_extended_header_for_non_verbose = DLT_USER_USE_EXTENDED_HEADER_FOR_NONVERBOSE;

    /* Use extended header for non verbose is modified as per environment variable */
    env_disable_extended_header_for_nonverbose = getenv(DLT_USER_ENV_DISABLE_EXTENDED_HEADER_FOR_NONVERBOSE);

    if (env_disable_extended_header_for_nonverbose) {
        if (strcmp(env_disable_extended_header_for_nonverbose, "1") == 0)
            dlt_user.use_extended_header_for_non_verbose = DLT_USER_NO_USE_EXTENDED_HEADER_FOR_NONVERBOSE;
    }

    if (dlt_user.use_extended_header_for_non_verbose == DLT_USER_USE_EXTENDED_HEADER_FOR_NONVERBOSE)
    ...
    dlt_user.with_filename_and_line_number = DLT_USER_WITH_FILENAME_AND_LINE_NUMBER;
    ...
    if (env_initial_log_level != NULL) {
        if (dlt_env_extract_ll_set(&env_initial_log_level, &dlt_user.initial_ll_set) != 0)
            dlt_vlog(
                LOG_WARNING, "Unable to parse initial set of log-levels from environment! Env:\n%s\n",
    ...
    if (dlt_buffer_init_dynamic(&(dlt_user.startup_buffer), buffer_min, buffer_max, buffer_step) == DLT_RETURN_ERROR) {
        dlt_user_init_state = INIT_UNITIALIZED;
        dlt_mutex_unlock();
        return DLT_RETURN_ERROR;
    }
...
void dlt_user_atexit_handler(void)
{
    ...
    if (!DLT_USER_INITIALIZED) {
        dlt_vlog(
            LOG_WARNING, "%s dlt_user_init_state=%i (expected INIT_DONE), dlt_user_freeing=%i\n", __func__,
            dlt_user_init_state, dlt_user_freeing);
        /* close file */
        dlt_log_free();
        return;
    }
            ...
            if (dlt_user.dlt_log_handle == -1) {
                ...
                if ((dlt_user.dlt_log_handle != -1) && (dlt_user.overflow_counter)) {
                    if (dlt_user_log_send_overflow() == 0) {
                        dlt_vnlog(
                            LOG_WARNING, DLT_USER_BUFFER_LENGTH, "%u messages discarded!\n", dlt_user.overflow_counter);
                        dlt_user.overflow_counter = 0;
                    }
                }
            }
...
DltReturnValue dlt_free(void)
{
    ...
    if (dlt_user.dlt_user_handle != DLT_FD_INIT) {
        ...
        dlt_user.dlt_user_handle = DLT_FD_INIT;
        ret_fifo = snprintf(filename, DLT_PATH_MAX, "%s/dlt%d", dlt_user_dir, getpid());
        ...
        if (ret < 0) {
            ...
            while (1) {
                ...
                if (ret < 0) {
                    ...
                    if (bytes_read >= 0) {
                        dlt_vlog(
                            LOG_DEBUG, "%s - %d: %d bytes read from resend buffer\n", __func__, __LINE__,
                            (int)bytes_read);
                        if (!bytes_read)
                            break;
    ...
    if ((dlt_user.dlt_segmented_queue_write_handle > 0) || (dlt_user.dlt_segmented_queue_read_handle > 0))
...
DltReturnValue dlt_register_app(const char* apid, const char* description)
{
    ...
    if (apid[1] == 0) {
        ...
        if ((apid[0] == dlt_user.appID[0]) && (apid[1] == dlt_user.appID[1]) && (apid[2] == dlt_user.appID[2]))
            return DLT_RETURN_OK;
    } else if (
        (apid[0] == dlt_user.appID[0]) && (apid[1] == dlt_user.appID[1]) && (apid[2] == dlt_user.appID[2])
...
DltReturnValue dlt_register_context(DltContext* handle, const char* contextid, const char* description)
{
    ...
    return dlt_register_context_ll_ts(
        handle, contextid, description, DLT_USER_LOG_LEVEL_NOT_SET, DLT_USER_TRACE_STATUS_NOT_SET);
}
...
DltReturnValue dlt_register_context_v2(DltContext* handle, const char* contextid, const char* description)
{
    ...
    return dlt_register_context_ll_ts_v2(
        handle, contextid, description, DLT_USER_LOG_LEVEL_NOT_SET, DLT_USER_TRACE_STATUS_NOT_SET);
}

DltReturnValue dlt_register_context_ll_ts_llccb(
    DltContext* handle, const char* contextid, const char* description, int loglevel, int tracestatus,
    void (*dlt_log_level_changed_callback)(char context_id[DLT_ID_SIZE], uint8_t log_level, uint8_t trace_status))
{
    ...
    if (dlt_user.dlt_ll_ts == NULL) {
        ...
        dlt_user.dlt_ll_ts_max_num_entries =
            ((dlt_user.dlt_ll_ts_num_entries / DLT_USER_CONTEXT_ALLOC_SIZE) + 1) * DLT_USER_CONTEXT_ALLOC_SIZE;
        dlt_user.dlt_ll_ts = (dlt_ll_ts_type*)malloc(sizeof(dlt_ll_ts_type) * dlt_user.dlt_ll_ts_max_num_entries);
    ...
    envLogLevel =
        dlt_env_adjust_ll_from_env(&dlt_user.initial_ll_set, dlt_user.appID, contextid, DLT_USER_LOG_LEVEL_NOT_SET);
...
DltReturnValue dlt_register_context_ll_ts(
    DltContext* handle, const char* contextid, const char* description, int loglevel, int tracestatus)
{
    return dlt_register_context_ll_ts_llccb(handle, contextid, description, loglevel, tracestatus, NULL);
}
...
DltReturnValue dlt_register_context_ll_ts_llccb_v2(
    DltContext* handle, const char* contextid, const char* description, int loglevel, int tracestatus,
    void (*dlt_log_level_changed_callback_v2)(char* context_id, uint8_t log_level, uint8_t trace_status))
{
    DltContextData log;
    uint32_t i;
    int envLogLevel = DLT_USER_LOG_LEVEL_NOT_SET;
    ...
    envLogLevel = dlt_env_adjust_ll_from_env_v2(
        &dlt_user.initial_ll_set, dlt_user.appID2, dlt_user.appID2len, contextid, contextidlen,
    ...
    *(ctx_entry->log_level_ptr) = ctx_entry->log_level;
    *(ctx_entry->trace_status_ptr) = ctx_entry->trace_status = (int8_t)tracestatus;
    ctx_entry->log_level_changed_callback_v2 = dlt_log_level_changed_callback_v2;
...
DltReturnValue dlt_register_context_ll_ts_v2(
    DltContext* handle, const char* contextid, const char* description, int loglevel, int tracestatus)
{
    return dlt_register_context_ll_ts_llccb_v2(handle, contextid, description, loglevel, tracestatus, NULL);
}
...
{
    size_t contextidlen_sz = strlen(contextid);
    if ((handle == NULL) || (contextid == NULL) || (contextidlen_sz == 0) || (contextidlen_sz > INT8_MAX))
        return DLT_RETURN_WRONG_PARAMETER;
...
DltReturnValue dlt_user_log_write_start_init(
    DltContext* handle, DltContextData* log, DltLogLevelType loglevel, bool is_verbose)
{
...
static DltReturnValue dlt_user_log_write_start_internal(
    DltContext* handle, DltContextData* log, DltLogLevelType loglevel, uint32_t messageid, bool is_verbose);
...
DltReturnValue dlt_user_log_write_start_id(
    DltContext* handle, DltContextData* log, DltLogLevelType loglevel, uint32_t messageid)
{
    return dlt_user_log_write_start_internal(handle, log, loglevel, messageid, false);
}

DltReturnValue dlt_user_log_write_start_internal(
    DltContext* handle, DltContextData* log, DltLogLevelType loglevel, uint32_t messageid, bool is_verbose)
{
    ...
    if (ret == DLT_RETURN_TRUE) {
        ...
        if (log->buffer == NULL) {
            ...
            if (!is_verbose_mode(dlt_user.verbose_mode, log)) {
                ...
                log->msid = messageid;
                /* TODO: For non verbose mode in Version 2, following should be uncommented
                 * and above copying should be commented */
                // log->size = 0;
            }
        }
    }
...
DltReturnValue dlt_user_log_write_start_w_given_buffer(
    DltContext* handle, DltContextData* log, DltLogLevelType loglevel, char* buffer, size_t size, int32_t args_num)
{
    int ret = DLT_RETURN_TRUE;
...
static DltReturnValue dlt_user_log_write_raw_internal(
    DltContextData* log, const void* data, uint16_t length, DltFormatType type, const char* name, bool with_var_info)
{
    /* check nullpointer */
    ...
    const uint16_t name_size = (name != NULL) ? (uint16_t)(strlen(name) + 1) : 0;
...
DltReturnValue dlt_user_log_write_raw_formatted_attr(
    DltContextData* log, const void* data, uint16_t length, DltFormatType type, const char* name)
{
...
static DltReturnValue dlt_user_log_write_generic_attr(
    DltContextData* log, const void* datap, size_t datalen, uint32_t type_info, const VarInfo* varinfo)
{
    if (log == NULL)
        return DLT_RETURN_WRONG_PARAMETER;
    ...
    if (is_verbose_mode(dlt_user.verbose_mode, log)) {
        ...
        needed_size += sizeof(uint32_t);  // Type Info field
        if (with_var_info) {
            name_size = (varinfo->name != NULL) ? (uint16_t)(strlen(varinfo->name) + 1) : 0;
            unit_size = (varinfo->unit != NULL) ? (uint16_t)(strlen(varinfo->unit) + 1) : 0;
...
static DltReturnValue dlt_user_log_write_generic_formatted(
    DltContextData* log, const void* datap, size_t datalen, uint32_t type_info, DltFormatType type)
{
    if (log == NULL)
        return DLT_RETURN_WRONG_PARAMETER;
...
DltReturnValue dlt_user_log_write_ptr(DltContextData* log, void* data)
{
    ...
    switch (sizeof(void*)) {
    case 4:
        return dlt_user_log_write_uint32_formatted(log, (uint32_t)(uintptr_t)data, DLT_FORMAT_HEX32);
        break;
    case 8:
        return dlt_user_log_write_uint64_formatted(log, (uintptr_t)data, DLT_FORMAT_HEX64);
        break;
    default:; /* skip */
    }
...
DltReturnValue dlt_user_log_write_sized_string_attr(
...
{
    /* Send parameter only in verbose mode */
    return is_verbose_mode(dlt_user.verbose_mode, log) ? dlt_user_log_write_string_attr(log, text, name) :
                                                         DLT_RETURN_OK;
}
...
{
    /* Send parameter only in verbose mode */
    return is_verbose_mode(dlt_user.verbose_mode, log) ? dlt_user_log_write_sized_string(log, text, length) :
                                                         DLT_RETURN_OK;
}
...
{
    /* Send parameter only in verbose mode */
    return is_verbose_mode(dlt_user.verbose_mode, log) ? dlt_user_log_write_sized_string_attr(log, text, length, name) :
                                                         DLT_RETURN_OK;
}
...
DltReturnValue dlt_user_log_write_sized_utf8_string_attr(
    DltContextData* log, const char* text, uint16_t length, const char* name)
{
...
{
    /* Send parameter only in verbose mode */
    return is_verbose_mode(dlt_user.verbose_mode, log) ? dlt_user_log_write_utf8_string_attr(log, text, name) :
                                                         DLT_RETURN_OK;
}
...
{
    /* Send parameter only in verbose mode */
    return is_verbose_mode(dlt_user.verbose_mode, log) ? dlt_user_log_write_sized_utf8_string(log, text, length) :
                                                         DLT_RETURN_OK;
}
    ...
    DltContextData* log, const char* text, uint16_t length, const char* name)
{
    ...
    return is_verbose_mode(dlt_user.verbose_mode, log) ?
               dlt_user_log_write_sized_utf8_string_attr(log, text, length, name) :
               DLT_RETURN_OK;
}
    ...
    bool with_var_info)
{
    ...
    if (new_log_size > dlt_user.log_buf_len) {
        ...
        arg_size = (size_t)(dlt_user.log_buf_len - (size_t)log->size - sizeof(uint16_t));

        size_t min_payload_str_truncate_msg = (size_t)log->size + str_truncate_message_length + sizeof(uint16_t);
...
DltReturnValue dlt_register_injection_callback_with_id(
    DltContext* handle, uint32_t service_id, dlt_injection_callback_id dlt_injection_cbk, void* priv)
{
    DltContextData log;
    uint32_t i, j, k;
    int found = 0;
    ...
    if (found) {
        ...
        if (dlt_user.dlt_ll_ts[i].injection_table == NULL) {
            ...
            dlt_user.dlt_ll_ts[i].injection_table =
                (DltUserInjectionCallback*)malloc(sizeof(DltUserInjectionCallback) * (j + 1));
    ...
    if (priv == NULL) {
        /* Use union to convert between function pointer types without violating ISO C */
        dlt_injection_callback_internal callback_internal;
        callback_internal.with_id = dlt_injection_cbk;
        dlt_user.dlt_ll_ts[i].injection_table[j].injection_callback = callback_internal.without_id;
...
DltReturnValue dlt_register_injection_callback(
    ...
    int (*dlt_injection_callback_fn)(uint32_t service_id, void* data, uint32_t length))
{
    ...
    return dlt_register_injection_callback_with_id(handle, service_id, callback_internal.with_id, NULL);
}
...
DltReturnValue dlt_user_trace_network_segmented_start(
    uint32_t* id, DltContext* handle, DltNetworkTraceType nw_trace_type, uint16_t header_len, void* header,
...
DltReturnValue dlt_user_trace_network_segmented_segment(
    uint32_t id, DltContext* handle, DltNetworkTraceType nw_trace_type, int sequence, uint16_t payload_len,
    void* payload)
{
    int ret = DLT_RETURN_ERROR;
    struct timespec ts;
...
void* dlt_user_trace_network_segmented_thread(void* unused)
{
    ...
    while (1) {
        ...
        ssize_t read =
            mq_receive(dlt_user.dlt_segmented_queue_read_handle, (char*)&data, sizeof(s_segmented_data*), NULL);
...
void dlt_user_trace_network_segmented_thread_segmenter(s_segmented_data* data)
{
    ...
    do {
        ...
        DltReturnValue err =
            dlt_user_trace_network_segmented_segment(data->id, data->handle, data->nw_trace_type, sequence++, len, ptr);
...
DltReturnValue dlt_user_trace_network_segmented(
    DltContext* handle, DltNetworkTraceType nw_trace_type, uint16_t header_len, void* header, uint16_t payload_len,
    void* payload)
{
    /* forbid dlt usage in child after fork */
    if (g_dlt_is_child)
    ...
    if ((size_t)header_len + (size_t)payload_len + sizeof(uint16_t) < dlt_user.log_buf_len)
    ...
    DltReturnValue err = dlt_user_trace_network_segmented_start(
        &(thread_data->id), thread_data->handle, thread_data->nw_trace_type, (uint16_t)thread_data->header_len,
        thread_data->header, (uint16_t)thread_data->payload_len);
    ...
    /* Add to queue */
    if (mq_send(dlt_user.dlt_segmented_queue_write_handle, (char*)&thread_data, sizeof(s_segmented_data*), 1) < 0) {
        if (errno == EAGAIN)
...
DltReturnValue dlt_user_trace_network(
    DltContext* handle, DltNetworkTraceType nw_trace_type, uint16_t header_len, void* header, uint16_t payload_len,
    void* payload)
{
...
DltReturnValue dlt_user_trace_network_truncated(
    DltContext* handle, DltNetworkTraceType nw_trace_type, uint16_t header_len, void* header, uint16_t payload_len,
    void* payload, int allow_truncate)
{
    int ret = DLT_RETURN_ERROR;
    DltContextData log = {0};
    ...
    if (handle->trace_status_ptr && (*(handle->trace_status_ptr) == DLT_TRACE_STATUS_ON)) {
        ...
        if ((allow_truncate > 0)
            && ((size_t)header_len + (size_t)payload_len + sizeof(uint16_t) > dlt_user.log_buf_len)) {
            ...
            uint16_t truncated_payload_len =
                (uint16_t)((size_t)dlt_user.log_buf_len - (size_t)log.size - sizeof(uint16_t) - sizeof(uint32_t));
...
void* dlt_user_housekeeperthread_function(void* ptr)
{
    ...
    *dlt_housekeeper_running = true;
    ...
    while (in_loop) {
        ...
        /* Check if shutdown was requested cooperatively */
...
DltReturnValue dlt_user_log_send_log(DltContextData* log, const int mtype, int* const sent_size)
{
    ...
    dlt_mutex_lock();
    if ((log == NULL) || (log->handle == NULL) || (log->handle->contextID[0] == '\0') || (mtype < DLT_TYPE_LOG)
    ...
    if (DLT_IS_HTYP_UEH(msg.standardheader->htyp)) {
        ...
        msg.headersize = (int32_t)(sizeof(DltStorageHeader) + sizeof(DltStandardHeader)
                                   + DLT_STANDARD_HEADER_EXTRA_SIZE(msg.standardheader->htyp));
    }

    int32_t tmplen = (int32_t)msg.headersize - (int32_t)sizeof(DltStorageHeader) + (int32_t)log->size;
    ...
    if ((dlt_user.local_print_mode != DLT_PM_FORCE_OFF) && (dlt_user.local_print_mode != DLT_PM_AUTOMATIC)) {
    ...
    if (dlt_user.dlt_is_file) {
        if (dlt_user_file_reach_max) {
            ...
            if (fstat(dlt_user.dlt_log_handle, &st) != 0) {
                dlt_vlog(LOG_WARNING, "%s: Cannot get file information (errno=%d)\n", __func__, errno);
                dlt_mutex_unlock();
                return DLT_RETURN_ERROR;
            }
            ...
            unsigned int msg_size = (unsigned int)st.st_size + (unsigned int)msg.headersize + (unsigned int)log->size;
            if (msg_size > dlt_user.filesize_max) {
                dlt_user_file_reach_max = true;
                dlt_vlog(
                    LOG_ERR, "%s: File size (%lld bytes) reached to defined maximum size (%d bytes)\n", __func__,
                    (long long)st.st_size, dlt_user.filesize_max);
                dlt_mutex_unlock();
                return DLT_RETURN_FILESZERR;
            } else {
                /* log to file */
                ret = dlt_user_log_out2(
                    dlt_user.dlt_log_handle, msg.headerbuffer, (size_t)msg.headersize, log->buffer, (size_t)log->size);
                dlt_mutex_unlock();
                return ret;
            }
        }
        ...
        if ((ret == DLT_RETURN_OK) && (dlt_user.appID[0] != '\0')) {
            ...
            if (dlt_user.dlt_log_handle != -1)
                dlt_shm_push(
                    &dlt_user.dlt_shm, msg.headerbuffer + sizeof(DltStorageHeader),
                    msg.headersize - sizeof(DltStorageHeader), log->buffer, log->size, 0, 0);

            ret = dlt_user_log_out3(dlt_user.dlt_log_handle, &(userheader), sizeof(DltUserHeader), 0, 0, 0, 0);
#else
#ifdef DLT_TEST_ENABLE
            ...
            if (!sent_size) {
                ...
                if ((uint32_t)pos > dlt_user.dlt_ll_ts_num_entries) {
                    char msg_buffer[255];
                    sprintf(
                        msg_buffer, "log handle has invalid log level pos %d, current entries: %u, dropping message\n",
                ...
                dlt_mutex_unlock();
                size_t trace_load_size = (size_t)sizeof(DltUserHeader) + (size_t)msg.headersize
                                         - (size_t)sizeof(DltStorageHeader) + (size_t)log->size;

                int32_t trace_load_size_i32 = safe_size_to_int32(trace_load_size);
                const bool trace_load_in_limits = dlt_check_trace_load(
                    computed_settings, log->log_level, time_stamp, trace_load_size_i32, dlt_user_output_internal_msg,
                    NULL);
                ...
                {
                    size_t total_size = (size_t)sizeof(DltUserHeader) + (size_t)msg.headersize
                                        - (size_t)sizeof(DltStorageHeader) + (size_t)log->size;
            ...
            ret = dlt_user_log_out3(
                ...
                msg.headerbuffer + sizeof(DltStorageHeader), (size_t)msg.headersize - (size_t)sizeof(DltStorageHeader),
        ...
        if ((ret != DLT_RETURN_OK) || (dlt_user.appID[0] == '\0'))
            ...
            process_error_ret = dlt_user_log_out_error_handling(
                ...
                (size_t)msg.headersize - (size_t)sizeof(DltStorageHeader), log->buffer, (size_t)log->size);
...
DltReturnValue dlt_user_log_send_log_v2(
    DltContextData* log, const int mtype, DltHtyp2ContentType msgcontent, int* const sent_size)
{
    DltMessageV2 msg = {0};
    DltUserHeader userheader;
    ...
    if ((log == NULL) || (log->handle == NULL) || (log->handle->contextID2 == NULL) || (mtype < DLT_TYPE_LOG)
        || (mtype > DLT_TYPE_CONTROL) || (msgcontent < DLT_VERBOSE_DATA_MSG) || (msgcontent > DLT_CONTROL_MSG))
        return DLT_RETURN_WRONG_PARAMETER;

    /* also for Trace messages */
    if (dlt_user_set_userheader_v2(&userheader, DLT_USER_MESSAGE_LOG) < DLT_RETURN_OK)
        return DLT_RETURN_ERROR;

    if (dlt_message_init_v2(&msg, 0) == DLT_RETURN_ERROR)
        return DLT_RETURN_ERROR;
    msg.storageheadersizev2 = STORAGE_HEADER_V2_FIXED_SIZE + (uint32_t)dlt_user.ecuID2len;
    ...
    */
     ...
     * copied into the message header buffer by dlt_message_set_extendedparameters_v2()
     * to avoid leaking the allocated array of DltTag structures. The actual
     * tag name strings are owned by dlt_user.tag and are freed in dlt_free().
     */
    ...
    */
    ...
    int32_t tmplen = (int32_t)msg.headersizev2 - (int32_t)msg.storageheadersizev2 + (int32_t)log->size;
    if (log->size < 0 || tmplen < 0) {
        dlt_log(LOG_WARNING, "Negative message length!\n");
        return DLT_RETURN_ERROR;
    }
    ...
    if (dlt_user.dlt_is_file) {
        if (dlt_user_file_reach_max) {
            ...
            if (st.st_size < 0 || st.st_size > UINT_MAX) {
                dlt_vlog(
                    LOG_ERR, "%s: File size (%lld bytes) is invalid or too large for unsigned int\n", __func__,
            ...
            if (msg_size > dlt_user.filesize_max) {
                ...
                ret = dlt_user_log_out2(
                    dlt_user.dlt_log_handle, msg.headerbufferv2, (size_t)headersizev2, log->buffer, (size_t)logsize);
                return ret;
            }
        }
    } else {
        ...
        if ((ret == DLT_RETURN_OK) && (dlt_user.appID2len != 0)) {
            ...
            if (dlt_user.dlt_log_handle != -1)
                dlt_shm_push(
                    &dlt_user.dlt_shm, msg.headerbufferv2 + msg.storageheadersizev2,
                    msg.headersizev2 - msg.storageheadersizev2, log->buffer, log->size, 0, 0);
            ...
            if (dlt_user.corrupt_message_size)
                msg.baseheaderv2->len = DLT_HTOBE_16(dlt_user.corrupt_message_size_size);
            ...
            if (!sent_size) {
                pthread_rwlock_wrlock(&trace_load_rw_lock);
                DltTraceLoadSettings* settings = dlt_find_runtime_trace_load_settings(
                    trace_load_settings, trace_load_settings_count, dlt_user.appID2, log->handle->contextID2);
                const bool trace_load_in_limits = dlt_check_trace_load(
                    settings, log->log_level, time_stamp,
                    sizeof(DltUserHeader) + msg.headersizev2 - msg.storageheadersizev2 + log->size,
            ...
            int32_t header_size = (int32_t)msg.headersizev2 - (int32_t)msg.storageheadersizev2;
            int32_t logsize = log->size;
            ...
            ret = dlt_user_log_out3(
                dlt_user.dlt_log_handle, &(userheader), sizeof(DltUserHeader),
                msg.headerbufferv2 + msg.storageheadersizev2, (uint32_t)header_size, log->buffer, (size_t)logsize);
...
#endif
        {
            int32_t header_size2 = (int32_t)msg.headersizev2 - (int32_t)msg.storageheadersizev2;
            ...
            process_error_ret = dlt_user_log_out_error_handling(
                &(userheader), sizeof(DltUserHeader), msg.headerbufferv2 + msg.storageheadersizev2,
...
DltReturnValue dlt_user_log_send_register_application(void)
{
    ...
    ret = dlt_user_log_out3(
        dlt_user.dlt_log_handle, &(userheader), sizeof(DltUserHeader), &(usercontext),
        sizeof(DltUserControlMsgRegisterApplication), dlt_user.application_description, usercontext.description_length);

    /* store message in ringbuffer, if an error has occured */
    if (ret < DLT_RETURN_OK)
        return dlt_user_log_out_error_handling(
            &(userheader), sizeof(DltUserHeader), &(usercontext), sizeof(DltUserControlMsgRegisterApplication),
...
DltReturnValue dlt_user_log_send_register_application_v2(void)
{
    ...
    /* set userheader */
    if (dlt_user_set_userheader_v2(&userheader, DLT_USER_MESSAGE_REGISTER_APPLICATION) < DLT_RETURN_OK)
        return DLT_RETURN_ERROR;
    ...
    usercontextSize = (int)sizeof(uint8_t) + usercontext.apidlen + (int)sizeof(pid_t) + (int)sizeof(uint32_t);
    ...
    memcpy((buffer + 1 + usercontext.apidlen + sizeof(pid_t)), &(usercontext.description_length), sizeof(uint32_t));

    ret = dlt_user_log_out3(
        dlt_user.dlt_log_handle, &(userheader), sizeof(DltUserHeader), buffer, (size_t)usercontextSize,
        dlt_user.application_description, usercontext.description_length);

    /* store message in ringbuffer, if an error has occured */
    if (ret < DLT_RETURN_OK)
        ret = dlt_user_log_out_error_handling(
            &(userheader), sizeof(DltUserHeader), buffer, (size_t)usercontextSize, dlt_user.application_description,
...
DltReturnValue dlt_user_log_send_unregister_application(void)
{
    ...
    if (ret < DLT_RETURN_OK)
        return dlt_user_log_out_error_handling(
            &(userheader), sizeof(DltUserHeader), &(usercontext), sizeof(DltUserControlMsgUnregisterApplication), NULL,
            0);
...
DltReturnValue dlt_user_log_send_unregister_application_v2(void)
{
    ...
    /* set userheader */
    if (dlt_user_set_userheader_v2(&userheader, DLT_USER_MESSAGE_UNREGISTER_APPLICATION) < DLT_RETURN_OK)
        return DLT_RETURN_ERROR;
    ...
    usercontextSize = (int)sizeof(uint8_t) + usercontext.apidlen + (int)sizeof(pid_t);
    buffer = (uint8_t*)malloc((size_t)usercontextSize);
    if (!buffer)
        return DLT_RETURN_ERROR;
    ...
    if (ret < DLT_RETURN_OK)
        ret = dlt_user_log_out_error_handling(
            &(userheader), sizeof(DltUserHeader), buffer, (size_t)usercontextSize, NULL, 0);
    free(buffer);
    return ret;
}
...
DltReturnValue dlt_user_log_send_register_context(DltContextData* log)
{
    ...
    if (dlt_user.appID[0] != '\0')
        ret = dlt_user_log_out3(
            dlt_user.dlt_log_handle, &(userheader), sizeof(DltUserHeader), &(usercontext),
            sizeof(DltUserControlMsgRegisterContext), log->context_description, usercontext.description_length);

    /* store message in ringbuffer, if an error has occured */
    if ((ret != DLT_RETURN_OK) || (dlt_user.appID[0] == '\0'))
        return dlt_user_log_out_error_handling(
            &(userheader), sizeof(DltUserHeader), &(usercontext), sizeof(DltUserControlMsgRegisterContext),
...
DltReturnValue dlt_user_log_send_register_context_v2(DltContextData* log)
{
    ...
    usercontextSize = (int)sizeof(uint8_t) + usercontext.apidlen + (int)sizeof(uint8_t) + usercontext.ctidlen + 10
                      + (int)sizeof(pid_t);
    buffer = (uint8_t*)malloc((size_t)usercontextSize);
    if (!buffer)
        return DLT_RETURN_ERROR;
    ...
    if ((ret != DLT_RETURN_OK) || (dlt_user.appID2len == 0))
        ret = dlt_user_log_out_error_handling(
            &(userheader), sizeof(DltUserHeader), buffer, (size_t)usercontextSize, log->context_description,
            usercontext.description_length);
...
DltReturnValue dlt_user_log_send_unregister_context(DltContextData* log)
{
    ...
    if (ret < DLT_RETURN_OK)
        return dlt_user_log_out_error_handling(
            &(userheader), sizeof(DltUserHeader), &(usercontext), sizeof(DltUserControlMsgUnregisterContext), NULL, 0);
...
DltReturnValue dlt_user_log_send_unregister_context_v2(DltContextData* log)
{
    ...
    /* set userheader */
    if (dlt_user_set_userheader_v2(&userheader, DLT_USER_MESSAGE_UNREGISTER_CONTEXT) < DLT_RETURN_OK)
        return DLT_RETURN_ERROR;
    ...
    usercontextSize =
        (int)sizeof(uint8_t) + usercontext.apidlen + (int)sizeof(uint8_t) + usercontext.ctidlen + (int)sizeof(pid_t);
    buffer = (uint8_t*)malloc((size_t)usercontextSize);
    if (!buffer)
        return DLT_RETURN_ERROR;
...
DltReturnValue dlt_send_app_ll_ts_limit(const char* apid, DltLogLevelType loglevel, DltTraceStatusType tracestatus)
{
    ...
    if (ret < DLT_RETURN_OK)
        return dlt_user_log_out_error_handling(
            &(userheader), sizeof(DltUserHeader), &(usercontext), sizeof(DltUserControlMsgAppLogLevelTraceStatus), NULL,
            0);
...
DltReturnValue dlt_send_app_ll_ts_limit_v2(const char* apid, DltLogLevelType loglevel, DltTraceStatusType tracestatus)
{
    ...
    ret = dlt_user_log_out2(dlt_user.dlt_log_handle, &(userheader), sizeof(DltUserHeader), buffer, (size_t)buffersize);

    /* store message in ringbuffer, if an error has occured */
    if (ret < DLT_RETURN_OK)
        return dlt_user_log_out_error_handling(
            &(userheader), sizeof(DltUserHeader), buffer, (size_t)buffersize, NULL, 0);
...
DltReturnValue dlt_user_log_send_log_mode(DltUserLogMode mode, uint8_t version)
{
    ...
    if (version == DLTProtocolV1) {
        ...
        if (dlt_user_set_userheader_v2(&userheader, DLT_USER_MESSAGE_LOG_MODE) < DLT_RETURN_OK)
            return DLT_RETURN_ERROR;
    } else {
        return DLT_RETURN_ERROR;
    }
    ...
    ret = dlt_user_log_out2(
        dlt_user.dlt_log_handle, &(userheader), sizeof(DltUserHeader), &(logmode), sizeof(DltUserControlMsgLogMode));

    /* store message in ringbuffer, if an error has occured */
    if (ret < DLT_RETURN_OK)
        return dlt_user_log_out_error_handling(
            &(userheader), sizeof(DltUserHeader), &(logmode), sizeof(DltUserControlMsgLogMode), NULL, 0);
...
DltReturnValue dlt_user_log_send_marker()
{
    ...
    /* store message in ringbuffer, if an error has occured */
    if (ret < DLT_RETURN_OK)
        return dlt_user_log_out_error_handling(&(userheader), sizeof(DltUserHeader), NULL, 0, NULL, 0);
...
int dlt_get_extendedheadersize_v2(DltUser dlt_user_param, int contextIDSize)
{
    ...
    size += (1 + ((int)dlt_user_param.filenamelen) + (int)sizeof(dlt_user_param.linenumber))
    ...
    if (fd >= 0) {
        ...
        if (ret) {
            ...
            while (1) {
                ...
                /* Check for user header pattern */
                if ((dlt_user_check_userheader(userheader) < 0) || (dlt_user_check_userheader(userheader) == 0))
                    break;
                ...
                case DLT_USER_MESSAGE_LOG_LEVEL: {
                    ...
                    if (usercontextll != NULL) {
                        ...
                        if ((usercontextll->log_level_pos >= 0)
                            ...
                            if (dlt_user.dlt_ll_ts) {
                                dlt_user.dlt_ll_ts[usercontextll->log_level_pos].log_level =
                                ...
                                if (version == DLTProtocolV1) {
                                    ...
                                    dlt_set_id(
                                        ...
                                        dlt_user.dlt_ll_ts[usercontextll->log_level_pos].contextID);
                                    ...
                                    delayed_log_level_changed_callback.contextID2len =
                    ...
                    if (version == DLTProtocolV1) {
                        ...
                        if (dlt_receiver_remove(receiver, sizeof(DltUserHeader) + sizeof(DltUserControlMsgLogLevel))
                            == DLT_RETURN_ERROR)
                            return DLT_RETURN_ERROR;
                    } else if (version == DLTProtocolV2) {
                ...
                case DLT_USER_MESSAGE_INJECTION: {
                    /* At least, user header, user context, and service id and data_length of injected message is
                     * available */
                    ...
                    if (userbuffer != NULL) {
                        ...
                        if ((usercontextinj->data_length_inject > 0) && (dlt_user.dlt_ll_ts))
                            ...
                            for (i = 0; i < dlt_user.dlt_ll_ts[usercontextinj->log_level_pos].nrcallbacks; i++)
                                if ((dlt_user.dlt_ll_ts[usercontextinj->log_level_pos].injection_table)
                                    ...
                                    if (dlt_user.dlt_ll_ts[usercontextinj->log_level_pos]
                                            .injection_table[i]
                                            .injection_callback
                                        != NULL) {
                                        delayed_injection_callback.injection_callback =
                                            dlt_user.dlt_ll_ts[usercontextinj->log_level_pos]
                                                .injection_table[i]
                                                .injection_callback;
                                    } else if (
                                        dlt_user.dlt_ll_ts[usercontextinj->log_level_pos]
                        ...
                        if ((delayed_inject_buffer != NULL)
                            ...
                            delayed_injection_callback.injection_callback(
                                delayed_injection_callback.service_id, delayed_inject_buffer,
                            ...
                            delayed_injection_callback.injection_callback_with_id(
                                delayed_injection_callback.service_id, delayed_inject_buffer,
                                delayed_inject_data_length, delayed_injection_callback.data);
                            delayed_injection_callback.injection_callback_with_id = NULL;
                        }
                        ...
                        if (dlt_receiver_remove(
                                receiver, (int)(sizeof(DltUserHeader) + sizeof(DltUserControlMsgInjection)
                                                + usercontextinj->data_length_inject))
                            != DLT_RETURN_OK)
                            return DLT_RETURN_ERROR;
                    }
                ...
                case DLT_USER_MESSAGE_LOG_STATE: {
                    ...
                    /* keep not read data in buffer */
                    if (dlt_receiver_remove(receiver, (sizeof(DltUserHeader) + sizeof(DltUserControlMsgLogState)))
                        == DLT_RETURN_ERROR)
                        return DLT_RETURN_ERROR;
                } break;
#ifdef DLT_TRACE_LOAD_CTRL_ENABLE
                case DLT_USER_MESSAGE_TRACE_LOAD: {
                    ...
                    trace_load_settings_alloc_size =
                        sizeof(DltTraceLoadSettings) * trace_load_settings_user_messages_count;
                    trace_load_settings = malloc(trace_load_settings_alloc_size);
                    if (trace_load_settings == NULL) {
                        pthread_rwlock_unlock(&trace_load_rw_lock);
                        dlt_vlog(
                            LOG_EMERG,
                            "Unable to allocate memory for trace load settings, no logging will be possible\n");
                    } else {
                        memset(trace_load_settings, 0, trace_load_settings_alloc_size);
                        for (i = 0; i < trace_load_settings_user_messages_count; i++) {
                            memcpy(trace_load_settings[i].apid, dlt_user.appID, DLT_ID_SIZE);
                            memcpy(trace_load_settings[i].ctid, trace_load_settings_user_messages[i].ctid, DLT_ID_SIZE);
                        ...
                        if (messages == NULL) {
                            pthread_rwlock_unlock(&trace_load_rw_lock);
                            dlt_vlog(LOG_ERR, "unable to allocate memory for trace load message buffer\n");
                        } else {
                            ...
                            for (i = 0U; i < trace_load_settings_count; i++) {
                                ...
                                if (messages[i] == NULL) {
                                    dlt_vlog(
                                        LOG_ERR,
                                        "unable to allocate memory for trace load message buffer, index: %u, skipping "
                                        "remaining entries\n",
                                        i);
                                    break;
                                }
                                ...
                                snprintf(
                                    messages[i], 255,
                                    "Received trace load settings: apid=%.4s%s%.4s, soft_limit=%u, hard_limit=%u\n",
                                    trace_load_settings[i].apid,
                                    trace_load_settings[i].ctid[0] == '\0' ? "" : ", ctid=",
                                    trace_load_settings[i].ctid[0] == '\0' ? "" : trace_load_settings[i].ctid,
                                    trace_load_settings[i].soft_limit, trace_load_settings[i].hard_limit);
                            }
                    ...
                    /* keep not read data in buffer */
                    if (dlt_receiver_remove(receiver, (int)trace_load_settings_user_message_bytes_required)
                        == DLT_RETURN_ERROR) {
                        return DLT_RETURN_ERROR;
                    }
                } break;
            ...
            if (size > 0) {
                ...
                if (dlt_user_check_userheader(userheader)) {
                    ...
                    case DLT_USER_MESSAGE_LOG: {
                        ...
                        if (((extendedHeader) != 0)
                            && (extendedHeader->apid[0] == '\0')) /* if application id is empty, add it */
...
#ifdef DLT_SHM_ENABLE
                dlt_shm_push(
                    &dlt_user.dlt_shm, dlt_user.resend_buffer + sizeof(DltUserHeader), size - sizeof(DltUserHeader), 0,
                    0, 0, 0);

                ret = dlt_user_log_out3(
                    dlt_user.dlt_log_handle, dlt_user.resend_buffer, sizeof(DltUserHeader), 0, 0, 0, 0);
#else  /* DLT_SHM_ENABLE */
            ...
            if (size > 0) {
                ...
                if (dlt_user_check_userheader(userheader)) {
                    ...
                    case DLT_USER_MESSAGE_REGISTER_CONTEXT: {
                        ...
                        memcpy(dlt_user.resend_buffer + sizeof(DltUserHeader), &(usercontextv2.apidlen), 1);
                        if (usercontextv2.apid != NULL && usercontextv2.apidlen > 0) {
                            memcpy(
                                dlt_user.resend_buffer + sizeof(DltUserHeader) + 1, usercontextv2.apid,
                    ...
                    case DLT_USER_MESSAGE_LOG: {
                        ...
                        memcpy(
                            dlt_user.resend_buffer + sizeof(DltUserHeader) + BASE_HEADER_V2_FIXED_SIZE
                                + dlt_message_get_extraparameters_size_v2(msgcontent) + offset,
                            &(extendedheaderv2.apidlen), 1);

                        if (extendedheaderv2.apid != NULL && extendedheaderv2.apidlen > 0) {
                            memcpy(
                                dlt_user.resend_buffer + sizeof(DltUserHeader) + BASE_HEADER_V2_FIXED_SIZE
                                    + dlt_message_get_extraparameters_size_v2(msgcontent) + offset + 1,
                                extendedheaderv2.apid, extendedheaderv2.apidlen);
                        }
                        break;
                    }
                    default: {
    ...
    if (dlt_user.dlt_log_handle < 0) {
        ...
        for (num = 0; num < dlt_user.dlt_ll_ts_num_entries; num++)
            ...
            if ((dlt_user.appID[0] != '\0') && (dlt_user.dlt_ll_ts) && (dlt_user.dlt_ll_ts[num].contextID[0] != '\0')) {
                ...
                (dlt_user.appID2len != 0) && (dlt_user.dlt_ll_ts) && (dlt_user.dlt_ll_ts[num].contextID2[0] != 0)) {
                handle.contextID2len = dlt_user.dlt_ll_ts[num].contextID2len;
                /* Ensure handle.contextID2 points to a valid buffer before copying */
...
DltReturnValue dlt_user_log_send_overflow(void)
{
    ...
    if (dlt_user.appID[0] != '\0') {
        ...
        return dlt_user_log_out2(
            dlt_user.dlt_log_handle, &(userheader), sizeof(DltUserHeader), &(userpayload),
        ...
        int buffersize = (int)sizeof(uint32_t) + (int)sizeof(uint8_t) + (int)userpayload2.apidlen;
        ...
        return dlt_user_log_out2(
            dlt_user.dlt_log_handle, &(userheader), sizeof(DltUserHeader), buffer, (size_t)buffersize);
    } else {
        return DLT_RETURN_ERROR;
    }
}
...
int dlt_start_threads()
{
    ...
    if (pthread_create(&(dlt_user.dlt_segmented_nwt_handle), NULL, dlt_user_trace_network_segmented_thread, NULL)) {
...
void dlt_stop_threads()
{
    ...
    if (dlt_housekeeperthread_handle) {
        /*
         * Cooperatively signal the housekeeper thread to exit.
         * This avoids relying on pthread_cancel, which is unreliable
         * on some platforms (e.g. MSYS2) and can cause deadlocks
         * when the thread holds a mutex at the cancellation point.
         * The housekeeper checks this flag each loop iteration
         ...
         * The housekeeper checks the exit flag each loop iteration
         * (at most DLT_USER_RECEIVE_MDELAY ms latency), so we poll
         * with short sleeps. If the thread doesn't exit within the
         * timeout, fall back to pthread_cancel.
         */
        {
            struct timespec ts;
            int wait_ms;
            for (wait_ms = 0; wait_ms < 3000; wait_ms += DLT_USER_RECEIVE_MDELAY) {
        ...
        if (pthread_kill(dlt_housekeeperthread_handle, 0) != 0) {
            ...
            /* Thread didn't exit cooperatively, cancel then join */
    ...
    if (dlt_user.dlt_segmented_nwt_handle) {
        ...
        if (dlt_segmented_nwt_result != 0)
            dlt_vlog(
                LOG_ERR, "ERROR pthread_cancel(dlt_user.dlt_segmented_nwt_handle): %s\n",
    ...
    if ((dlt_housekeeperthread_result == 0) && dlt_housekeeperthread_handle) {
        ...
        if (joined != 0)
            dlt_vlog(LOG_ERR, "ERROR pthread_join(dlt_housekeeperthread_handle, NULL): %s\n", strerror(joined));
    ...
    if ((dlt_segmented_nwt_result == 0) && dlt_user.dlt_segmented_nwt_handle) {
        ...
        if (joined != 0)
            dlt_vlog(LOG_ERR, "ERROR pthread_join(dlt_user.dlt_segmented_nwt_handle, NULL): %s\n", strerror(joined));
...
DltReturnValue dlt_user_log_out_error_handling(
    void* ptr1, size_t len1, void* ptr2, size_t len2, void* ptr3, size_t len3)
{
    DltReturnValue ret = DLT_RETURN_ERROR;
    ...
    if (dlt_buffer_push3(
            &(dlt_user.startup_buffer), ptr1, (unsigned int)len1, ptr2, (unsigned int)len2, ptr3, (unsigned int)len3)
        == DLT_RETURN_ERROR) {

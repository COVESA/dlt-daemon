                                      ...
                                      DltLogLevelType log_level)
{
    ...
    *buffer = '\0';
    ...
    int head_line = 1, first_row = 1, cpu_count = 0, column = 0,
        buffer_offset = 0;
    DltReturnValue ret;

    if ((ret = dlt_kpi_read_file("/proc/interrupts", file_buffer,
                                 BUFFER_SIZE)) < DLT_RETURN_OK)
        return ret;
    ...
    while (token != NULL) {
        if (head_line) {
            if ((strlen(token) > 3) && (token[0] == 'C') && (token[1] == 'P') &&
                (token[2] == 'U')) {
                cpu_count++;
            }
            else if (cpu_count <= 0) {
    ...
    if ((ret = dlt_user_log_write_start(ctx, &ctx_data, log_level)) <
        DLT_RETURN_OK) {
    ...
    while (token != NULL) {
        if (dlt_user_log_write_string(&ctx_data, token) < DLT_RETURN_OK) {
            ...
            if ((ret = dlt_user_log_write_string(&ctx_data, "IRQ")) <
                DLT_RETURN_OK) {
                fprintf(stderr,

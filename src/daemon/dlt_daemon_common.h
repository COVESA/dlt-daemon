...
#define DLT_DAEMON_RINGBUFFER_MIN_SIZE                                       \
    500000 /**< Ring buffer size for storing log messages while no client is \
              connected */
#define DLT_DAEMON_RINGBUFFER_MAX_SIZE                                         \
    10000000 /**< Ring buffer size for storing log messages while no client is \
                connected */
...
#define DLT_DAEMON_SEND_TO_ALL \
    (-3) /**< Constant value to identify the command "send to all" */
#define DLT_DAEMON_SEND_FORCE \
    (-4) /**< Constant value to identify the command "send force to all" */
...
typedef enum {
    ...
    DLT_DAEMON_STATE_BUFFER =
        1, /**< logging is buffered until external logger is connected or
              internal logging is activated */
    DLT_DAEMON_STATE_BUFFER_FULL =
        2, /**< then internal buffer is full, wait for connect from client */
    DLT_DAEMON_STATE_SEND_BUFFER =
        3, /**< external logger is connected, but buffer is still not empty or
              external logger queue is full */
    DLT_DAEMON_STATE_SEND_DIRECT =
        4 /**< External logger is connected or internal logging is active, and
             buffer is empty */
} DltDaemonState;
...
typedef struct {
    ...
    char *ctid; /**< Context id for which the settings are valid, empty if valid
                   for all ap ids  */
    DltLogLevelType log_level; /**< Log level to use */
...
typedef struct {
    ...
    int log_level_pos;      /**< offset of context in context field on user
                               application */
    int user_handle; /**< connection handle for connection to user application
                      */
    char *context_description; /**< context description */
    int8_t storage_log_level;  /**< log level set for offline logstorage */
    bool predefined; /**< set to true if this context is predefined by runtime
                        configuration file */
#ifdef DLT_TRACE_LOAD_CTRL_ENABLE
    DltTraceLoadSettings
        *trace_load_settings; /**< trace load setting for the context */
#endif
} DltDaemonContext;
...
typedef struct {
    ...
    int num_contexts; /**< Total number of all contexts in all applications in
                         this list */
...
typedef struct DltDaemon {
    ...
    int8_t force_ll_ts; /**< Enforce ll and ts to not exceed default_log_level,
                           default_trace_status */
    unsigned int overflow_counter;  /**< counts the number of lost messages. */
    int runtime_context_cfg_loaded; /**< Set to one, if runtime context
                                       configuration has been loaded, zero
                                       otherwise */
    ...
    int sendserialheader; /**< 1: send serial header; 0 don't send serial header
                           */
    int timingpackets;    /**< 1: send continous timing packets; 0 don't send
                             continous timing packets */
    DltBuffer client_ringbuffer; /**< Ring-buffer for storing received logs
                                    while no client connection is available */
    char runtime_application_cfg[PATH_MAX +
                                 1];        /**< Path and filename of persistent
                                               application configuration. Set to path
                                               max, as it specifies a full path*/
    char runtime_context_cfg[PATH_MAX + 1]; /**< Path and filename of persistent
                                               context configuration */
    char runtime_configuration[PATH_MAX + 1]; /**< Path and filename of
                                                 persistent configuration */
    DltUserLogMode
        mode; /**< Mode used for tracing: off, external, internal, both */
    char connectionState;   /**< state for tracing: 0 = no client connected, 1 =
                               client connected */
    char *ECUVersionString; /**< Version string to send to client. Loaded from a
                               file at startup. May be null. */
    ...
    unsigned int
        watchdog_trigger_interval; /**< watchdog trigger interval in [s] */
    unsigned int watchdog_last_trigger_time; /**< when the watchdog was last
                                                triggered in [s] */
#endif
#ifdef DLT_LOG_LEVEL_APP_CONFIG
    DltDaemonContextLogSettings
        *app_id_log_level_settings; /**< Settings for app id specific log levels
                                     */
    int num_app_id_log_level_settings; /**< count of log level settings */
#endif
#ifdef DLT_TRACE_LOAD_CTRL_ENABLE
    DltTraceLoadSettings
        *preconfigured_trace_load_settings;      /**< Settings for trace load */
    int preconfigured_trace_load_settings_count; /**< count of trace load
                                                    settings */
    int bytes_sent;
    int bytes_recv;
#endif
 ...
 * @param InitialContextLogLevel loglevel to be sent to context when those
 * register with loglevel default, read from dlt.conf
 * @param InitialContextTraceStatus tracestatus to be sent to context when those
 * register with tracestatus default, read from dlt.conf
...
/**
 * Initialize data structures to store information about applications running on
 * same or passive node.
 * @param daemon pointer to dlt daemon structure
 ...
 */
DltDaemonContextLogSettings *
dlt_daemon_find_app_log_level_config(const DltDaemonApplication *app,
                                     const char *ctid);
...
/**
 * DLTv2 Clear all applications in internal application management of specific
 * ecu
 * @param daemon pointer to dlt daemon structure
 * @param ecu pointer to ecu id of node to clear applications
 * @param verbose if set to true verbose information is printed out.
 ...
 * @return negative value if there was an error
 */
int dlt_daemon_user_send_trace_load_config(DltDaemon *daemon,
                                           DltDaemonApplication *app,
                                           int verbose);
#endif

/**
 * Send user messages to all user applications using default context, or trace
 * status to update those values
 * @param daemon pointer to dlt daemon structure
 * @param verbose if set to true verbose information is printed out.
 */
...
/**
 * DLTv2 Send user messages to all user applications using default context, or
 * trace status to update those values
 * @param daemon pointer to dlt daemon structure
 * @param verbose if set to true verbose information is printed out.
 */
...
/**
 * Send user messages to all user applications context to update with the new
 * log level
 * @param daemon pointer to dlt daemon structure
...
/**
 * DLTv2 Send user messages to all user applications context to update with the
 * new log level
 * @param daemon pointer to dlt daemon structure
 * @param enforce_context_ll_and_ts defines if enforcement of log levels is on
 * @param context_log_level the log level of the context

...
TEST(t_dlt_daemon_init_user_information, normal_multiple_lists)
{
    ...
    gateway.connections = (DltGatewayConnection*)calloc(2, sizeof(DltGatewayConnection));
...
TEST(t_dlt_daemon_application_add, normal)
{
    ...
    /*printf("### APP: APID=%s  DESCR=%s NUMCONTEXT=%i PID=%i USERHANDLE=%i\n", app->apid,app->application_description,
     * app->num_contexts, app->pid, app->user_handle); */
...
TEST(t_dlt_daemon_application_add, abnormal)
{
     ...
     * DLT_DAEMON_RINGBUFFER_STEP_SIZE, DLT_RUNTIME_DEFAULT_DIRECTORY,DLT_LOG_INFO, DLT_TRACE_STATUS_OFF,0,0)); */
    ...
    /*    app = dlt_daemon_application_add(&daemon,(char *) apid, pid, (char *) desc, 0); */
    /*    EXPECT_EQ((DltDaemonApplication *)0, dlt_daemon_application_add(&daemon,(char *) apid, pid, (char *) desc,
     * 12345678)); */
    /*    dlt_daemon_application_del(&daemon, app, 0); */
    /*    dlt_daemon_applications_clear(&daemon, 0); */
...
TEST(t_dlt_daemon_applications_clear, normal)
{
    ...
    EXPECT_LE((DltDaemonApplication*)0, dlt_daemon_application_add(&daemon, apid1_buf, pid, desc1_buf, fd, ecu, 0));
    memcpy(apid1_buf, "TES1", DLT_ID_SIZE);
    apid1_buf[DLT_ID_SIZE] = '\0';
    strncpy(desc1_buf, "Test clear 1", sizeof(desc1_buf) - 1);
    desc1_buf[sizeof(desc1_buf) - 1] = '\0';
...
TEST(t_dlt_daemon_context_add, normal)
{
    ...
    daecontext =
        dlt_daemon_context_add(&daemon, apid, ctid, DLT_LOG_DEFAULT, DLT_TRACE_STATUS_DEFAULT, 0, 0, desc, ecu, 0);
...
TEST(t_dlt_daemon_context_add, abnormal)
{
    ...
    app = dlt_daemon_application_add(&daemon, apid, 0, desc, fd, ecu, 0);
    daecontext =
        dlt_daemon_context_add(&daemon, apid, ctid, invalid_log_level, DLT_TRACE_STATUS_DEFAULT, 0, 0, desc, ecu, 0);
    /*printf("### CONTEXT: APID=%s\tCTID=%s\n", daecontext->apid,daecontext->ctid); */
    ...
    app = dlt_daemon_application_add(&daemon, apid, 0, desc, fd, ecu, 0);
    daecontext = dlt_daemon_context_add(&daemon, apid, ctid, DLT_LOG_DEFAULT, invalid_trace_status, 0, 0, desc, ecu, 0);
    /*printf("### CONTEXT: APID=%s\tCTID=%s\n", daecontext->apid,daecontext->ctid); */
    EXPECT_EQ((DltDaemonContext*)0, daecontext);
    EXPECT_GE(-1, dlt_daemon_context_del(&daemon, daecontext, ecu, 0));
    EXPECT_LE(0, dlt_daemon_application_del(&daemon, app, ecu, 0));
...
TEST(t_dlt_daemon_context_find, abnormal)
{
    ...
    daecontext =
        dlt_daemon_context_add(&daemon, no_apid, ctid, DLT_LOG_DEFAULT, DLT_TRACE_STATUS_DEFAULT, 0, 0, desc, ecu, 0);
    ...
    app = dlt_daemon_application_add(&daemon, apid, 0, desc, fd, ecu, 0);
    daecontext =
        dlt_daemon_context_add(&daemon, apid, no_ctid, DLT_LOG_DEFAULT, DLT_TRACE_STATUS_DEFAULT, 0, 0, desc, ecu, 0);
...
TEST(t_dlt_daemon_user_send_log_level, normal)
{
    ...
    app = dlt_daemon_application_add(&daemon, apid, 0, desc, fd, ecu, 0);
    daecontext =
        dlt_daemon_context_add(&daemon, apid, ctid, DLT_LOG_DEFAULT, DLT_TRACE_STATUS_DEFAULT, 0, 1, desc, ecu, 0);
...
TEST(t_dlt_daemon_user_send_log_state, normal)
{
     ...
     * DLT_DAEMON_RINGBUFFER_STEP_SIZE, "",0); */
...
TEST(t_dlt_daemon_find_preconfigured_trace_load_settings, nullpointer)
{
    ...
    EXPECT_EQ(
        dlt_daemon_find_preconfigured_trace_load_settings(NULL, "APP1", "CTID", &settings, &num_settings, 0),
        DLT_RETURN_WRONG_PARAMETER);
    EXPECT_EQ(
        dlt_daemon_find_preconfigured_trace_load_settings(&daemon, NULL, "CTID", &settings, &num_settings, 0),
        DLT_RETURN_WRONG_PARAMETER);
}
...
TEST(t_dlt_daemon_find_preconfigured_trace_load_settings, empty_trace_settings)
{
    ...
    EXPECT_EQ(
        dlt_daemon_find_preconfigured_trace_load_settings(&daemon, "APP1", NULL, &settings, &num_settings, 0),
        DLT_RETURN_OK);
    ...
    EXPECT_EQ(
        dlt_daemon_find_preconfigured_trace_load_settings(&daemon, "APP1", "CTID", &settings, &num_settings, 0),
        DLT_RETURN_OK);
...
TEST(t_dlt_daemon_find_preconfigured_trace_load_settings, search_with_app_id)
{
    ...
    for (int i = 0; i < num_settings; i++) {
        EXPECT_EQ(memcmp(&settings[i], &daemon.preconfigured_trace_load_settings[i], sizeof(DltTraceLoadSettings)), 0);
    }
...
TEST(t_dlt_daemon_application_add, trace_load_settings_not_found)
{
    ...
    EXPECT_EQ(app->trace_load_settings[0].soft_limit, DLT_TRACE_LOAD_DAEMON_SOFT_LIMIT_DEFAULT);
    EXPECT_EQ(app->trace_load_settings[0].hard_limit, DLT_TRACE_LOAD_DAEMON_HARD_LIMIT_DEFAULT);
    EXPECT_STREQ(app->trace_load_settings[0].apid, apid);
    EXPECT_TRUE(app->trace_load_settings[0].ctid == NULL || strlen(app->trace_load_settings[0].ctid) == 0);
...
TEST(t_dlt_daemon_application_add, trace_load_settings_configured)
{
    ...
    for (uint32_t i = 0; i < app->trace_load_settings_count; i++) {
        EXPECT_EQ(
            memcmp(
                &app->trace_load_settings[i], &daemon.preconfigured_trace_load_settings[i],

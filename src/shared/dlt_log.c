...
MultipleFilesRingBuffer multiple_files_ring_buffer = {.directory = {0},
                                                      .filename = {0},
                                                      .fileSize = 0,
                                                      .maxSize = 0,
                                                      .filenameTimestampBased =
                                                          false,
                                                      .filenameBase = {0},
                                                      .filenameExt = {0},
                                                      .ohandle = -1};
...
{
    return dlt_log_init_multiple_logfiles_support((DltLoggingMode)mode, false,
                                                  0, 0);
}

DltReturnValue dlt_log_init_multiple_logfiles_support(
    const DltLoggingMode mode, const bool enable_multiple_logfiles,
    const int logging_file_size, const int logging_files_max_size)
{
    ...
    if (enable_multiple_logfiles) {
        ...
        result = dlt_log_init_multiple_logfiles(logging_file_size,
                                                logging_files_max_size);
        if (result != DLT_RETURN_OK) {
            dlt_user_printf("dlt logging for limits fails with error code=%d, "
                            "use logging without limits as fallback\n",
                            result);
...
DltReturnValue dlt_log_init_multiple_logfiles(const int logging_file_size,
                                              const int logging_files_max_size)
{
    ...
    if (directory[0]) {
        ...
        if (!dlt_extract_base_name_without_ext(file_name, filename_base,
        ...
        DltReturnValue result = multiple_files_buffer_init(
            &multiple_files_ring_buffer, directory, logging_file_size,
            logging_files_max_size, false, true, filename_base, filename_ext);
...
static int dlt_user_printf_v(const char *format, va_list *args)
{
    int ret = 0;

    if (logging_mode == DLT_LOG_TO_STDERR)
...
int dlt_user_printf(const char *format, ...)
{
    ...
    int ret = dlt_user_printf_v(format, &args);
...
{
    static const char asSeverity[LOG_DEBUG + 2][11] = {
    ...
    case DLT_LOG_TO_CONSOLE:
        /* log to stdout */
        fprintf(stdout, sFormatString, (unsigned int)sTimeSpec.tv_sec,
                (unsigned int)(sTimeSpec.tv_nsec / 1000), getpid(),
                asSeverity[prio], s);
        fflush(stdout);
        break;
    case DLT_LOG_TO_STDERR:
        /* log to stderr */
        fprintf(stderr, sFormatString, (unsigned int)sTimeSpec.tv_sec,
                (unsigned int)(sTimeSpec.tv_nsec / 1000), getpid(),
                asSeverity[prio], s);
        break;
    case DLT_LOG_TO_SYSLOG:
        ...
        syslog(prio, sFormatString, (unsigned int)sTimeSpec.tv_sec,
               (unsigned int)(sTimeSpec.tv_nsec / 1000), getpid(),
               asSeverity[prio], s);
        closelog();
#endif
    ...
    case DLT_LOG_TO_FILE:
        ...
        if (dlt_is_log_in_multiple_files_active()) {
            dlt_log_multiple_files_write(
                sFormatString, (unsigned int)sTimeSpec.tv_sec,
                (unsigned int)(sTimeSpec.tv_nsec / 1000), getpid(),
                asSeverity[prio], s);
        }
        else if (logging_handle) {
            fprintf(logging_handle, sFormatString,
...
static DltReturnValue dlt_vlog_v(int prio, const char *format, va_list *args)
{
    char outputString[2048] = {
        0}; /* TODO: what is a reasonable string length here? */
...
DltReturnValue dlt_vlog(int prio, const char *format, ...)
{
    ...
    DltReturnValue ret = dlt_vlog_v(prio, format, &args);
...
static DltReturnValue dlt_vnlog_v(int prio, size_t size, const char *format,
                                  va_list *args)
{
    char *outputString = (char *)calloc(size + 1, sizeof(char));
...
DltReturnValue dlt_vnlog(int prio, size_t size, const char *format, ...)
{
    ...
    DltReturnValue ret = dlt_vnlog_v(prio, size, format, &args);
...
static void dlt_log_multiple_files_write_v(const char *format, va_list *args)
{
    char output_string[2048] = {0};
    vsnprintf(output_string, 2047, format, *args);
    multiple_files_buffer_write(&multiple_files_ring_buffer,
                                (unsigned char *)output_string,
                                (int)strlen(output_string));
}
...
void dlt_log_multiple_files_write(const char *format, ...)
{
    va_list args;
    va_start(args, format);
    dlt_log_multiple_files_write_v(format, &args);
    va_end(args);
}
...
{
    if (DLT_RETURN_ERROR ==
        multiple_files_buffer_free(&multiple_files_ring_buffer))
        return;

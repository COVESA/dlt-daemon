                                                    ...
                                                    int verbose)
{
    ...
    if ((daemon_local->flags.evalue[0]) &&
        ...
        char ecuid_buf[DLT_ID_SIZE + 1];
        dlt_set_id_v2((char **)&ecuid_buf, daemon->ecuid2, (int8_t)daemon->ecuid2len);
        daemon_local->msgv2.extendedheaderv2.ecid = ecuid_buf;
                                              ...
                                              int verbose)
{
    ...
    if (DLT_IS_HTYP2_WEID(msg->baseheaderv2->htyp2)) {
        ...
        char ecid_buf[DLT_ID_SIZE];
        dlt_set_id_v2((char **)&ecid_buf, daemon->ecuid2, (int8_t)msg->extendedheaderv2.ecidlen);
    ...
    if (DLT_IS_HTYP2_WACID(msg->baseheaderv2->htyp2)) {
        ...
        if (apid == NULL) {
            dlt_set_id_v2((char **)&apid_buf, DLT_DAEMON_CTRL_APID, (int8_t)msg->extendedheaderv2.apidlen);
        } else {
            dlt_set_id_v2((char **)&apid_buf, apid, (int8_t)msg->extendedheaderv2.apidlen);
        }
        msg->extendedheaderv2.apid = apid_buf;
        msg->extendedheaderv2.ctidlen = ctxidlen;
        char ctid_buf[DLT_ID_SIZE];
        if (ctid == NULL) {
            dlt_set_id_v2((char **)&ctid_buf, DLT_DAEMON_CTRL_CTID, (int8_t)msg->extendedheaderv2.ctidlen);
        } else {
            dlt_set_id_v2((char **)&ctid_buf, ctid, (int8_t)msg->extendedheaderv2.ctidlen);
        }
                                     ...
                                     int verbose)
{
    ...
    db_offset = db_offset + (int)sizeof(uint8_t);
    char apid_buf[DLT_ID_SIZE];
    dlt_set_id_v2((char **)&apid_buf, (const char *)(msg->databuffer + db_offset), (int8_t)req->apidlen);
    req->apid = apid_buf;
    db_offset = db_offset + (int)req->apidlen;
    memcpy(&(req->ctidlen), (const char *)(msg->databuffer + db_offset), sizeof(uint8_t));
    db_offset = db_offset + (int)sizeof(uint8_t);
    char ctid_buf[DLT_ID_SIZE];
    dlt_set_id_v2((char **)&ctid_buf, (const char *)(msg->databuffer + db_offset), (int8_t)req->ctidlen);
    req->ctid = ctid_buf;
    ...
    if (count_app_ids != 0) {
        ...
        for (int i = 0; i < count_app_ids; i++) {
            ...
            if ((user_list->applications) && (application)) {
                ...
                for (int j = 0; j < (application - (user_list->applications)); j++)
                    offset_base += user_list->applications[j].num_contexts;
                                                  ...
                                                  int verbose)
{
    ...
    char apid_buf[DLT_ID_SIZE];
    dlt_set_id_v2((char **)&apid_buf, apid, (int8_t)apidlen);
    resp.apid = apid_buf;
    resp.ctidlen = ctidlen;
    char ctid_buf[DLT_ID_SIZE];
    dlt_set_id_v2((char **)&ctid_buf, ctid, (int8_t)ctidlen);
    resp.ctid = ctid_buf;
    dlt_set_id(resp.comid, comid);
                                          ...
                                          int verbose)
{
    ...
    if (id >= DLT_DAEMON_INJECTION_MIN) {
        ...
        if (DLT_IS_HTYP2_EH(msg->baseheaderv2->htyp2)) {
            ...
            dlt_set_id_v2(&apid, msg->extendedheaderv2.apid, (int8_t)msg->extendedheaderv2.apidlen);
            ctidlen = msg->extendedheaderv2.ctidlen;
            dlt_set_id_v2(&ctid, msg->extendedheaderv2.ctid, (int8_t)msg->extendedheaderv2.ctidlen);
        }
        else {
            /* No extended header, and therefore no apid and ctid available */
                                                         ...
                                                         int verbose)
{
    ...
    for (count = 0; count < user_list->num_contexts; count++) {
        ...
        if (context) {
            if (app_flag == 1)
                dlt_set_id_v2(&src_str, context->apid2, (int8_t)context->apid2len);
            else
                dlt_set_id_v2(&src_str, context->ctid2, (int8_t)context->ctid2len);

            ret = strncmp(src_str, str, (size_t)len);

            if (ret == 0)
                dlt_daemon_send_log_level_v2(sock, daemon, daemon_local, context, loglevel, verbose);
            else if ((ret > 0) && (app_flag == 1))
                break;
            else
                continue;
                                      ...
                                      int verbose)
{
    ...
    offset = offset + (int)sizeof(uint8_t);
    char apid_buf[DLT_ID_SIZE];
    dlt_set_id_v2((char **)&apid_buf, (const char *)(msg->databuffer + offset), (int8_t)req.apidlen);
    req.apid = apid_buf;
    offset = offset + req.apidlen;
    memcpy(&(req.ctidlen), msg->databuffer + offset, sizeof(uint8_t));
    offset = offset + (int)sizeof(uint8_t);
    char ctid_buf[DLT_ID_SIZE];
    dlt_set_id_v2((char **)&ctid_buf, (const char *)(msg->databuffer + offset), (int8_t)req.ctidlen);
    req.ctid = ctid_buf;
    ...
    dlt_set_id_v2(&apid, req.apid, (int8_t)req.apidlen);
    ctid_length = (int8_t) req.ctidlen;
    dlt_set_id_v2(&ctid, req.ctid, (int8_t)req.ctidlen);
                                                            ...
                                                            int verbose)
{
    ...
    for (count = 0; count < user_list->num_contexts; count++) {
        ...
        if (context) {
            ...
            if (ret == 0)
                dlt_daemon_send_trace_status_v2(sock, daemon, daemon_local, context, tracestatus, verbose);
            else if ((ret > 0) && (app_flag == 1))
                break;
            else
                continue;

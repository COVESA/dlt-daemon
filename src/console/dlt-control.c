...
int main(int argc, char *argv[])
{
    ...
    if (dlt_client_connect(&g_dltclient, dltdata.vflag) != DLT_RETURN_ERROR) {
        ...
        if (dltdata.mvalue && dltdata.avalue && dltdata.cvalue) {
            ...
            if (dlt_client_send_inject_msg(&g_dltclient, dltdata.avalue,
                                           dltdata.cvalue,
                 ...
                 DLT_INVALID_LOG_LEVEL) /*&& dltdata.avalue && dltdata.cvalue)*/
        {
            if ((dltdata.avalue == 0) && (dltdata.cvalue == 0)) {
                ...
                if (0 != dlt_client_send_all_log_level(
                ...
                /* send control message*/
                if (0 != dlt_client_send_log_level(&g_dltclient, dltdata.avalue,
                                                   dltdata.cvalue,
            ...
            if ((dltdata.avalue == 0) && (dltdata.cvalue == 0)) {
                ...
                if (0 != dlt_client_send_all_trace_status(
                ...
                /* send control message*/
                if (0 != dlt_client_send_trace_status(
                             &g_dltclient, dltdata.avalue, dltdata.cvalue,
            ...
            if (dlt_client_send_default_log_level(
            ...
            /* send control message in*/
            if (dlt_client_send_default_trace_status(
            ...
            /* send control message in*/
            if (dlt_client_send_timing_pakets(
            ...
            /* send control message in*/
            if (dlt_client_send_reset_to_factory_default(&g_dltclient) !=
                DLT_RETURN_OK) {
                fprintf(stderr,
...
int dlt_receive_message_callback(DltMessage *message, void *data)
{
    ...
    if ((id > DLT_SERVICE_ID) && (id < DLT_SERVICE_ID_LAST_ENTRY) &&
        ...
        case DLT_SERVICE_ID_GET_LOG_INFO: {
            ...
            /* get response data */
            ret =
                dlt_message_header(message, resp_text, DLT_RECEIVE_BUFSIZE, 0);
            ...
            ret = dlt_message_payload(message, resp_text, DLT_RECEIVE_BUFSIZE,
                                      DLT_OUTPUT_ASCII, 0);
            ...
            if ((ret == 0) &&
                (resp->service_id == DLT_SERVICE_ID_GET_LOG_INFO)) {
                ret = dlt_client_parse_get_log_info_resp_text(resp, resp_text);
        ...
        case DLT_SERVICE_ID_GET_SOFTWARE_VERSION: {
            ...
            DLT_MSG_READ_VALUE(uint32_tmp, ptr, datalength, uint32_t);
            resp->length =
                DLT_ENDIAN_GET_32(message->standardheader->htyp, uint32_tmp);
            ...
            for (payload_index = 0; payload_index < resp->length;
                 payload_index++)
                resp->payload[payload_index] = (char)ptr[payload_index];

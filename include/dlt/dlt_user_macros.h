 ...
 * @param CONTEXT object containing information about one special logging
 * context
 * @note To avoid the MISRA warning "Null statement is located close to other
 * code or comments" remove the semicolon when using the macro. Example:
 * DLT_DECLARE_CONTEXT(hContext)
 */
...
#define DLT_REGISTER_APP(APPID, DESCRIPTION)                                   \
    do {                                                                       \
        (void)dlt_check_library_version(_DLT_PACKAGE_MAJOR_VERSION,            \
                                        _DLT_PACKAGE_MINOR_VERSION);           \
 ...
 * @param DESCRIPTION ASCII string containing description
 */
#define DLT_REGISTER_APP_V2(APPID, DESCRIPTION)                                \
    do {                                                                       \
        (void)dlt_check_library_version(_DLT_PACKAGE_MAJOR_VERSION,            \
                                        _DLT_PACKAGE_MINOR_VERSION);           \
        (void)dlt_register_app_v2(APPID, DESCRIPTION);                         \
    } while (false)
 ...
 */
#define DLT_UNREGISTER_APP_V2()                                                \
    do {                                                                       \
        (void)dlt_unregister_app_v2();                                         \
    } while (false)

/**
 * Unregister application and flush the logs buffered in startup buffer if any.
 */
#define DLT_UNREGISTER_APP_FLUSH_BUFFERED_LOGS()                               \
    do {                                                                       \
        (void)dlt_unregister_app_flush_buffered_logs();                        \
    } while (false)

/**
 * DLTv2 Unregister application and flush the logs buffered in startup buffer if
 * any.
 */
#define DLT_UNREGISTER_APP_FLUSH_BUFFERED_LOGS_V2()                            \
    do {                                                                       \
        (void)dlt_unregister_app_flush_buffered_logs_v2();                     \
    } while (false)
...
#define DLT_REGISTER_CONTEXT(CONTEXT, CONTEXTID, DESCRIPTION)                  \
    do {                                                                       \
        (void)dlt_register_context(&(CONTEXT), CONTEXTID, DESCRIPTION);        \
    } while (false)
 ...
 * @param DESCRIPTION ASCII string containing description
 */
#define DLT_REGISTER_CONTEXT_V2(CONTEXT, CONTEXTID, DESCRIPTION)               \
    do {                                                                       \
        (void)dlt_register_context_v2(&(CONTEXT), CONTEXTID, DESCRIPTION);     \
    } while (false)
 ...
 */
#define DLT_REGISTER_CONTEXT_LL_TS(CONTEXT, CONTEXTID, DESCRIPTION, LOGLEVEL,  \
                                   TRACESTATUS)                                \
    do {                                                                       \
        (void)dlt_register_context_ll_ts(&(CONTEXT), CONTEXTID, DESCRIPTION,   \
                                         LOGLEVEL, TRACESTATUS);               \
    } while (false)

/**
 * DLTv2 Register context with pre-defined log level and pre-defined trace
 * status.
 * @param CONTEXT object containing information about one special logging
 * context
 * @param CONTEXTID context id
 ...
 * (DLT_TRACE_STATUS_DEFAULT is not allowed here)
 */
#define DLT_REGISTER_CONTEXT_LL_TS_V2(CONTEXT, CONTEXTID, DESCRIPTION,         \
                                      LOGLEVEL, TRACESTATUS)                   \
    do {                                                                       \
        (void)dlt_register_context_ll_ts_v2(                                   \
            &(CONTEXT), CONTEXTID, DESCRIPTION, LOGLEVEL, TRACESTATUS);        \
    } while (false)

/**
 * Register context (with default log level and default trace status and log
 ...
 */
#define DLT_REGISTER_CONTEXT_LLCCB(CONTEXT, CONTEXTID, DESCRIPTION, CBK)       \
    do {                                                                       \
        (void)dlt_register_context_llccb(&(CONTEXT), CONTEXTID, DESCRIPTION,   \
                                         CBK);                                 \
    } while (false)

/**
 * DLTv2 Register context (with default log level and default trace status and
 ...
 * @param CBK log level change callback to be registered
 */
#define DLT_REGISTER_CONTEXT_LLCCB_V2(CONTEXT, CONTEXTID, DESCRIPTION, CBK)    \
    do {                                                                       \
        (void)dlt_register_context_llccb_v2(&(CONTEXT), CONTEXTID,             \
                                            DESCRIPTION, CBK);                 \
    } while (false)
...
#define DLT_UNREGISTER_CONTEXT(CONTEXT)                                        \
    do {                                                                       \
        (void)dlt_unregister_context(&(CONTEXT));                              \
    } while (false)
 ...
 * context
 */
#define DLT_UNREGISTER_CONTEXT_V2(CONTEXT)                                     \
    do {                                                                       \
        (void)dlt_unregister_context_v2(&(CONTEXT));                           \
    } while (false)
...
#define DLT_REGISTER_INJECTION_CALLBACK(CONTEXT, SERVICEID, CALLBACK)          \
    do {                                                                       \
        (void)dlt_register_injection_callback(&(CONTEXT), SERVICEID,           \
                                              CALLBACK);                       \
    } while (false)
 ...
 */
#define DLT_REGISTER_INJECTION_CALLBACK_WITH_ID(CONTEXT, SERVICEID, CALLBACK,  \
                                                PRIV_DATA)                     \
    do {                                                                       \
        (void)dlt_register_injection_callback_with_id(&(CONTEXT), SERVICEID,   \
                                                      CALLBACK, PRIV_DATA);    \
    } while (false)
...
#define DLT_REGISTER_LOG_LEVEL_CHANGED_CALLBACK(CONTEXT, CALLBACK)             \
    do {                                                                       \
        (void)dlt_register_log_level_changed_callback(&(CONTEXT), CALLBACK);   \
    } while (false)
 ...
 * @param CALLBACK function pointer to callback function
 */
#define DLT_REGISTER_LOG_LEVEL_CHANGED_CALLBACK_V2(CONTEXT, CALLBACK)          \
    do {                                                                       \
        (void)dlt_register_log_level_changed_callback_v2(&(CONTEXT),           \
                                                         CALLBACK);            \
    } while (false)
 ...
 * @note To avoid the MISRA warning "The comma operator has been used outside a
 * for statement" use a semicolon instead of a comma to separate the
 * __VA_ARGS__. Example: DLT_LOG(hContext, DLT_LOG_INFO, DLT_STRING("Hello
 * world"); DLT_INT(123));
 */
#ifdef _MSC_VER
...
/**
 * DLTv2 Send log message with variable list of messages (intended for verbose
 * mode)
 * @param CONTEXT object containing information about one special logging
 * context
 * @param LOGLEVEL the log level of the log message
 ...
 * @note To avoid the MISRA warning "The comma operator has been used outside a
 * for statement" use a semicolon instead of a comma to separate the
 * __VA_ARGS__. Example: DLT_LOG_TS(hContext, DLT_LOG_INFO, timestamp,
 * DLT_STRING("Hello world"); DLT_INT(123));
 */
#ifdef _MSC_VER
...
/**
 * Send log message with variable list of messages (intended for non-verbose
 * mode)
 * @param CONTEXT object containing information about one special logging
 * context
 * @param LOGLEVEL the log level of the log message
 ...
 * @note To avoid the MISRA warning "The comma operator has been used outside a
 * for statement" use a semicolon instead of a comma to separate the
 * __VA_ARGS__. Example: DLT_LOG_ID(hContext, DLT_LOG_INFO, 0x1234,
 * DLT_STRING("Hello world"); DLT_INT(123));
 */
#ifdef _MSC_VER
...
#define DLT_LOG_ID(CONTEXT, LOGLEVEL, MSGID, ...)                              \
    do {                                                                       \
        DltContextData log_local;                                              \
        int dlt_local;                                                         \
        dlt_local = dlt_user_log_write_start_id(&CONTEXT, &log_local,          \
                                                LOGLEVEL, MSGID);              \
        if (dlt_local == DLT_RETURN_TRUE) {                                    \
            __VA_ARGS__;                                                       \
            (void)dlt_user_log_write_finish(&log_local);                       \
 ...
 * @note To avoid the MISRA warning "The comma operator has been used outside a
 * for statement" use a semicolon instead of a comma to separate the
 * __VA_ARGS__. Example: DLT_LOG_ID_TS(hContext, DLT_LOG_INFO, 0x1234,
 * timestamp, DLT_STRING("Hello world"); DLT_INT(123));
 */
#ifdef _MSC_VER
 ...
 */
#define DLT_WITH_TAGS(TAG, ...) (void)dlt_with_tags(TAG, __VA_ARGS__, NULL)

/**
 * Send privacy level in logs
 * @param prlv uint privacy level
 */
#define DLT_WITH_PRIVACYLEVEL(PRLV) (void)dlt_with_prlv(PRLV)
...
/**
 * Add constant utf8-encoded string parameter with given length to the log
 * messsage. The string in @a TEXT does not need to be null-terminated, but the
 * copied string will be null-terminated at its destination in the message
 * buffer.
 * @param TEXT Constant UTF8-encoded string
 * @param LEN length in bytes to take from @a TEXT
 */
...
/**
 * Add string parameter with given length and "name" attribute to the log
 * messsage. The string in @a TEXT does not need to be null-terminated, but the
 * copied string will be null-terminated at its destination in the message
 * buffer.
 * @param TEXT ASCII string
...
/**
 * Add constant string parameter with given length and "name" attribute to the
 * log messsage. The string in @a TEXT does not need to be null-terminated, but
 * the copied string will be null-terminated at its destination
 * in the message buffer.
 * @param TEXT Constant ASCII string
 * @param LEN length in bytes to take from @a TEXT
...
#define DLT_SIZED_CSTRING_ATTR(TEXT, LEN, NAME)                                \
    (void)dlt_user_log_write_sized_constant_string_attr(&log_local, TEXT, LEN, \
                                                        NAME)
...
/**
 * Add utf8-encoded string parameter with given length and "name" attribute to
 * the log messsage. The string in @a TEXT does not need to be null-terminated,
 * but the copied string will be null-terminated at its destination in the
 * message buffer.
 * @param TEXT UTF8-encoded string
 * @param LEN length in bytes to take from @a TEXT
 * @param NAME "name" attribute
...
/**
 * Add constant utf8-encoded string parameter with "name" attribute to the log
 * messsage.
 * @param TEXT Constant UTF8-encoded string
 * @param NAME "name" attribute
 */
...
/**
 * Add constant utf8-encoded string parameter with given length and "name"
 * attribute to the log messsage. The string in @a TEXT does not need to be
 * null-terminated, but the copied string will be null-terminated at its
 * destination in the message buffer.
 * @param TEXT Constant UTF8-encoded string
 * @param LEN length in bytes to take from @a TEXT
 * @param NAME "name" attribute
 */
#define DLT_SIZED_CUTF8_ATTR(TEXT, LEN, NAME)                                  \
    (void)dlt_user_log_write_sized_constant_utf8_string_attr(&log_local, TEXT, \
                                                             LEN, NAME)
...
#define DLT_HEX8(UINT_VAR)                                                     \
    (void)dlt_user_log_write_uint8_formatted(&log_local, UINT_VAR,             \
                                             DLT_FORMAT_HEX8)
#define DLT_HEX16(UINT_VAR)                                                    \
    (void)dlt_user_log_write_uint16_formatted(&log_local, UINT_VAR,            \
                                              DLT_FORMAT_HEX16)
#define DLT_HEX32(UINT_VAR)                                                    \
    (void)dlt_user_log_write_uint32_formatted(&log_local, UINT_VAR,            \
                                              DLT_FORMAT_HEX32)
#define DLT_HEX64(UINT_VAR)                                                    \
    (void)dlt_user_log_write_uint64_formatted(&log_local, UINT_VAR,            \
                                              DLT_FORMAT_HEX64)
 ...
 */
#define DLT_TRACE_NETWORK(CONTEXT, TYPE, HEADERLEN, HEADER, PAYLOADLEN,        \
                          PAYLOAD)                                             \
    do {                                                                       \
        if ((CONTEXT).trace_status_ptr &&                                      \
            *((CONTEXT).trace_status_ptr) == DLT_TRACE_STATUS_ON) {            \
            (void)dlt_user_trace_network(&(CONTEXT), TYPE, HEADERLEN, HEADER,  \
                                         PAYLOADLEN, PAYLOAD);                 \
        }                                                                      \
    } while (false)
 ...
 * @param PAYLOAD pointer to network message payload
 */
#define DLT_TRACE_NETWORK_TRUNCATED(CONTEXT, TYPE, HEADERLEN, HEADER,          \
                                    PAYLOADLEN, PAYLOAD)                       \
    do {                                                                       \
        if ((CONTEXT).trace_status_ptr &&                                      \
            *((CONTEXT).trace_status_ptr) == DLT_TRACE_STATUS_ON) {            \
            (void)dlt_user_trace_network_truncated(                            \
                &(CONTEXT), TYPE, HEADERLEN, HEADER, PAYLOADLEN, PAYLOAD, 1);  \
        }                                                                      \
    } while (false)
 ...
 * @param PAYLOAD pointer to network message payload
 */
#define DLT_TRACE_NETWORK_SEGMENTED(CONTEXT, TYPE, HEADERLEN, HEADER,          \
                                    PAYLOADLEN, PAYLOAD)                       \
    do {                                                                       \
        if ((CONTEXT).trace_status_ptr &&                                      \
            *((CONTEXT).trace_status_ptr) == DLT_TRACE_STATUS_ON) {            \
            (void)dlt_user_trace_network_segmented(                            \
                &(CONTEXT), TYPE, HEADERLEN, HEADER, PAYLOADLEN, PAYLOAD);     \
        }                                                                      \
    } while (false)
...
#define DLT_LOG_STRING(CONTEXT, LOGLEVEL, TEXT)                                \
    do {                                                                       \
        if (dlt_user_is_logLevel_enabled(&CONTEXT, LOGLEVEL) ==                \
            DLT_RETURN_TRUE) {                                                 \
...
#define DLT_LOG_STRING_UINT_V2(CONTEXT, LOGLEVEL, TEXT, UINT_VAR)              \
    do {                                                                       \
        if (dlt_user_is_logLevel_enabled(&CONTEXT, LOGLEVEL) ==                \
            DLT_RETURN_TRUE) {                                                 \
            (void)dlt_log_string_uint_v2(&(CONTEXT), LOGLEVEL, TEXT,           \
                                         UINT_VAR);                            \
        }                                                                      \
    } while (false)
 ...
 * @param TRACESTATUS This is the trace status to be set for the whole
 * application
 */
#define DLT_SET_APPLICATION_LL_TS_LIMIT(LOGLEVEL, TRACESTATUS)                 \
    do {                                                                       \
        (void)dlt_set_application_ll_ts_limit(LOGLEVEL, TRACESTATUS);          \
    } while (false)

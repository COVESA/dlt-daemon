...
int read_command_line(DltSystemCliOptions *options, int argc, char *argv[])
{
    ...
    while ((opt = getopt(argc, argv, "c:hd")) != -1)
        ...
        case 'c': {
            ...
            memcpy(options->ConfigurationFileName, optarg, strlen(optarg) + 1);
            break;
        }
        case 'h': {
            usage(argv[0]);
    ...
    while (fgets(line, MAX_LINE, file) != NULL) {
        ...
        if (token[0] && value[0]) {
            ...
            /* Log files */
                ...
                memcpy(config->LogFile.Filename[config->LogFile.Count], value,
                       strlen(value) + 1);
            }
            ...
            /* Log Processes */
                ...
                MALLOC_ASSERT(
                    config->LogProcesses.Name[config->LogProcesses.Count]);
                memcpy(config->LogProcesses.Name[config->LogProcesses.Count],
                       value, strlen(value) + 1);
            }

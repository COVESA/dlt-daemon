...
int dlt_shell_injection_callback(uint32_t service_id, void* data, uint32_t length)
{
    ...
    DLT_LOG(shellContext, DLT_LOG_DEBUG, DLT_STRING("dlt-system-shell, injection callback"));
    ...
    DLT_LOG(
        shellContext, DLT_LOG_DEBUG, DLT_STRING("dlt-system-shell, injection injection id:"), DLT_UINT32(service_id));
    DLT_LOG(shellContext, DLT_LOG_DEBUG, DLT_STRING("dlt-system-shell, injection data:"), DLT_STRING(text));
    ...
    case 0x1001:
        ...
        if ((syserr = system(text)) != 0)
            DLT_LOG(
                shellContext, DLT_LOG_ERROR, DLT_STRING("dlt-system-shell, abnormal exit status."), DLT_STRING(text),
                DLT_INT(syserr));
        else
            DLT_LOG(shellContext, DLT_LOG_INFO, DLT_STRING("Shell command executed:"), DLT_STRING(text));

        break;
    default:
        DLT_LOG(
            shellContext, DLT_LOG_ERROR, DLT_STRING("dlt-system-shell, unknown command received."),
...
void init_shell()
{
    DLT_LOG(dltsystem, DLT_LOG_DEBUG, DLT_STRING("dlt-system-shell, register callback"));

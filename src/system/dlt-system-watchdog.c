...
int calculate_period(struct itimerspec *itval)
{
    ...
    if (watchdogUSec == NULL) {
        DLT_LOG(
            watchdogContext, DLT_LOG_ERROR,
            DLT_STRING("systemd watchdog timeout (WATCHDOG_USEC) is null\n"));
        return -1;
    }

    DLT_LOG(watchdogContext, DLT_LOG_DEBUG, DLT_STRING("watchdogusec: "),
            DLT_STRING(watchdogUSec));
    ...
    snprintf(str, 512,
             "systemd watchdog timeout: %u nsec - timer will be initialized: "
             "%u nsec\n",
    ...
    itval->it_interval.tv_sec = (long int)sec;
    itval->it_interval.tv_nsec = (long int)ns;
    itval->it_value.tv_sec = (long int)sec;
...
{
    DLT_REGISTER_CONTEXT(watchdogContext, "DOG",
                         "dlt system watchdog context.");
    ...
    if (timerfd < 0) {
        DLT_LOG(dltsystem, DLT_LOG_ERROR,
                DLT_STRING("Failed to create timer fd\n"));
        return -1;
    }
    ...
    if (timerfd_settime(timerfd, 0, &timerValue, NULL) < 0) {
        DLT_LOG(dltsystem, DLT_LOG_ERROR,
                DLT_STRING("Could not start timer\n"));
        return -1;
    }
    return 0;
}
...
#endif
{
    ...
    *received_message_since_last_watchdog_interval = 0;
    ...
    if (sd_notify(0, "WATCHDOG=1") < 0)
        DLT_LOG(watchdogContext, DLT_LOG_ERROR,
                DLT_STRING("Could not reset systemd watchdog\n"));

    DLT_LOG(watchdogContext, DLT_LOG_DEBUG,

 ...
 *
 * \copyright Copyright (C) 2015-2018 Advanced Driver Information Technology. \n
    ...
    while (token != NULL) {
        ...
        if ((p_token != NULL) && (strlen(p_token) != 0)) {
            ...
            while (con->p_control_msgs != NULL) {
                if (con->p_control_msgs->id == id) {
                    ...
                    con->p_control_msgs->interval =
                        (unsigned int)strtol(p_rest, NULL, 10);

                    if (con->p_control_msgs->interval == 0)
                        dlt_vlog(LOG_WARNING,
                                 "%s interval is %d. It won't be send "
                                 "periodically.\n",
            ...
            if (con->p_control_msgs == NULL) {
                ...
                if (dlt_gateway_allocate_control_messages(con) !=
                    DLT_RETURN_OK) {
                    dlt_log(LOG_ERR,
                            "Passive Control Message could not be allocated\n");
                    return DLT_RETURN_ERROR;
...
DLT_STATIC DltGatewayConf configuration_entries[GW_CONF_COUNT] = {
    [GW_CONF_IP_ADDRESS] = {.key = "IPaddress",
                            .func = dlt_gateway_check_ip,
                            .is_opt = 0},
    [GW_CONF_PORT] = {.key = "Port",
                      .func = dlt_gateway_check_port,
                      .is_opt = 1},
    [GW_CONF_ECUID] = {.key = "EcuID",
                       .func = dlt_gateway_check_ecu,
                       .is_opt = 0},
    [GW_CONF_CONNECT] = {.key = "Connect",
                         .func = dlt_gateway_check_connect_trigger,
                         .is_opt = 1},
    [GW_CONF_TIMEOUT] = {.key = "Timeout",
                         .func = dlt_gateway_check_timeout,
                         .is_opt = 0},
    [GW_CONF_SEND_CONTROL] = {.key = "SendControl",
 ...
 * @return Value from DltReturnValue enum
 */
DLT_STATIC DltReturnValue dlt_gateway_check_general_param(
    DltGateway *gateway, DltGatewayGeneralConfType ctype, char *value)
{
    if ((gateway == NULL) || (value == NULL)) {
        dlt_vlog(LOG_ERR, "%s: wrong parameter\n", __func__);
        return DLT_RETURN_WRONG_PARAMETER;
                                 ...
                                 int verbose)
{
    ...
    if (dlt_client_init_port(&gateway->connections[i].client,
                             gateway->connections[i].port, verbose) != 0) {
        ...
        dlt_log(LOG_CRIT,
                "dlt_client_init_port() failed for gateway connection\n");
        return DLT_RETURN_ERROR;
    }
...
int dlt_gateway_configure(DltGateway *gateway, char *config_file, int verbose)
{
    ...
    ret = dlt_config_file_check_section_name_exists(
        file, DLT_GATEWAY_GENERAL_SECTION_NAME);
    if (ret == -1) {
        /*
        ...
        dlt_vlog(LOG_WARNING,
                 "Missing General section in gateway. Using default interval "
                 "%d (secs)\n",
                 gateway->interval);
    }
    else {
    ...
    gateway->connections =
        calloc((size_t)gateway->num_connections, sizeof(DltGatewayConnection));
    ...
    for (i = 0; i < num_sections; i++) {
        ...
        if (strncmp(section, DLT_GATEWAY_GENERAL_SECTION_NAME,
            ...
            for (g = 0; g < GW_CONF_GENEREL_COUNT; g++) {
                ret = dlt_config_file_get_value(file, section,
            ...
            for (j = 0; j < GW_CONF_COUNT; j++) {
                ret = dlt_config_file_get_value(
                    file, section, configuration_entries[j].key, value);
...
int dlt_gateway_init(DltDaemonLocal *daemon_local, int verbose)
{
    ...
    if (gateway != NULL) {
        ...
        if (dlt_gateway_configure(
                gateway, daemon_local->flags.gatewayConfigFile, verbose) != 0) {
                                             ...
                                             int verbose)
{
    ...
    while (control_msg != NULL) {
        if ((control_msg->type == CONTROL_MESSAGE_ON_STARTUP) ||
            ...
            if (dlt_gateway_send_control_message(con, control_msg, NULL,
                                      ...
                                      DltDaemonLocal *daemon_local, int verbose)
{
    ...
    for (i = 0; i < gateway->num_connections; i++) {
        ...
        if ((con->status != DLT_GATEWAY_CONNECTED) &&
            ...
            if (ret == 0) {
                /* setup dlt connection and add to poll event loop here */
                if (dlt_gateway_add_to_event_loop(daemon_local, con, verbose) !=
                    DLT_RETURN_OK) {
                    dlt_log(LOG_ERR, "Gateway connection creation failed\n");
                    return DLT_RETURN_ERROR;
                }
            ...
            while (control_msg != NULL) {
                if ((control_msg->type == CONTROL_MESSAGE_PERIODIC) ||
                    (control_msg->type == CONTROL_MESSAGE_BOTH)) {
                    if (dlt_gateway_send_control_message(
                            con, control_msg, NULL, verbose) == DLT_RETURN_OK)
                        control_msg->req = CONTROL_MESSAGE_REQUESTED;
                }
                                                         ...
                                                         int req, int verbose)
{
    ...
    if (dlt_check_rcv_data_size(msg->datasize,
                                sizeof(DltServiceGetLogInfoResponse)) < 0)
        return DLT_RETURN_ERROR;

    /* if the request was send from gateway, clear all application and context
     * list */
    if (req == CONTROL_MESSAGE_REQUESTED) {
        /* clear application list */
        if (dlt_daemon_applications_clear(daemon, ecu, verbose) ==
            DLT_RETURN_ERROR) {
        ...
        /* clear context list */
        if (dlt_daemon_contexts_clear(daemon, ecu, verbose) ==
            DLT_RETURN_ERROR) {
    ...
    /* check response */
    if (dlt_message_payload(msg, resp_text, DLT_RECEIVE_BUFSIZE,
    ...
    resp = (DltServiceGetLogInfoResponse *)calloc(
        1, sizeof(DltServiceGetLogInfoResponse));
    ...
    if (dlt_set_loginfo_parse_service_id(resp_text, &resp->service_id,
                                         &resp->status) != DLT_RETURN_OK) {
        dlt_log(LOG_ERR, "Parsing GET_LOG_INFO failed\n");
        dlt_client_cleanup_get_log_info(resp);
        return DLT_RETURN_ERROR;
    }

    if (dlt_client_parse_get_log_info_resp_text(resp, resp_text) !=
        DLT_RETURN_OK) {
        dlt_log(LOG_ERR, "Parsing GET_LOG_INFO failed\n");
        dlt_client_cleanup_get_log_info(resp);
        return DLT_RETURN_ERROR;
    ...
    for (i = 0; i < resp->log_info_type.count_app_ids; i++) {
        ...
        /* add application */
        if (dlt_daemon_application_add(daemon, app.app_id, 0,
                                       app.app_description, -1, ecu,
                                       verbose) == 0) {
        ...
        for (j = 0; j < app.count_context_ids; j++) {
            ...
            if (dlt_daemon_context_add(
                    daemon, app.app_id, con.context_id, (int8_t)con.log_level,
                    (int8_t)con.trace_status, 0, -1, con.context_description,
                    ecu, verbose) == 0) {
                dlt_vlog(LOG_WARNING,
 ...
 */
DLT_STATIC int
dlt_gateway_parse_get_default_log_level(DltDaemon *daemon,
                                        DltDaemonLocal *daemon_local, char *ecu,
                                        DltMessage *msg, int verbose)
{
    ...
    if (dlt_check_rcv_data_size(
            msg->datasize, sizeof(DltServiceGetDefaultLogLevelResponse)) < 0) {
 ...
 * @return 0 on success, -1 otherwise
 */
DLT_STATIC int dlt_gateway_control_service_logstorage(
...
DltReturnValue
dlt_gateway_process_passive_node_messages(DltDaemon *daemon,
                                          ...
                                          DltReceiver *receiver, int verbose)
{
    ...
    if (dlt_receiver_receive(receiver) <= 0) {
        ...
        if (con->reconnect_cnt < DLT_GATEWAY_RECONNECT_MAX) {
            ...
            if (dlt_event_handler_unregister_connection(
                    &daemon_local->pEvent, daemon_local, receiver->fd) != 0)
    ...
    while (dlt_message_read(&msg, (unsigned char *)receiver->buf,
                            (unsigned int)receiver->bytesRcvd, 0,
                            verbose) == DLT_MESSAGE_ERROR_OK) {
        DltStandardHeaderExtra *header =
            (DltStandardHeaderExtra *)(msg.headerbuffer +
        ...
        if (strncmp(header->ecu, con->ecuid, DLT_ID_SIZE) == 0) {
            ...
            if (id == DLT_SERVICE_ID_GET_LOG_INFO) {
                while (control_msg) {
                    if (control_msg->id == id) {
                        if (dlt_gateway_parse_get_log_info(
                                daemon, header->ecu, &msg, control_msg->req,
                                verbose) == DLT_RETURN_ERROR)
                            dlt_log(LOG_WARNING,
                                    "Parsing GET_LOG_INFO message failed!\n");

                        /* Check for logstorage */
                        dlt_gateway_control_service_logstorage(
                ...
                if (dlt_gateway_parse_get_default_log_level(
                        daemon, daemon_local, header->ecu, &msg, verbose) ==
                    DLT_RETURN_ERROR)
                    dlt_log(LOG_WARNING,
            ...
            if (dlt_set_storageheader(msg.storageheader, msg.headerextra.ecu) ==
                DLT_RETURN_ERROR) {
            ...
            /* it is possible that a partial log was received through the last
             * recv call */
            /* however, the rest will never be received since the socket will be
             * closed by above method */
            /* as such, we need to reset the receiver to prevent permanent
             * corruption */
            b_reset_receiver = true;
        }

        if (msg.found_serialheader) {
            if (dlt_receiver_remove(receiver, (int)((size_t)msg.headersize +
                                                    (size_t)msg.datasize -
                                                    sizeof(DltStorageHeader) +
                                      ...
                                      DltReceiver *receiver, int verbose)
{
    ...
    dlt_gateway_establish_connections(&daemon_local->pGateway, daemon_local,
                                      verbose);
...
int dlt_gateway_forward_control_message(DltGateway *gateway,
                                        DltDaemonLocal *daemon_local,
                                        DltMessage *msg, char *ecu, int verbose)
{
    int i = 0;
    int ret = 0;
    DltGatewayConnection *con = NULL;
    ...
    if ((gateway == NULL) || (daemon_local == NULL) || (msg == NULL) ||
        (ecu == NULL)) {
        dlt_vlog(LOG_ERR, "%s: wrong parameter\n", __func__);
        return DLT_RETURN_WRONG_PARAMETER;
    }
    ...
    if (con->send_serial) { /* send serial header */
        ret = (int)send(con->client.sock, (const void *)dltSerialHeader,
    ...
    if (ret == -1) {
        ...
        ret = (int)send(con->client.sock, msg->databuffer,
...
int dlt_gateway_forward_control_message_v2(DltGateway *gateway,
                                           DltDaemonLocal *daemon_local,
                                           DltMessageV2 *msg, uint8_t eculen,
...
DltReturnValue dlt_gateway_process_on_demand_request(
    DltGateway *gateway, DltDaemonLocal *daemon_local, char *node_id,
    int conn_status, int verbose)
{
    int i = 0;
    DltGatewayConnection *con = NULL;
    ...
    else if (conn_status == 0) /* disconnect*/
    {
        ...
        if (dlt_event_handler_unregister_connection(
                &daemon_local->pEvent, daemon_local, con->client.sock) != 0)
            dlt_log(LOG_ERR,
                                     ...
                                     void *data, int verbose)
{
    ...
    if ((control_msg->type != CONTROL_MESSAGE_ON_DEMAND) &&
        (con->sendtime_cnt > 0)) {
    ...
    case DLT_SERVICE_ID_SET_LOG_LEVEL:
        ...
        if (data == NULL) {
            dlt_vlog(LOG_WARNING,
                     "Insufficient data for %s received. Send control request "
                     "failed.\n",
        ...
        return dlt_client_send_log_level(&con->client, req->apid, req->ctid,
                                         req->log_level);
        break;
    default:
                                        ...
                                        void *data, int verbose)
{
    ...
    case DLT_SERVICE_ID_SET_LOG_LEVEL:
        ...
        DltServiceSetLogLevel *req = (DltServiceSetLogLevel *)data;
        return dlt_client_send_log_level_v2(&con->client, req->apid, req->ctid,
                                            req->log_level);
        break;
    default:
        dlt_vlog(LOG_WARNING, "Cannot forward request: %s.\n",
...
DltGatewayConnection *dlt_gateway_get_connection(DltGateway *gateway, char *ecu,
                                                 int verbose)
{
    DltGatewayConnection *con = NULL;
    int i = 0;

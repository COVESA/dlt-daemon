...
TEST(t_trace_load_keep_message_v2, normal)
{
    ...
    const auto set_extended_header = [&daemon_local]() {
        daemon_local.msg.extendedheader =
            (DltExtendedHeaderV2 *)(daemon_local.msg.headerbuffer +
    ...
    const auto log_until_hard_limit_reached =
        [&daemon, &daemon_local](DltDaemonApplication *app) {
            while (trace_load_keep_message_v2(app, _trace_load_send_size,
    ...
    for (int i = 0; i < num_apps; i++) {
        apps[i] = dlt_daemon_application_add_v2(&daemon, (char *)app_ids[i],
                                                pid, (char *)desc, fd, ecu, 0);
    ...
    EXPECT_FALSE(
        trace_load_keep_message_v2(&app, 42, &daemon, &daemon_local, 0));
    ...
    EXPECT_FALSE(trace_load_keep_message_v2(apps[0], _trace_load_send_size,
                                            &daemon, &daemon_local, 0));
    ...
    EXPECT_FALSE(trace_load_keep_message_v2(apps[1], _trace_load_send_size,
                                            &daemon, &daemon_local, 0));
    // CT01 has reached its limit, make sure CT02 still can log
    memcpy(daemon_local.msg.extendedheader->ctid, "CT02", DLT_ID_SIZE);
    EXPECT_TRUE(trace_load_keep_message_v2(apps[1], _trace_load_send_size,
                                           &daemon, &daemon_local, 0));
    ...
    EXPECT_FALSE(trace_load_keep_message_v2(apps[2], _trace_load_send_size,
                                            &daemon, &daemon_local, 0));
    // Context logging should still be possible
    memcpy(daemon_local.msg.extendedheader->ctid, "CT01", DLT_ID_SIZE);
    EXPECT_TRUE(trace_load_keep_message_v2(apps[2], _trace_load_send_size,
                                           &daemon, &daemon_local, 0));

    // Test not configured context
    memcpy(daemon_local.msg.extendedheader->ctid, "CTXX", DLT_ID_SIZE);
    EXPECT_EQ(trace_load_keep_message_v2(apps[1], _trace_load_send_size,
                                         &daemon, &daemon_local, 0),

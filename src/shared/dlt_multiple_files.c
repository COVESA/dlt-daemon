        ...
        if ((strncmp(files[i]->d_name, file_name, (size_t)len) == 0) &&
            ...
            if (tmp_old == NULL || strlen(tmp_old) > strlen(files[i]->d_name) ||
                (strlen(tmp_old) == strlen(files[i]->d_name) &&
                 strcmp(tmp_old, files[i]->d_name) > 0)) {
                /* when file name is smaller or lexicographically
                 * smaller (same length), it is older */
                tmp_old = files[i]->d_name;
            }

            if (tmp_new == NULL || strlen(tmp_new) < strlen(files[i]->d_name) ||
                (strlen(tmp_new) == strlen(files[i]->d_name) &&
                 strcmp(tmp_new, files[i]->d_name) < 0)) {
                /* when file name is longer or lexicographically
                 * larger (same length), it is younger */
...
unsigned int multiple_files_buffer_get_idx_of_log_file(char *file)
{
    ...
    /* skip first token, we are interested in 2. token because of
     * log file name */
    (void)strtok(file, d);
    token = strtok(NULL, d);

...
trap cleanup EXIT
# Args: <dlt-daemon> <test-bin> <ctrl-bin> <ctxnum> <dlt-convert>
DAEMON_BIN="$1"
TEST_BIN="$2"
CTRL_BIN="$3"
CTXNUM="$4"
...
if [ -n "$CTRL_BIN" ] && [ "$CTRL_BIN" != "none" ]; then
    "$CTRL_BIN" -s -C "$DIR/dlt.conf"
fi
sleep 2
...
find "$DIR" -name '*.dlt' -print0 | xargs -0 -r "$CONVERT_BIN" -a

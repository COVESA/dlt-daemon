...
int dlt_receive_message_callback(DltMessage *message, void *data)
{
    ...
    if (((dltdata->fvalue || dltdata->jvalue) == 0) ||
        ...
        if (dltdata->xflag) {
            dlt_message_print_hex(message, text, DLT_RECEIVE_BUFSIZE,
                                  dltdata->vflag);
        }
        else if (dltdata->aflag) {

            dlt_message_header(message, text, DLT_RECEIVE_BUFSIZE,
                               dltdata->vflag);

            printf("%s ", text);

            dlt_message_payload(message, text, DLT_RECEIVE_BUFSIZE,
                                DLT_OUTPUT_ASCII, dltdata->vflag);

            printf("[%s]\n", text);
        }
        else if (dltdata->mflag) {
            dlt_message_print_mixed_plain(message, text, DLT_RECEIVE_BUFSIZE,
                                          dltdata->vflag);
        }
        ...
        if (dltdata->ovalue) {
            ...
            if (dltdata->climit > -1) {
                uint32_t bytes_to_write =
                    (uint32_t)message->headersize + (uint32_t)message->datasize;
